    ...
    async def _run_coordinator_dispatch(
        ...
        try:
            ...
            _plan_prompt = (
                ...
                + f"\nЗАДАЧА: Назначь каждому агенту КОНКРЕТНОЕ действие исходя из его РЕАЛЬНЫХ интеграций и АКТИВНЫХ ЦЕЛЕЙ.\n"
                "ПРИНЦИП АВТОНОМНОСТИ: Агенты — это сотрудники, которые ДЕЛАЮТ сами, НЕ спрашивают разрешения на каждый шаг.\n"
                ...
                "• Задачи назначай ДЕЙСТВЕННЫЕ: 'найди → напиши → сохрани → отправь', не 'исследуй для понимания'.\n"
                "• Если у агента есть email → ОТПРАВЛЯЙ письма новым контактам, не повторяя уже написанных.\n"
                "• save_note обязателен при любом исследовании — агент должен зафиксировать результат.\n"
            ...
            if _results_summary:
                try:
                    ...
                    _report_prompt = (
                        ...
                        + f"Состояние целей:\n{_goals_state_now}\n\n"
                        f"Правила:\n"
                        f"- Называй КОНКРЕТНО: что нашёл каждый агент, кому написали, цифры\n"
                        f"- Описывай каждого агента ОТДЕЛЬНО — НЕ придумывай причинно-следственные связи между их действиями\n"
                        ...
                        f"- СИНТЕЗ ПЕРСПЕКТИВ: если разные агенты нашли разное (возможности vs риски vs примеры) — "
                        f"объедини в цельную картину: «Кристина нашла X, а Марк обнаружил Y — это значит Z для цели»\n"
                        f"- Если агенты нашли ПРОТИВОРЕЧИВЫЕ данные — это ценно! Скажи: «Есть разные сигналы: ... — стоит разобраться»\n"
                        + _note_hint
