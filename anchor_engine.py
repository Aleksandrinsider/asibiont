...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None) -> str:
    ...
    return (
        ...
        "   • НЕЙТРАЛЬНОЕ (уточнения, 'расскажите подробнее') → отвечай вежливо\n"
        "   • НЕГАТИВНОЕ (не интересно, не пишите, отписка, stop, любой отказ на ЛЮБОМ языке) → НЕ ОТВЕЧАЙ!\n"
        "   Если контакт попросил не писать (на любом языке!) — вызови update_email_contact_status(email, status='unsubscribed').\n"
        "   Уважай волю человека. Один лишний ответ после отказа = потерянный контакт навсегда.\n"
        "   Пиши ответ НА ЯЗЫКЕ контакта! Если ответ на греческом — пиши на греческом, испанском — на испанском.\n"
