...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None) -> str:
    ...
    if _has_email:
        _action_plan = (
            ...
            "  Или отправь письмо конкретному человеку (из контактов или найди через web_search).\n"
            "  Или найди email адреса через web_search → сразу отправь каждому.\n"
        )
    elif _has_rss:
        _action_plan = (
            ...
            "  Или исследуй тему глубже через web_search — ищи конкретные контакты, сообщества.\n"
        )
    elif _has_github:
        _action_plan = (
            "У тебя есть GitHub API.\n\n"
            "Сейчас сядь и сделай ОДНО из этого:\n"
            "  Найди разработчиков/контрибьюторов через run_agent_action (поиск по интересам).\n"
            "  Или посмотри issues/PRs релевантных проектов — ищи контакты.\n"
            "  Если нашёл email — делегируй коллеге: DELEGATE[Имя]: отправить приглашение.\n"
        )
    elif _has_content:
        _action_plan = (
            ...
            "  Или найди целевые каналы/сообщества через web_search.\n"
        )
    else:
        _action_plan = (
    ...
    return (
        ...
        "Если ничего полезного не получил — верни пустую строку, не выдумывай.\n"
