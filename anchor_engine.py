...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None) -> str:
    ...
    return (
        ...
        "8. Задания МОГУТ БЫТЬ КОСВЕННЫМИ: изучи тренды → предложи темы, найди экспертов → сохрани.\n"
    ...
    async def _run_coordinator_dispatch(
        ...
        try:
            ...
            _plan_prompt = (
                ...
                "12. ТАКТИЧЕСКОЕ РАЗНООБРАЗИЕ — для одной и той же цели есть 5 разных стратегий:\n"
                "    А) Прямой поиск → письмо (GitHub/email)\n"
                "    Б) Сообщества и площадки (форумы, Telegram-группы, Discord, Reddit, HN)\n"
                "    В) Контент-приманка: опубликуй полезный материал с CTA — люди приходят сами\n"
                "    Г) Реферал: попроси уже ответивших порекомендовать 2-3 человека\n"
                "    Д) Смена сегмента: другая аудитория, другая площадка, другой запрос\n"
                "    → ЖЁСТКОЕ ПРАВИЛО: Если агент в последнем цикле делал А) — в ЭТОМ цикле назначь Б), В), Г) или Д).\n"
                "      Запрещено назначать одну и ту же стратегию 2+ циклов подряд. Используй блок ПРОШЛЫЙ ЦИКЛ выше.\n"
                "13. МЫСЛИТЬ КОМПЛЕКСНО — координатор должен искать НЕПРЯМЫЕ пути к цели:\n"
                "    НЕ ТОЛЬКО: Марк ищет контакты → Кристина пишет письма (повторяется).\n"
                "    НО ТАКЖЕ: Марк анализирует тренды → формулирует ценное предложение → Кристина публикует контент с CTA → люди сами приходят.\n"
                "    ИЛИ: Кристина проверяет ответы на письма → находит заинтересованного → Марк исследует его профиль/компанию → Кристина персонализирует оффер.\n"
                "    → ВОПРОС: какие действия КОСВЕННО помогут цели? Не только 'найти 50 человек', но и 'создать причину почему они захотят участвовать'.\n"
                "14. АГЕНТЫ БЕЗ ИНТЕГРАЦИЙ (интеграции=[нет]) — СТРОГОЕ ПРАВИЛО:\n"
                "    ⛔ ЗАПРЕЩЕНО назначать: check_emails, run_agent_action (требует внешнего API).\n"
                "    ✅ РАЗРЕШЕНО: research_topic, web_search, find_relevant_contacts_for_task, save_note, add_task, create_post.\n"
                "    Думай о таком агенте как об аналитике без спецоборудования: ищет, изучает, планирует, пишет — но НЕ дёргает внешние API.\n"
                "    Конкретные задачи для агента без интеграций:\n"
                "      • research_topic('Где найти [ЦА] — Telegram, Reddit, форумы, конференции') → add_task с конкретными каналами\n"
                "      • web_search('[тема] telegram group OR discord community') → save_note с ссылками → add_task 'вступить и написать'\n"
                "      • find_relevant_contacts_for_task(task=задача) → save_note с контактами → add_task с планом outreach\n"
                "      • create_post(poleznyy_kontent) → publish_to_telegram (если канал подключён)\n\n"
