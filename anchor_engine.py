    ...
    async def _ai_decide_and_compose(self, user, anchors: list, session) -> str | None:
        ...
        try:
            ...
            prompt_parts = [
                ...
                "3. СНАЧАЛА вызови инструменты (research_topic, get_news_trends, list_tasks) для получения РЕАЛЬНЫХ данных. ПОТОМ пиши сообщение на основе результатов.",
                "4. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: писать 'проверил тренды', 'посмотрел новости', 'нашёл данные' если ты НЕ вызвал инструмент. Это ФАЛЬСИФИКАЦИЯ и обман пользователя.",
                "5. Принцип: ВЫЗВАЛ ИНСТРУМЕНТ → ПОЛУЧИЛ ДАННЫЕ → ПОКАЗАЛ РЕЗУЛЬТАТ → ПРЕДЛОЖИЛ ДЕЙСТВИЕ.",
                "6. Не создавай и не меняй задачи без просьбы пользователя.",
                ...
                "9. Если у пользователя НЕТ ЗАДАЧ — предложи создать первую задачу на основе его интересов/целей",
                "10. КОНКРЕТИКА: каждое сообщение содержит минимум 1 факт/цифру/источник из вызванного инструмента. Общие фразы без данных = лучше SKIP чем вода",
                "",
                f"=== ВРЕМЯ ===",
            ...
            result = await agent.generate_system_message(
                ...
                mode='anchor',
                instruction="СНАЧАЛА вызови research_topic или get_news_trends по теме якорей, чтобы получить РЕАЛЬНЫЕ данные. Потом реши: написать или SKIP. Если пишешь — дай конкретное полезное сообщение с реальными фактами из инструментов. Если у пользователя нет задач — предложи создать первую.",
