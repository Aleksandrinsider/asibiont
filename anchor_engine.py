...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None, team_history=None, python_code=None) -> str:
    ...
    _adaptive_block = (
        ...
        "  → Сразу DELEGATE[Кристина/агент с Gmail]: проверь входящие по [цели]\n"
    ...
    async def _run_coordinator_dispatch(
        ...
        try:
            ...
            _plan_prompt = (
                ...
                "• Думай как живой специалист: задай себе вопрос 'Что здесь НЕ очевидно?' "
                "перед тем как писать план. Иногда лучший следующий шаг — поставить гипотезу "
                "и проверить её через web_search или research_topic.\n\n"
