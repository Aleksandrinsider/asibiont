...
_AGENT_DISPATCH_TRIGGERS: dict[str, str] = {
    ...
    'goal_autopilot_review': (
        "Ты — автономный агент. Используй ВЕСЬ свой арсенал для продвижения целей пользователя.\n\n"
        "ПРАВИЛА:\n"
        "1. ИЗУЧИ recent_messages и recent_actions — НЕ ПОВТОРЯЙ уже сделанное\n"
        "2. Если задача уже создана (pending/in_progress) — НЕ создавай дублей\n"
        ...
        "ЗАПРЕЩЕНО: update_goal_progress, delete_task. НЕ ОТПРАВЛЯЙ email на адрес самого пользователя.\n"
        "ФОРМАТ ОТВЕТА: живой отчёт от первого лица — что сделала, что нашла, каков результат."
    ),
}
