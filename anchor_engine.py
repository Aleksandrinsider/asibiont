    ...
    async def _ai_decide_and_compose(self, user, anchors: list, session) -> str | None:
        ...
        try:
            ...
            prompt_parts = [
                ...
                "— email_need_leads: Кампания активна, но ЧЕРНОВИКОВ НЕТ — нужно найти новых контактов и добавить через add_email_leads. В data: campaign_goal, target_audience, offer, remaining_daily (сколько ещё можно сегодня). ИЩИ ПАРАЛЛЕЛЬНО минимум через 3-5 запросов. ЕСЛИ target_audience на русском: (1) web_search('vc.ru [роль/ниша] email автор'), (2) web_search('habr.com [тема] автор email контакт'), (3) web_search('[ниша] блог автор email портфолио'), (4) web_search('[отрасль] основатель CEO email telegram'), (5) web_search('[ключевое слово] автор разработчик email контакт'). ЕСЛИ target_audience на английском: (1) web_search('medium.com [role/niche] author email'), (2) web_search('producthunt.com [topic] maker email'), (3) web_search('[role] [industry] email contact portfolio'), (4) web_search('[industry] founder CEO email profile'), (5) web_search('indiehackers.com [niche] author email contact'). ВНИМАНИЕ: поисковик — DuckDuckGo, оператор site: НЕ работает! Просто пишите доменное имя как слово (github.com, habr.com). Оператор OR тоже не работает. Собирай ТОЛЬКО личные email (john@company.com — ОК; info@company.com — НЕТ). Цель: минимум 5-10 контактов за один запуск. Передавай в add_email_leads JSON [{{\"email\":...,\"name\":...,\"company\":...,\"context\":...}}] с пояснением почему этот человек релевантен цели кампании. Верни SKIP после добавления — engine сам запустит отправку.",
                "",
                "ПРИНЦИПЫ:",
