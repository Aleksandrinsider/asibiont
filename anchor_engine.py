                ...
                if anchor.anchor_type == 'goal_autopilot_review' and _chosen_id != 0:
                    ...
                    try:
                        ...
                        _coord_prompt = (
                            ...
                            "2. СТРАТЕГИЯ — объясни ПОЧЕМУ именно этот подход: 'в прошлый раз Telegram не дал результатов, давай попробуем LinkedIn'\n"
                            ...
                            "  «Марк, прошлые два раза мы искали в Telegram — результатов мало. "
                            "Попробуй сегодня hh.ru или LinkedIn — там реальные QA с контактами. "
                            "Нужны 2-3 человека с публичным email.»\n"
                            "  «Кристина, Марк нашёл 3 контакта на GitHub. Напиши каждому персональное письмо — "
                    ...
                    try:
                        ...
                        if _ack_ok:
                            ...
                            _ack_prompt = (
                                ...
                                f"ASI поручил{'а' if _is_ack_fem else ''} тебе: «{_coord_text[:200]}».\n"
                                f"Напиши 1-2 предложения: подтверди задачу и скажи КАК ИМЕННО ты её будешь решать — "
                                f"какой конкретный подход или канал используешь.\n"
                                f"От первого лица, {'по-женски' if _is_ack_fem else 'по-мужски'}. Живо, с характером.\n"
                                f"✅ Примеры:\n"
                                f"  «Принято! Начну с проверки почты — должны быть ответы от вчерашней рассылки.»\n"
                                f"  «Ок, поищу на hh.ru — там реальные специалисты с контактами.»\n"
                                f"  «Сделаю. Сначала гляну свежие новости по теме, потом подготовлю выжимку.»\n"
                                f"  «Есть идея — попробую зайти через LinkedIn, раньше этот канал не использовали.»\n"
                                f"❌ Нельзя: технические термины, названия инструментов, своё имя в третьем лице, "
                                f"дословно повторять поручение ASI."
                            )
        ...
        try:
            ...
            for _p_cr in _profiles:
                ...
                if _has_email_cr and _is_github_agent:
                    ...
                    _cap_rules_lines.append(
                        f"  📰 {_p_cr['name']} [АНАЛИТИК + контент]: "
                        f"Основное — мониторинг новостей/трендов, аналитика, создание контента. "
                        f"Инструменты: run_agent_action(get_latest), web_search, research_topic, "
                        f"get_news_trends, create_post, publish_to_telegram. "
                        f"Если нужны контакты/люди — DELEGATE коллеге с GitHub/Email API (не изобретать костыли через web_search). "
                        f"Если нужной интеграции нет ни у кого → предложить пользователю её подключить."
                    )
            _cap_rules_str = (
            ...
            while _executed < _MAX_DYNAMIC_STEPS:
                ...
                _agent_prompt = (
                    ...
                    f"\n  • Это список того что ты РЕАЛЬНО УМЕЕШЬ. Не обещай действий за его пределами."
                    f"\n  • Если задача требует то чего у тебя нет — скажи прямо какая интеграция нужна. НЕ говори «я могу» если не можешь."
                    ...
                    f"\n  Из этого пересечения — твой план. Остальное НЕ предлагай как «я могу»."
                    f"\n  Если пересечение пустое → честно скажи что нужна другая интеграция и какая именно."
                    f"\n  [АНАЛИТИК] Оцени задание: какой РЕАЛЬНЫЙ результат нужен пользователю? Не формальное выполнение, а польза."
                    f"\n  [СТРАТЕГ] Какой из ТВОИХ инструментов даст максимум для этой цели? Есть ли короткий путь?"
                    f"\n  [КРИТИК] Что может пойти не так? Данные могут быть устаревшими? Запрос слишком общий? Уточни сам."
                    f"\n  Итог: {'действуй' if not _ag_is_fem else 'действуй'} по лучшему плану — только то, что реально можешь сделать."
            ...
            if _results_for_report:
                try:
                    ...
                    _report_prompt = (
                        ...
                        + f"Состояние целей:\n{_goals_state_now}\n\n"
                        f"Правила:\n"
                        f"- Пиши КАК РУКОВОДИТЕЛЬ ПРОЕКТА — живо, по-человечески, с оценкой.\n"
                        f"- Называй агентов по именам: 'Кристина проверила почту и нашла ответ от...' НЕ 'команда работала'.\n"
