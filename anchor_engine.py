    ...
    async def _ai_decide_and_compose(self, user, anchors: list, session) -> str | None:
        ...
        try:
            ...
            prompt_parts = [
                ...
                "— email_need_leads: Кампания активна, но ЧЕРНОВИКОВ НЕТ — нужно найти новых контактов и добавить через add_email_leads. В data: campaign_goal, target_audience, offer, remaining_daily (сколько ещё можно сегодня). ИЩИ ПАРАЛЛЕЛЬНО минимум через 3-5 запросов. ЕСЛИ target_audience на русском: (1) web_search('site:vc.ru [роль/ниша] email автор'), (2) web_search('site:habr.com [тема] email'), (3) web_search('site:spark.ru [ниша] email контакт'), (4) web_search('[отрасль] основатель CEO email telegram'), (5) web_search('[ключевое слово] автор блог email -info@ -support@'). ЕСЛИ target_audience на английском: (1) web_search('site:medium.com [role/niche] email'), (2) web_search('site:producthunt.com [topic] maker email'), (3) web_search('site:linkedin.com [role] email contact'), (4) web_search('[industry] founder CEO email \"@gmail.com\"'), (5) web_search('site:substack.com [niche] author email'). Собирай ТОЛЬКО личные email (john@company.com — ОК; info@company.com — НЕТ). Цель: минимум 5-10 контактов за один запуск. Передавай в add_email_leads JSON [{{\"email\":...,\"name\":...,\"company\":...,\"context\":...}}] с пояснением почему этот человек релевантен цели кампании. Верни SKIP после добавления — engine сам запустит отправку.",
                "",
                "ПРИНЦИПЫ:",
