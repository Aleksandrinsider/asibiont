...
_INTEGRATION_PLANS = [
    ...
    (lambda c: any(w in c for w in ('mail', 'почт', 'email', 'imap', 'smtp', 'gmail', 'yandex', 'mailru')),
     ...
     ["A) check_emails → есть ответ → ПРОЧИТАЙ ТЕКСТ → КЛАССИФИЦИРУЙ (интерес/вопрос/отказ) → "
      ...
      "📅 СОЗВОН/ВСТРЕЧА: если контакт предлагает звонок/встречу — сначала уточни у пользователя через "
      "send_message_to_user('Контакт [имя] хочет созвон [дата/время]. Подтвердить? Пришли ссылку на Zoom.'). "
      "После подтверждения → reply_to_outreach_email + add_task с датой встречи. "
      "💡 Вместо плейсхолдеров вроде [вставьте ссылку] — пиши 'ссылку пришлю отдельно'.",
...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None, team_history=None, python_code=None) -> str:
    ...
    if _has_rss and not _has_github:
        _rss_rules = (
            "\n📰 ТВОЯ РОЛЬ — КОНТЕНТ И АНАЛИТИКА. GitHub API и Email API недоступны.\n"
            "  • Доступные инструменты: run_agent_action(точные RSS action-имена из списка выше), web_search, research_topic,\n"
            "    create_post, publish_to_telegram, get_news_trends\n"
            "  • Используй точные action-имена из профиля агента, не выдумывай generic вроде get_latest, read_rss_feed.\n"
            "\nℹ️ ВМЕСТО GitHub/Email:\n"
            "  • Для поиска контактов и outreach делегируй коллеге с GitHub/Email\n"
            "  • Или предложи пользователю подключить соответствующую интеграцию\n"
        )
    ...
    if not _has_imap:
        _no_imap_block = (
            "\nℹ️ check_emails недоступен (нет IMAP). "
            "Делегируй проверку почты агенту с Gmail: DELEGATE[Имя]: проверь входящие.\n"
        )
    ...
    _capability_limits_block = (
        "\n\nℹ️ ТЕХНИЧЕСКИЕ ВОЗМОЖНОСТИ ПЛАТФОРМЫ:\n"
        + '\n'.join(f"  {l}" for l in _platform_limits) + '\n'
        "→ Опирайся на интеграции из блока ИНТЕГРАЦИИ АГЕНТА выше.\n"
        "  Если чего-то нет в списке — предложи пользователю подключить, если это поможет цели.\n"
        "→ Прогресс = реальные действия. Стремись доводить каждое исследование до конкретного действия.\n"
        "→ update_goal_progress: при подтверждённом результате, не чаще 1 раза в 30 минут.\n"
    )
            ...
            try:
                ...
                if _cancelled:
                    ...
                    _cancelled_tasks_block = (
                        ...
                        + '\n'.join(_ct_lines) + '\n'
                        "  → Эти подходы уже пробовались. Подумай о другом пути к цели.\n"
                    )
            finally:
                _sess_ct.close()
    ...
    return (
        ...
        "6. Разнообразие подходов: каждый цикл старайся пробовать другой инструмент или метод.\n"
        "7. Персонализируй: упомяни проект/стек получателя. Не пиши повторно тем же людям.\n"
        ...
        "   🟡 ВОПРОС → ответь на конкретный вопрос, дай факт + мягкий CTA\n"
        "   🔴 ОТКАЗ → не отвечай, система отпишет\n"
        "   ⚪ НЕЯСНО → если есть вопрос — ответь; если нет — жди\n"
        "   Пиши на языке контакта. check_emails покажет классификацию.\n"
        "9. add_task — для внутренних шагов (source=agent), не для задач пользователю.\n"
        "10. Отчёт = факты и цифры, сплошной текст 2-3 абзаца. Формат мессенджера, без списков и заголовков.\n"
        "    Блокер → начни с 'БЛОКЕР:' — ASI спросит пользователя.\n"
        "11. Стратегическая инициатива: если прогресс стоит — предложи пользователю смену подхода с аргументами.\n"
        "12. Используй работу коллег: если есть контакты/данные от команды — работай с ними.\n"
        "13. Прямой путь не работает → переключись на косвенный (контент, community, партнёрства).\n"
        "14. Выбирай инструменты сам: какой канал быстрее даст результат для этой цели?\n"
        "15. Ошибка или лимит → сразу переключись на альтернативу из 'АДАПТИВНЫЕ РЕШЕНИЯ'.\n"
        "16. Создавай ценность даже в тупике: заготовь контент, делегируй, предложи стратегию.\n"
        "17. Команда — единый организм: каждый агент может больше своей основной роли.\n"
        "    Делегирование = разумное командное решение. DELEGATE[Имя]: задача с данными.\n"
        "18. Исследование → действие: доводи каждый поиск до create_post, save_email_contact, delegate_task или update_goal_progress.\n"
        "19. Нашёл контакт → сразу save_email_contact → send_outreach_email. Действие, не план.\n"
...
class AnchorEngine:
        ...
        try:
            ...
            _asi_synth = _NS_ap(
                ...
                personality=(
                    ...
                    'КАЖДЫЙ ЦИКЛ ищи НОВЫХ людей на РАЗНЫХ площадках — не зацикливайся на одних и тех же контактах. '
                    'Меняй запросы, ниши, платформы. Разнообразие = рост. '
                    + _delegate_examples +
                    'Действуй инструментами, не описывай намерения.'
                ),
            ...
            if recent_proactive_topics:
                task_text += (
                    "\n\n💡 О ЧЁМ УЖЕ ПИСАЛ (ищи новую информацию):\n"
                    + '\n'.join(f"  • {t}" for t in recent_proactive_topics[:5])
                    + "\nПостарайся найти новые факты и подходы, а не повторять уже известное."
                )
            ...
            if _tool_freq:
                ...
                if _overused:
                    ...
                    task_text += (
                        f"\n\n⚠️ Часто используемые инструменты: {_ban_list} — {4}+ раз за 48ч. "
                        f"Попробуй другой подход для разнообразия."
                    )
            ...
            if _exhausted:
                ...
                task_text += (
                    f"\n\n� Стратегии с низкой эффективностью (>50% не дали результата — рассмотри альтернативы):\n"
                    + '\n'.join(
            ...
            if _feasibility:
                task_text += (
                    "\n\n[СИСТЕМНАЯ АНАЛИТИКА — внутренняя информация для корректировки действий. "
                    "Не пересказывай эти данные пользователю напрямую — используй для принятия решений]:\n"
                ...
                if anchor.anchor_type == 'goal_autopilot_review' and _chosen_id != 0:
                    ...
                    try:
                        ...
                        _coord_prompt = (
                            ...
                            f"\n  1) Что {_chosen_name} делал{'а' if _chosen_name and _chosen_name[-1] in 'аяАЯ' else ''} в последний раз?"
                            f"\n  2) Что нового можно попробовать? Новая аудитория, новый подход, новая площадка."
                            f"\n  3) Если прошлый подход не дал результата — предложи что-то принципиально другое.\n"
                            + f"\nНапиши 2-3 предложения. Обратись по имени. Будь конкретным:\n"
                            "1. КОНКРЕТНОЕ действие — не 'займись целями', а 'проверь ответы в почте' или 'найди 3 контакта на hh.ru'\n"
                            "2. СТРАТЕГИЯ — объясни ПОЧЕМУ именно этот подход\n"
                            "3. ОЖИДАНИЕ — что ты ждёшь в отчёте\n"
                            "4. КАНАЛЫ — предлагай подключённые каналы из списка выше\n"
                            "5. Если один агент нашёл контакты — назначь другому отправить письма ЭТИМ людям\n"
                            "6. АУДИТОРИЯ-FIRST: (кто аудитория) + (какая их боль) + (персональное предложение)\n"
                            "Тон: как старший коллега — уважительно, но конкретно.\n"
            ...
            if _is_inbox_result:
                ...
                _decision = {
                    ...
                    'task': (
                        ...
                        f"send_message_to_user('Контакт [имя] хочет созвон [когда]. Подтвердить? Zoom-ссылка?'). "
                        f"После подтверждения → reply + add_task.\n"
                        f"💡 Вместо плейсхолдеров вроде [вставьте ссылку] — пиши 'ссылку пришлю отдельно'.\n\n"
                        f"Входящие письма:\n{result[:2000]}"
                    ),
                }
    ...
    async def _run_coordinator_dispatch(
        ...
        try:
            ...
            if _stagnant_goals and len(_recent) >= 4:
                ...
                if _already_sent and _email_sent > 0:
                    _stagnant_instr = (
                        ...
                        "Варианты для этого цикла:\n"
                        "  1. Проверить входящие — возможно пришли ответы.\n"
                        "  2. Отправить письма новым контактам.\n"
                        + _gh_step +
                        "  4. Поискать авторов через RSS → сохранить контакты.\n"
                        "  5. Попробовать принципиально другой подход, если прежний не работает.\n"
                    )
                else:
                    _stagnant_instr = (
            ...
            _github_dedup_note = (
                f'\n  📋 GitHub запросы уже использованные (придумай новый): {"; ".join(_used_github_queries[:5])}'
                f'\n  → Попробуй другие параметры: language, followers, repos, location, topic.\n'
            ...
            if _strategy_usage or _all_cycles_summaries:
                ...
                if _heavily_overused:
                    _strategy_lines.append(f"     ℹ️ Часто используемые: {', '.join(_heavily_overused)} — 4+ раз. Рассмотри альтернативы.")
            ...
            if _overworked_goals:
                ...
                _goal_rotation_str = (
                    ...
                    f"  💡 Рекомендуется включить хотя бы 1 заброшенную цель в этот цикл."
            ...
            try:
                ...
                if not _eff_lines and len(_recent) >= 3:
                    ...
                    _eff_lines.append("📊 АНАЛИЗ ЭФФЕКТИВНОСТИ:")
                    _eff_lines.append("  ⚠️ Последние циклы: активность есть, но метрики целей не растут.")
                    _eff_lines.append("     Рассмотри смену стратегии:")
            ...
            _plan_prompt = (
                ...
                "• Наибольшая ценность когда агент дошёл до действующего инструмента:\n"
                ...
                "• Формулируй задачу так, чтобы она вела к конкретному действию\n\n"
                ...
                "• После research/поиска превращай данные в гипотезу по аудитории: кто эти люди, что у них болит, что им важно сейчас.\n"
                "• Затем сформируй персональное предложение под этот сегмент, а не общий шаблон.\n"
                "• Добавь в task 1-2 конкретных примера внедрения в проект пользователя через доступные интеграции/каналы.\n"
                "• Если данных сегмента недостаточно — назначай шаг на уточнение сегмента, а не абстрактную рассылку.\n\n"
                ...
                "• Для каждой интеграции выбирай РАЗНЫЕ режимы применения.\n"
                "• Думай как живой директор: 'Что принесёт максимум результата?'\n"
                "• Мир огромный — есть dev.to, HackerNews, ProductHunt, Reddit, конференции, сообщества.\n"
                "  Каждый цикл — возможность попробовать что-то новое.\n"
                "• Цени разнообразие: новые люди, новые площадки, новые гипотезы.\n"
                "• Результат = что нового узнал и как это двигает цель.\n\n"
