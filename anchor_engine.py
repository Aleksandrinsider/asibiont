            ...
            if _results_summary:
                try:
                    ...
                    _report_prompt = (
                        ...
                        + f"Состояние целей:\n{_goals_state_now}\n\n"
                        f"Правила:\n"
                        f"- Адресат — ПОЛЬЗОВАТЕЛЬ, не агенты. НЕ обращайся к агентам по имени (не 'Кристина, проверь', не 'Марк, сделай').\n"
                        f"- Пиши ЧТО БЫЛО СДЕЛАНО этот цикл (прошедшее время). НЕ пиши что надо сделать дальше.\n"
                        f"- Называй КОНКРЕТНО: что нашёл агент, кому написали, сколько контактов.\n"
                        f"- Если агент видел нерелевантные данные — честно скажи 'подходящих данных не нашлось'.\n"
                        f"- Если ничего не продвинулось — 'Новых подтверждённых результатов нет'.\n"
                        f"- Синтез: если разные агенты нашли разное — объедини в картину для пользователя.\n"
                        + _note_hint
                        + f"- 3-5 предложений. Без markdown. Без [АВТОПИЛОТ]. Без квадратных скобок."
                    )
