...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None, team_history=None, python_code=None) -> str:
    ...
    _MULTI_USE_OK = {
        ...
        # check_emails удалён из MULTI_USE — допускаем max 2 раза, дальше банится
    }
    ...
    # check_emails: если вызывался 3+ раз — добавляем подсказку о бесполезности повторов
    _check_emails_overuse = ''
    _ce_count = _tool_cnt.get('check_emails', 0)
    if _ce_count >= 3:
        _check_emails_overuse = (
            f"\n📧 check_emails вызывался {_ce_count} раз за последние циклы. "
            "Повторная проверка без нового письма от тебя — пустая трата времени. "
            "ПЕРЕКЛЮЧИСЬ: найди НОВЫЕ контакты, напиши НОВЫЕ письма, создай контент. "
            "Проверяй почту ТОЛЬКО после отправки нового письма.\n"
        )
    ...
    return (
        ...
        f"{_memory_block}\n"
        "АВТОПИЛОТ — ПРИНЦИПЫ:\n"
        "1. Первый ответ = вызов инструмента (НЕ текст). После инструмента — следующий инструмент (НЕ описание что ты нашёл).\n"
        "   СТОП-ФРАЗА: если хочешь написать «нашёл X» или «думаю сделать Y» — НЕ ПИШИ. Просто сделай Y.\n"
        ...
        "19. ⛔ ЗАПРЕЩЁННЫЙ ПАТТЕРН — «НАШЁЛ И РАССКАЖУ»:\n"
        "  НЕЛЬЗЯ: найти контакт → написать текст «нашёл X, план: сохраню и напишу» → save_note/add_note.\n"
        "  НУЖНО:  найти контакт → НЕМЕДЛЕННО save_email_contact → НЕМЕДЛЕННО send_outreach_email.\n"
        "  Правило: нашёл человека = следующий вызов инструмента. Не текст, не план, не заметка.\n"
        "  add_note/save_note = фиксация ЗАВЕРШЁННОГО результата. НЕ план, НЕ черновик, НЕ «намерение».\n"
        "  Пример ПЛОХО: «Нашёл профессора Иванова... думаю сохранить в базу и написать...» → В заметки.\n"
        "  Пример ХОРОШО: save_email_contact(name='Иванов', ...) → send_outreach_email(...) → краткий отчёт.\n"
...
class AnchorEngine:
                ...
                if anchor.anchor_type == 'goal_autopilot_review' and _chosen_id != 0:
                    ...
                    try:
                        ...
                        _coord_prompt = (
                            ...
                            f"\n  5) Каждый человек в базе контактов максимум 2 касания. После этого — ищи НОВЫХ."
                            f"\n  6) Если агент 2+ раза делал одно и то же (check_emails без результата, поиск одних людей) — дай ПРИНЦИПИАЛЬНО другую задачу.\n"
                            ...
                            "7. АУДИТОРИЯ-FIRST: если есть данные исследования, сформулируй задачу так: "
                            "(кто аудитория) + (какая их боль) + (персональное предложение) + "
                            "(1-2 конкретных примера как это внедрить в проект пользователя через доступные интеграции).\n"
    ...
    async def _run_coordinator_dispatch(
        ...
        try:
            ...
            # check_emails удалён — допускаем max 2 раза, дальше банится на уровне координатора
            _COORD_MULTI_USE_OK = {
                'run_agent_action', 'save_email_contact', 'send_outreach_email',
                'reply_to_outreach_email', 'find_relevant_contacts_for_task',
            ...
            if _stagnant_goals and len(_recent) >= 4:
                ...
                if _already_sent and _email_sent > 0:
                    _stagnant_instr = (
                        ...
                        "  1. Email-агент: check_emails — проверить входящие ответы (ТОЛЬКО 1 раз за цикл!).\n"
                        "  2. Если ответов < gap — email-агент: send_outreach_email НОВЫМ контактам (не из уже_написали).\n"
                        "     ⚡ ВАЖНО: ищи контакты на РАЗНЫХ площадках каждый цикл. Не повторяй одни и те же запросы.\n"
                        + _gh_step +
                        ...
                        "  6. Каждому контакту — ПЕРСОНАЛЬНОЕ письмо с упоминанием его работы. Шаблоны не работают.\n"
                    )
                else:
                    _stagnant_instr = (
            ...
            _plan_prompt = (
                ...
                + (
                    ...
                    "если check_emails НЕ выполнялся в НЕДАВНО ВЫПОЛНЕННЫХ задачах, назначь его ОДИН раз. "
                    "Если уже выполнен — НЕ повторяй, дай другую задачу. "
                    "check_emails больше 1 раза за цикл = пустая трата. Лучше: новые контакты, контент, follow-up.\n"
                ...
                "ИНТЕЛЛЕКТУАЛЬНАЯ АДАПТАЦИЯ (универсально, без привязки к конкретному продукту):\n"
                "• После research/поиска ОБЯЗАТЕЛЬНО преврати данные в гипотезу по аудитории: кто эти люди, что у них болит, что им важно сейчас.\n"
                ...
                "РАЗНООБРАЗИЕ КОНТАКТОВ И АУДИТОРИЙ (критически важно!):\n"
                "• ⛔ НЕ зацикливайся на одном человеке. Если контакт упоминался 3+ раз в последних циклах — ПЕРЕКЛЮЧИСЬ на нового.\n"
                "• Каждый цикл — НОВЫЙ сегмент аудитории, НОВАЯ ниша, НОВАЯ платформа для поиска.\n"
                "• Вместо повторного поиска тех же людей — ищи на ДРУГИХ площадках: dev.to, HackerNews, ProductHunt, Reddit, LinkedIn, конференции.\n"
                "• Вместо 10 писем одному — 1 письмо 10 разным людям из разных сфер.\n"
                "• check_emails: вызывай МАКСИМУМ 1 раз за цикл. Если уже проверено — НЕ проверяй снова, займись другим.\n"
                "• find_relevant_contacts_for_task: каждый раз используй ДРУГИЕ ключевые слова. Не повторяй запрос.\n\n"

                "МЫШЛЕНИЕ, НЕ ШАБЛОН:\n"
                "• Перед каждым шагом спроси себя: «Что я узнал за последние 3 цикла? Что изменилось?»\n"
                "• Если подход не дал результата 2 раза — он НЕ РАБОТАЕТ. Не пробуй в третий раз.\n"
                "• Ищи нестандартные пути: коллаборации, гостевые посты, AMA-сессии, комментирование чужих постов.\n"
                "• Каждая задача должна содержать ГИПОТЕЗУ: «Проверяем, что [сегмент X] отреагирует на [предложение Y]».\n"
                "• Результат цикла = не «сделал шаги», а «что нового узнал и как это меняет стратегию».\n\n"
            ...
            while _executed < _MAX_DYNAMIC_STEPS:
                ...
                _agent_prompt = (
                    ...
                    f"\n  • Это то что ты РЕАЛЬНО УМЕЕШЬ. НЕ упоминай и НЕ предлагай сервисы"
                    f" (Discord, Slack, GitHub, Telegram и т.д.) если их НЕТ в этом списке."
                    f" Не говори 'переключаюсь на Discord/форумы' без Discord в твоих интеграциях."
                    ...
                    f"\n  [ШИРОКИЙ ФОКУС] Не зацикливайся на одном контакте или методе."
                    f" Если ты 2 раза обращалась к одному человеку — СТОП, ищи нового."
                    f" Каждый цикл = новый эксперимент, новая гипотеза, новая аудитория."
                    f" Мир огромный — не сиди в одной нише."
                    f"\n  [ЯЗЫК] Пиши живо и конкретно. ЗАПРЕЩЁННЫЕ ФРАЗЫ (НИКОГДА не используй):"
                    f" «думаю, стоит», «давай», «планирую», «собираюсь», «было бы хорошо»,"
                    f" «предлагаю рассмотреть», «проверила входящие — нет писем»."
                    f" ВМЕСТО «думаю, стоит написать» → сразу пиши (send_outreach_email) и покажи результат."
                    f" ВМЕСТО «давай проверим» → проверь и скажи ЧТО нашла."
                    f" Если проверила и пусто — не сообщай об этом, переключись на другое действие молча."
