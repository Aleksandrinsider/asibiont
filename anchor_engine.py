...
def _build_autopilot_prompt(goals_summary: list, user=None, agent_caps=None, agent_name=None, team_profiles=None, agent_history=None) -> str:
    ...
    if _has_rss and not _has_github:
        _rss_rules = (
            "\n📰 ТВОЯ РОЛЬ (RSS-агент) — АНАЛИТИК И КОНТЕНТ:\n"
            "  • run_agent_action(get_latest) — получи новости из RSS-ленты\n"
            "  • web_search(query) — поиск в интернете (DDG/Bing/Google)\n"
            ...
            "\n🧠 ГИБКОЕ МЫШЛЕНИЕ ПО ИНТЕГРАЦИЯМ:\n"
            "Для каждой конкретной цели оценивай, какой инструмент/интеграция подойдёт ЛУЧШЕ ВСЕГО.\n"
            "  • Если цель требует поиска людей/контактов — web_search может найти их НА ЛЮБОЙ ПЛОЩАДКЕ\n"
            "    (GitHub, LinkedIn, Twitter, форумы, сайты компаний и т.д.).\n"
            "  • Если есть агент с GitHub — делегируй через DELEGATE[имя] поиск по GitHub.\n"
            "  • Если нужна интеграция которой НЕТ (CRM, LinkedIn API и т.д.) — ЯВНО СКАЖИ пользователю:\n"
            '    "Для этой цели лучше всего подойдёт [интеграция]. Хочешь подключить её?"\n'
            "  • Не ограничивай себя только своими текущими инструментами — думай стратегически!\n"
        )
            ...
            for _p_cr in _profiles:
                ...
                if _has_email_cr and _is_github_agent:
                    ...
                    _cap_rules_lines.append(
                        ...
                        f"ГИБКО: для каждой цели определяй оптимальный способ — "
                        f"web_search может искать людей/контакты на ЛЮБОЙ платформе (GitHub, LinkedIn, форумы). "
                        f"Если нужна специфическая интеграция → DELEGATE к коллеге или предложи пользователю подключить её. "
                        f"НЕ ограничивай себя ролью только контент-агента — думай стратегически!"
                    )
            _cap_rules_str = (
