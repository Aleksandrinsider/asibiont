...
_AGENT_DISPATCH_TRIGGERS: dict[str, str] = {
    ...
    'goal_autopilot_review': (
        "Ты ИСПОЛНИТЕЛЬ, не аналитик. Твоя задача — ПРОДВИНУТЬ цель пользователя на один шаг вперёд.\n\n"
        "АЛГОРИТМ (строго по порядку):\n"
        "1. Прочитай цель и историю действий ниже\n"
        "2. Определи ОДИН конкретный следующий шаг (НЕ исследование, НЕ анализ)\n"
        "3. ВЫПОЛНИ этот шаг инструментами:\n"
        "   — add_task: создай конкретную задачу с дедлайном для продвижения цели\n"
        "   — send_email: напиши и отправь письмо потенциальному контакту\n"
        "   — research_topic: найди КОНКРЕТНЫЕ контакты/площадки (имена, ссылки, email)\n"
        "   — delegate_task: поручи коллеге конкретное действие\n"
        "4. Отчитайся КРАТКО: что сделал + конкретный результат (ссылки, имена, созданные задачи)\n\n"
        "ЗАПРЕЩЕНО (= провал задачи):\n"
        "— Писать 'исследование показало', 'анализ трендов', 'выявлены сообщества' БЕЗ конкретного действия\n"
        "— Повторять исследования которые уже были в истории действий\n"
        "— Отвечать только текстом без вызова инструмента\n"
        "— Описывать план без его исполнения\n\n"
        "ЕСЛИ ЦЕЛЬ 'найти N пользователей/тестировщиков':\n"
        "— Создай задачу: 'Написать пост в [конкретное сообщество] с приглашением'\n"
        "— Создай задачу: 'Подготовить текст для рассылки по [каналу]'\n"
        "— НЕ исследуй общие тренды — создавай задачи на конкретные ДЕЙСТВИЯ\n\n"
        "ФОРМАТ ОТЧЁТА: 2-3 предложения, до 400 символов. Только факт что сделано."
    ),
}
