                ...
                try:
                    if _prev_steps_context and len(_prev_steps_context.strip()) > 30:
                        ...
                        _assign_prompt = (
                            ...
                            f"Правила написания:\n"
                            f"- Начни с имени: «{_ag_name},» — затем ПОВЕЛИТЕЛЬНОЕ наклонение глагола (проверь, найди, отправь, посмотри)\n"
                            f"- 2-3 живых предложения, как руководитель говорит сотруднику напрямую\n"
                            f"- ОБЯЗАТЕЛЬНО используй конкретику из результатов предыдущих агентов если она связана с заданием\n"
                            f"- Объясни ЗАЧЕМ — исходя из того что уже узнала команда\n"
                            f"- НЕ используй инфинитив («проверить», «найти») — только прямой приказ («проверь», «найди»)\n"
                            f"- Без markdown. Без скобок."
                        )
                        _assign_max_tok = 130
                    else:
                        # Первый шаг: простое поручение
                        _assign_prompt = (
                            f"Ты — ASI, координатор. Напиши живое прямое поручение агенту {_ag_name}.\n"
                            f"Задача: {_ag_task[:200]}\n"
                            f"Правила: начни с «{_ag_name},» затем ПОВЕЛИТЕЛЬНОЕ наклонение (проверь / найди / отправь / посмотри)."
                            f" 1-2 фразы. НЕ инфинитив. Без markdown."
                        )
                        _assign_max_tok = 80
