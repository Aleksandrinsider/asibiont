...
if __name__ == "__main__":
    asyncio.run(test_all_message_types())
            ...
            # Избегать шаблонных фраз
            if any(phrase in response.lower() for phrase in ["отлично", "замечательно", "конечно", "хорошо", "понятно", "ясно"]):
                issues.append("Содержит шаблонные фразы")

            # Должен быть conversational тон
            has_personal_touch = any(word in response.lower() for word in [
                "интересно", "кстати", "знаешь", "представь", "вообще", "смотри", "слушай",
                "давай", "можешь", "стоит", "думаю", "чувствую", "вижу", "понимаю"
            ])
            if not has_personal_touch:
            ...
            # Проверяем наличие полезного контекста и предложений
            has_suggestions = any(word in response.lower() for word in [
                "можно", "стоит", "теперь", "далее", "следующ", "перепланир", "итог",
                "альтернатив", "вариант", "идея", "предложение", "совет", "рекомендую"
            ])
            if not has_suggestions:
                issues.append("Отсутствуют предложения или идеи для следующих шагов")
