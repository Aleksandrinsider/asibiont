...
async def test_all_functions():
    ...
    if not test_user:
        print("ERROR: Тестовый пользователь не найден. Создайте пользователя с username='test_user'")
        return
    ...
    print("-"*70)
    try:
        result = get_partners_list(user_id=user_id, session=session)
        print(f"Найдено партнеров: {len(result) if isinstance(result, list) else 0}")
    except Exception as e:
        print(f"ERROR: {e}")
    ...
    try:
        ...
        print(f"Создали задачу: {add_result}")
        
        # Теперь удаляем по ключевому слову
