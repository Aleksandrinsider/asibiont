...
def _migrate_users(session, inspector):
    ...
    _add_columns(session, 'users', cols, {
        ...
        'vk_token': "ALTER TABLE users ADD COLUMN vk_token VARCHAR(500)",
        'vk_owner_id': "ALTER TABLE users ADD COLUMN vk_owner_id VARCHAR(64)",
        'vk_group_name': "ALTER TABLE users ADD COLUMN vk_group_name VARCHAR(255)",
...
def _cleanup_backfill_blog_duplicates(session, inspector):
    """Удаляет дубли blog Notes, созданные backfill-ом (slug ap-*),
    если есть исходная blog Note с тем же user_id + content."""
    if not inspector.has_table('notes'):
        return
    try:
        res = session.execute(text(
            ...
            "AND COALESCE(n.content, '') = COALESCE(o.content, '') "
            "AND n.slug LIKE 'ap-%' "
            "AND (o.slug IS NULL OR o.slug NOT LIKE 'ap-%') "
            "AND n.created_at >= o.created_at"
        ))
        deleted = int(getattr(res, 'rowcount', 0) or 0)
        if deleted:
            session.commit()
            logger.info(f"[MIGRATION] Removed {deleted} duplicate blog Notes from backfill")
    except Exception as _e:
        logger.warning(f"[MIGRATION] Blog duplicate cleanup failed: {_e}")
        session.rollback()
