...
- МИНИМУМ 2-3 ПРЕДЛОЖЕНИЯ в каждом ответе
...
- ЗАДАВАЙ УТОЧНЯЮЩИЕ ВОПРОСЫ: Если запрос неясен, спрашивай детали. Лучше переспросить, чем угадывать
...
- Если говорили о задаче → спроси про профиль (интересы, город, цели) или другие задачи
- Если обновили профиль → предложи найти партнеров с похожими интересами или спроси про планы
- Если просто ответ → задай вопрос про ДРУГУЮ тему (задачи, контакты, подписка, цели)
- Всегда ищи возможность расширить диалог: "Кстати, заметил что у тебя много задач - хочешь найти людей для совместной работы?", "А что насчет профиля - добавим твои интересы?"
- ПРОЯВЛЯЙ ИНИЦИАТИВУ: замечай неполный профиль, отсутствие контактов, возможности для делегирования, нераскрытые интересы
...
async def generate_reminder(user_id, task_title):
    ...
    try:
        ...
        system_prompt += "⚠️ МАКСИМУМ 1-3 АБЗАЦА! НЕ ДАВАЙ ДЛИННЫЕ ОТВЕТЫ!"
