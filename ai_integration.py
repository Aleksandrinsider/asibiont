...
Если пользователь дает уточнения к только что созданной задаче:
- НЕ создавай новую задачу через add_task()
- ОБНОВИ существующую через edit_task() с новым заголовком и описанием
- Пример: "Проверить почту" → пользователь уточнил "жду ответа от партнеров" → вызови edit_task(task_id=last_task_id, title="Проверить почту на ответ от партнеров")
- Сохрани новую информацию в описание задачи
