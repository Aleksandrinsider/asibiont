...
async def generate_result_check(user_id, task_title):
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": system_prompt + user_memory + "\n\nПРАВИЛА ДЛЯ ОТВЕТА: Минимум 3-4 предложения с анализом результата. Задай уточняющие вопросы о том, что пошло хорошо/плохо, какие улучшения можно внести. Предложи следующий шаг или новую задачу. Будь вовлекающим и полезным."},
            {
                "role": "user",
...
async def generate_proactive_message(user_id):
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": system_prompt + user_memory + plans_info + tasks_info + "\n\nПРАВИЛА ДЛЯ ПРОАКТИВНОГО СООБЩЕНИЯ: Минимум 4-6 предложений. Проанализируй текущую ситуацию пользователя. Предложи конкретные идеи для новых задач или планов. Упомяни планы других людей, если есть. Задай вопросы для вовлечения. Будь позитивным и мотивирующим."},
            {
                "role": "user",
...
async def generate_daily_report(user_id):
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": system_prompt + user_memory + "\n\nПРАВИЛА ДЛЯ ЕЖЕДНЕВНОГО ОТЧЕТА: Минимум 4-6 предложений. Проанализируй выполненные и невыполненные задачи. Дай рекомендации по улучшению продуктивности. Отметь достижения и предложи план на следующий день. Задай вопросы о планах."},
...
async def generate_overdue_reminder(user_id, overdue_tasks, escalation_level=1):
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": system_prompt + user_memory + "\n\nПРАВИЛА ДЛЯ НАПОМИНАНИЯ О ПРОСРОЧЕННЫХ ЗАДАЧАХ: Минимум 4-6 предложений. Проанализируй каждую просроченную задачу. Объясни последствия просрочки. Предложи конкретные шаги для выполнения или переноса. {tone_instruction} Задай вопросы для мотивации."},
            {
                "role": "user",
