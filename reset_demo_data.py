"""Очищает демо-данные и создаёт их заново"""
import os
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from models import User, UserProfile, Task
from datetime import datetime, timedelta

DATABASE_URL = os.getenv('DATABASE_URL', 'sqlite:///bot.db')
if DATABASE_URL.startswith('postgres://'):
    DATABASE_URL = DATABASE_URL.replace('postgres://', 'postgresql://', 1)

engine = create_engine(DATABASE_URL)
SessionLocal = sessionmaker(bind=engine)
session = SessionLocal()

try:
    # Находим основного пользователя
    main_user = session.query(User).filter_by(telegram_id=146333757).first()
    if not main_user:
        print("❌ Основной пользователь не найден")
        exit(1)
    
    print(f"✅ Основной пользователь: {main_user.username}")
    
    # Удаляем все демо-задачи (задачи делегирования)
    deleted_tasks = session.query(Task).filter(
        (Task.delegated_to_username.isnot(None)) | 
        (Task.delegated_to_username == main_user.username)
    ).delete(synchronize_session=False)
    print(f"🗑️  Удалено задач делегирования: {deleted_tasks}")
    
    # Удаляем демо-пользователей (telegram_id 200001-200007)
    demo_user_ids = list(range(200001, 200008))
    for tid in demo_user_ids:
        demo_user = session.query(User).filter_by(telegram_id=tid).first()
        if demo_user:
            # Удаляем профиль
            session.query(UserProfile).filter_by(user_id=demo_user.id).delete()
            # Удаляем задачи
            session.query(Task).filter_by(user_id=demo_user.id).delete()
            # Удаляем пользователя
            session.delete(demo_user)
    
    print("🗑️  Удалены демо-пользователи")
    
    session.commit()
    print("\n✅ Очистка завершена\n")
    
    # Обновляем профиль основного пользователя
    main_profile = session.query(UserProfile).filter_by(user_id=main_user.id).first()
    if not main_profile:
        main_profile = UserProfile(
            user_id=main_user.id,
            city='Moscow',
            position='Product Manager',
            skills='Project Management, Python, SQL',
            interests='Python, менеджмент, проектирование'
        )
        session.add(main_profile)
    else:
        main_profile.city = 'Moscow'
        main_profile.company = None
        main_profile.position = 'Product Manager'
        main_profile.goals = None
        main_profile.skills = 'Project Management, Python, SQL'
        main_profile.interests = 'Python, менеджмент, проектирование'
    
    session.commit()
    print(f"✅ Обновлён профиль основного пользователя")
    print(f"   Город: {main_profile.city}")
    print(f"   Компания: Не указана")
    print(f"   Должность: {main_profile.position}")
    print(f"   Цели: Не указаны")
    print(f"   Навыки: {main_profile.skills}")
    print(f"   Интересы: {main_profile.interests}\n")
    
    # 1. Создаём рекомендуемых контактов (со схожими интересами)
    recommended_users = [
        {
            'telegram_id': 200001,
            'username': 'anna_marketing',
            'first_name': 'Анна',
            'position': 'Маркетолог',
            'city': 'Москва',
            'company': 'Digital Agency',
            'interests': 'SMM, контент-маркетинг, менеджмент'
        },
        {
            'telegram_id': 200002,
            'username': 'sergey_analyst',
            'first_name': 'Сергей',
            'position': 'Аналитик данных',
            'city': 'Санкт-Петербург',
            'company': 'Tech Corp',
            'interests': 'Python, SQL, визуализация'
        },
        {
            'telegram_id': 200003,
            'username': 'elena_designer',
            'first_name': 'Елена',
            'position': 'UI/UX дизайнер',
            'city': 'Казань',
            'company': 'Creative Studio',
            'interests': 'Figma, проектирование, UX'
        }
    ]
    
    for user_data in recommended_users:
        user = User(
            telegram_id=user_data['telegram_id'],
            username=user_data['username'],
            first_name=user_data['first_name']
        )
        session.add(user)
        session.flush()
        
        profile = UserProfile(
            user_id=user.id,
            position=user_data['position'],
            city=user_data['city'],
            company=user_data.get('company'),
            interests=user_data['interests']
        )
        session.add(profile)
    
    print("✅ Создано 3 рекомендуемых контакта")
    
    # 2. Создаём пользователей, которые делегируют МНЕ
    delegating_to_me_users = [
        {
            'telegram_id': 200004,
            'username': 'boss_ivan',
            'first_name': 'Иван',
            'position': 'CEO',
            'city': 'Moscow',
            'company': 'StartupHub',
            'interests': 'Стратегия, рост бизнеса',
            'tasks': ['Подготовить презентацию для инвесторов', 'Проанализировать конкурентов']
        },
        {
            'telegram_id': 200005,
            'username': 'manager_olga',
            'first_name': 'Ольга',
            'position': 'Проект-менеджер',
            'city': 'Moscow',
            'company': 'IT Solutions',
            'interests': 'Agile, Scrum',
            'tasks': ['Обновить дорожную карту проекта', 'Провести ретроспективу спринта', 'Составить отчет по метрикам']
        }
    ]
    
    for user_data in delegating_to_me_users:
        user = User(
            telegram_id=user_data['telegram_id'],
            username=user_data['username'],
            first_name=user_data['first_name']
        )
        session.add(user)
        session.flush()
        
        profile = UserProfile(
            user_id=user.id,
            position=user_data['position'],
            city=user_data.get('city'),
            company=user_data.get('company'),
            interests=user_data['interests']
        )
        session.add(profile)
        
        # Создаём задачи, делегированные мне (БЕЗ маркера в title)
        for i, task_title in enumerate(user_data['tasks']):
            task = Task(
                user_id=user.id,
                title=task_title,  # Чистый title без маркеров
                description=f"Задача от {user.first_name}",
                status='pending',
                priority='high' if i == 0 else 'medium',
                created_at=datetime.utcnow() - timedelta(days=i),
                delegated_to_username=main_user.username,
                delegation_status='accepted'
            )
            session.add(task)
    
    print("✅ Создано 2 пользователя, делегирующих мне (5 задач)")
    
    # 3. Создаём пользователей, КОТОРЫМ Я делегирую
    delegating_by_me_users = [
        {
            'telegram_id': 200006,
            'username': 'junior_dmitry',
            'first_name': 'Дмитрий',
            'position': 'Junior Developer',
            'city': 'Moscow',
            'company': 'TechStart',
            'interests': 'JavaScript, React',
            'tasks': ['Исправить баг в форме регистрации', 'Написать unit-тесты']
        },
        {
            'telegram_id': 200007,
            'username': 'intern_maria',
            'first_name': 'Мария',
            'position': 'Стажёр',
            'city': 'Moscow',
            'company': 'Creative Lab',
            'interests': 'Обучение, дизайн',
            'tasks': ['Сделать макет новой страницы']
        }
    ]
    
    for user_data in delegating_by_me_users:
        user = User(
            telegram_id=user_data['telegram_id'],
            username=user_data['username'],
            first_name=user_data['first_name']
        )
        session.add(user)
        session.flush()
        
        profile = UserProfile(
            user_id=user.id,
            position=user_data['position'],
            city=user_data.get('city'),
            company=user_data.get('company'),
            interests=user_data['interests']
        )
        session.add(profile)
        
        # Создаём задачи, которые Я делегирую им (БЕЗ маркера в title)
        for i, task_title in enumerate(user_data['tasks']):
            task = Task(
                user_id=main_user.id,
                title=task_title,  # Чистый title без маркеров
                description=f"Задача для {user.first_name}",
                status='pending',
                priority='medium',
                created_at=datetime.utcnow() - timedelta(days=i),
                delegated_to_username=user.username,
                delegation_status='pending'
            )
            session.add(task)
    
    print("✅ Создано 2 пользователя, которым я делегирую (3 задачи)")
    
    session.commit()
    
    print("\n📊 Итоговая статистика:")
    print(f"- Рекомендуемые контакты: 3 (со схожими интересами)")
    print(f"- Делегируют мне: 2 пользователя (5 задач)")
    print(f"- Делегирую я: 2 пользователя (3 задачи)")
    print(f"\n✅ Данные успешно обновлены")
    
except Exception as e:
    print(f"❌ Ошибка: {e}")
    import traceback
    traceback.print_exc()
    session.rollback()
finally:
    session.close()
