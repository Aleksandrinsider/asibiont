...
async def create_auto_post(user_id, content, session, notify=True, post_type='progress'):
    ...
    try:
        ...
        try:
            if user.vk_token and hasattr(user, 'vk_owner_id') and user.vk_owner_id:
                import urllib.parse as _vk_url_parse
                _vk_owner_id = user.vk_owner_id
                _vk_name = getattr(user, 'vk_group_name', 'VK') or 'VK'
                try:
                    _vk_url = (
                        f"https://api.vk.com/method/wall.post?"
                        f"owner_id={_vk_owner_id}&message={_vk_url_parse.quote(content)}"
                    ...
                    if 'error' not in _vk_data:
                        ...
                        logger.info(f"[AUTO_POST VK] Published to {_vk_name} for user {user_id}, post_id={_vk_post_id}")
                        try:
                            from models import AgentActivityLog
                            _vk_log = AgentActivityLog(
                                ...
                                title=content,
                                content=content,
                                target=f'VK {_vk_name} ({_vk_owner_id})',
                                status='published',
                                ref_id=post.id,
                            )
                            session.add(_vk_log)
                        ...
                        logger.warning(f"[AUTO_POST VK] API error for {_vk_name}: {_vk_err}")
                except Exception as _tgt_err:
                    logger.warning(f"[AUTO_POST VK] Failed to publish to {_vk_name} for user {user_id}: {_tgt_err}")
