...
chk_rule("Различай вопрос vs действие",            r"РАЗЛИЧАЙ.*вопрос|вопрос.*Действие")
chk_rule("Один вопрос максимум",                   r"1\s+вопрос|один\s+вопрос|Нет 1 параметра")
chk_rule("Верификация данных",                     r"ВЕРИФИЦИРУЙ|не утверждай.*без.*данных")
chk_rule("Автопилот целей",                        r"автопилот.*целей|Автопилот целей")
chk_rule("Продолжение диалога",                    r"ПРОДОЛЖАЕТ разговор|перечитай последние")
chk_rule("Отчёт о выполнении",                     r"отчёт|результат.*задач|ОБЯЗАТЕЛЬНО сообщи")
chk_rule("Интеграции — предлагай",                 r"интеграци.*Предлагай|45\+\s+интеграци")
chk_rule("Без URL в постах/кампаниях",             r"Без URL|No URLs", re.IGNORECASE)
