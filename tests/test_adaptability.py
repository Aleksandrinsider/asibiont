...
chk_rule("ПРЯМАЯ КОМАНДА алгоритм присутствует",   r"АЛГОРИТМ\s+действий")
chk_rule("5 шагов алгоритма",                      r"платформа.*тема.*время", re.DOTALL)
chk_rule("ЗАПРЕЩЕНО 12:00 по умолчанию",           r"ЗАПРЕЩЕНО.*12:00|12:00.*ЗАПРЕЩЕНО")
chk_rule("ЗАПРЕЩЕНО буллеты в уточнении",          r"ЗАПРЕЩЕНО.*перечисл|ЗАПРЕЩЕНО.*пунктами")
chk_rule("РАЗЛИЧАЙ интерес и команду",             r"РАЗЛИЧАЙ\s+интерес\s+и\s+команду")
chk_rule("один вопрос максимум",                   r"более\s+одного\s+вопроса|больше\s+одного")
chk_rule("АНТИ-ГАЛЛЮЦИНАЦИЯ правило",              r"АНТИ-ГАЛЛЮЦИНАЦИЯ")
chk_rule("Правило проактивных якорей",             r"ПРОАКТИВН.*ЯК|goal_decomposition")
chk_rule("Принцип цепочки (Цепочка:)",             r"Цепочка:|Chaining:")
chk_rule("Принцип отчётности (ОТЧЁТНОСТЬ:)",       r"ОТЧЁТНОСТЬ:|REPORTING:")
chk_rule("ИНТЕГРАЦИИ В ОТВЕТАХ правило",           r"ИНТЕГРАЦИИ В ОТВЕТАХ|INTEGRATIONS IN RESPONSES")
chk_rule("Запрет URL в кампаниях",                 r"ВООБЩЕ НИКАКИХ URL|NO URLs AT ALL")
