...
def test_d8_autopilot_prompt_has_myshlenie_block():
    """_build_autopilot_prompt содержит блок МЫШЛЕНИЕ ПЕРЕД ДЕЙСТВИЕМ."""
    ...
    assert "МЫШЛЕНИЕ ПЕРЕД ДЕЙСТВИЕМ" in prompt, \
        f"Блок 'МЫШЛЕНИЕ ПЕРЕД ДЕЙСТВИЕМ' не найден в промпте: {prompt[:500]}"
