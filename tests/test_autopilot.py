...
def test_update_goal_progress_percentage():
    ...
    create_goal(title="Цель-прогресс-тест", metric_target=100, metric_unit="шагов", user_id=UID)
...
def test_update_goal_completion_auto():
    """update_goal_progress autosets status=completed при metric_current=target."""
    from ai_integration.handlers import create_goal, update_goal_progress
    create_goal(title="Цель-завершение", metric_target=10, metric_unit="шагов", user_id=UID)
    result = update_goal_progress(
        goal_title="Цель-завершение",
        metric_current=10,
        notes="Тест: цель полностью достигнута",
        user_id=UID,
    )
    assert result is not None
    with TestSession() as s:
