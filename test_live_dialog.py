...
class LiveDialogTester:
    ...
    def _setup(self):
        ...
        if old:
            from models import Interaction, Post, Subscription, ContactAlert, ActivityAlert, TokenTransaction, AnchorDeliveryLog
            for model in [AnchorDeliveryLog, TokenTransaction, ContactAlert, ActivityAlert, Task, Goal, UserProfile, Interaction, Post, Subscription]:
                try:
        ...
        self.db.commit()
        logger.info(f'[SETUP] user id={u.id} tg={TEST_TG_ID} tokens=50000')

    async def run(self):
        MAX_TURNS = 15
        GLOBAL_TIMEOUT = MAX_TURNS * 100  # ~100s per turn max
        print(f'\n{"="*70}')
    ...
    async def _dialog_loop(self, MAX_TURNS):
        for turn in range(1, MAX_TURNS + 1):
            ...
            try:
                ...
                err = f'TIMEOUT turn {turn}: >90s'
                logger.error(err)
                self.errors.append(err)
                self.timings.append(90)
                print(f'  TIMEOUT: {err}\n')
                continue
            except Exception as e:
