...
USER_SYSTEM_PROMPT_RU = """Ты автор проекта ASI Biont и занимаешься разработкой ии агентов.

Цель — Привлечь 1000 пользователей в ии-агент ASI Biont.
...
USER_SYSTEM_PROMPT_EN = """You are the author of the ASI Biont project and are engaged in developing AI agents.

Goal — Attract 1000 users to the AI agent ASI Biont.
        ...
        if old:
            ...
            for msg_filter in [UserMessage.sender_id == old.id, UserMessage.recipient_id == old.id]:
                try:
                    ...
                    pass
            # Удаляем рейтинги где пользователь — оценщик или оцениваемый
            for rating_filter in [UserRating.rater_user_id == old.id, UserRating.rated_user_id == old.id]:
                try:
