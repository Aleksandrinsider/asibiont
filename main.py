...
async def get_user_avatar_url(bot, user_id, force_refresh=False):
    ...
    try:
        ...
        try:
            ...
            # Negative cache: __no_avatar__ means TG confirmed no photo
            if user and user.photo_url == _NO_AVATAR:
                if force_refresh:
                    # Clear negative cache on force_refresh to retry TG API
                    user.photo_url = None
                    try:
                        db.commit()
                    except Exception:
                    ...
                    # Return None immediately — don't hit TG API again
                    return None
            ...
            # No avatar found — store negative cache to avoid hitting TG API again
            if user and not user.photo_url:
                user.photo_url = _NO_AVATAR
                try:
                    db.commit()
                    logger.debug(f"Stored __no_avatar__ for user {user_id}")
                except Exception:
                    pass
            return None
        finally:
...
async def csp_middleware(request, handler):
    response = await handler(request)
    response.headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://telegram.org https://fonts.googleapis.com https://mc.yandex.ru https://mc.yandex.com https://yastatic.net https://*.yastatic.net https://cdn.jsdelivr.net https://webvisor.com https://*.webvisor.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://api.deepseek.com https://mc.yandex.ru https://mc.yandex.com https://*.yandex.ru https://*.yandex.com https://webvisor.com https://*.webvisor.com wss://mc.yandex.ru wss://mc.yandex.com wss://*.yandex.ru wss://*.yandex.com wss://webvisor.com wss://*.webvisor.com; frame-src https://oauth.telegram.org https://mc.yandex.ru https://mc.yandex.com https://*.yandex.ru https://webvisor.com https://*.webvisor.com; child-src 'self' blob: https://mc.yandex.ru https://mc.yandex.com https://*.yandex.ru https://*.yandex.com https://webvisor.com https://*.webvisor.com; worker-src 'self' blob: https://mc.yandex.ru https://mc.yandex.com https://*.yandex.ru https://*.yandex.com https://webvisor.com https://*.webvisor.com; frame-ancestors 'self' https://webvisor.com https://*.webvisor.com;"
    ...
    if request.path.startswith('/static'):
        # CSS/JS/SVG/images: cache 1 hour, revalidate after (cache-busting via ?timestamp)
        response.headers['Cache-Control'] = 'public, max-age=3600, must-revalidate'
    elif request.path in ('/', '/faq', '/terms', '/subscription-tiers') or request.path.startswith('/en/'):
...
# ── Gzip compression middleware: reduces 1MB dashboard HTML to ~50-100KB ──
import gzip as _gzip_mod

@web.middleware
async def gzip_middleware(request, handler):
    response = await handler(request)
    # Only compress text responses > 1KB when client supports gzip
    ...
    if len(body) < 1024:
        return response
    compressed = _gzip_mod.compress(body, compresslevel=6)
    ...
    try:
        ...
        def _sanitize_avatar_in_content(content: str) -> str:
            ...
            try:
                ...
                if isinstance(_jp, dict) and isinstance(_jp.get('__agent'), dict):
                    _ag = _jp['__agent']
                    # Normalize name: bare "ASI" → "ASI Biont"
                    if _ag.get('name') == 'ASI':
                        _ag['name'] = 'ASI Biont'
                        _jp['__agent'] = _ag
                    _av = _ag.get('avatar_url', '')
                    _aid = _ag.get('id')
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                if 'gender' in data:
                    _gv = (data['gender'] or '').strip().lower()
                    profile.gender = _gv if _gv in ('male', 'female') else None
...
async def api_marketplace_publish_agent_handler(request):
    ...
    try:
        ...
        try:
            ...
            agent.gender = (data.get('gender') or 'male').strip()[:10]
            ...
            if ext_agent_ids:
                ...
                for ea in ext_objs:
                    ext_agents.append({
                        ...
                        'personality': '',
                        'avatar_url': f'/api/arena/agent_avatar/mkt_{ea.id}?v={int(ea.updated_at.timestamp())}' if ea.avatar_url and ea.updated_at else (f'/api/arena/agent_avatar/mkt_{ea.id}' if ea.avatar_url else ''),
    ...
    try:
        ...
        try:
            ...
            return web.json_response({'agent': {
                ...
                'avatar_url': f'/api/arena/agent_avatar/mkt_{agent.id}?v={int(agent.updated_at.timestamp())}' if agent.avatar_url and agent.updated_at else (f'/api/arena/agent_avatar/mkt_{agent.id}' if agent.avatar_url else ''),
