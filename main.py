...
async def api_gamification_handler(request):
    """API для получения данных геймификации: стрик, хитмап активности, ачивки"""
    try:
        session = await get_session(request)
        user_id = session.get('user_id') if session else None
        if not user_id:
        ...
        try:
            ...
            # === HEATMAP: 84 дня (12 недель) активности ===
            heatmap = []
            daily_completed = {}
            for days_ago in range(83, -1, -1):
                day = today - timedelta(days=days_ago)
                day_start = datetime(day.year, day.month, day.day, tzinfo=user_tz).astimezone(pytz.UTC)
                day_end = day_start + timedelta(days=1)
                
                count = session_db.query(Task).filter(
                    Task.user_id == user.id,
                ...
                daily_completed[day.isoformat()] = count
                heatmap.append({
                    'date': day.isoformat(),
                    'count': count,
            ...
            # === STREAK: подсчёт подряд идущих дней ===
            streak = 0
            check_day = today
            # Если сегодня ещё нет завершённых задач, начинаем со вчера
            if daily_completed.get(today.isoformat(), 0) == 0:
                check_day = today - timedelta(days=1)
            
            while True:
                day_key = check_day.isoformat()
                if daily_completed.get(day_key, 0) > 0:
                    streak += 1
            ...
            # Базовые метрики
            total_tasks = session_db.query(Task).filter(Task.user_id == user.id).count()
            completed_tasks = session_db.query(Task).filter(
                Task.user_id == user.id, Task.status == 'completed'
            ).count()
            total_interactions = session_db.query(Interaction).filter(
                Interaction.user_id == user.id, Interaction.message_type == 'user'
            ).count()
            total_posts = session_db.query(Post).filter(Post.user_id == user.id).count()
            delegated_by_me = session_db.query(Task).filter(
                Task.delegated_by == user.id,
                Task.delegated_to_username.isnot(None)
            ).count()
            delegated_to_me = session_db.query(Task).filter(
                Task.delegated_to_username.isnot(None),
                Task.delegated_to_username.ilike(user.username) if user.username else False,
                Task.delegation_status == 'accepted'
            ).count()
            ...
            if user.created_at:
                ...
                days_since_registration = (now_utc - created).days
            else:
                days_since_registration = 0
            
            # Быстрое выполнение (менее 1 часа)
            from sqlalchemy import func as sa_func
            fast_completions = session_db.query(Task).filter(
                Task.user_id == user.id,
                Task.status == 'completed',
                Task.actual_completion_time.isnot(None),
                Task.created_at.isnot(None)
            ).all()
            lightning_count = sum(1 for t in fast_completions 
                if t.actual_completion_time and t.created_at and
                (t.actual_completion_time.replace(tzinfo=pytz.UTC) if t.actual_completion_time.tzinfo is None else t.actual_completion_time) -
                (t.created_at.replace(tzinfo=pytz.UTC) if t.created_at.tzinfo is None else t.created_at) < timedelta(hours=1))
            
            # Ранние и поздние выполнения
            early_count = 0
            night_count = 0
            for t in fast_completions:
                if t.actual_completion_time:
                    ct = t.actual_completion_time
                    if ct.tzinfo is None:
                        ct = ct.replace(tzinfo=pytz.UTC)
                    local_time = ct.astimezone(user_tz)
                    if local_time.hour < 7:
                        early_count += 1
                    if local_time.hour >= 23:
                        night_count += 1
            
            # Активные задачи одновременно
            active_now = session_db.query(Task).filter(
                Task.user_id == user.id,
            ...
            if profile:
                for field in ['city', 'company', 'position', 'goals', 'skills', 'interests', 'birthdate']:
                    if getattr(profile, field, None):
                        profile_fields_filled += 1
            
            # Рейтинг
            avg_rating = profile.average_rating if profile and profile.average_rating else 0
            
            # Цели
            total_goals = session_db.query(Goal).filter(Goal.user_id == user.id).count()
            completed_goals = session_db.query(Goal).filter(
                Goal.user_id == user.id, Goal.status == 'completed'
            ).count()
            
            # Максимальный стрик (вычисляем из heatmap)
            max_streak = 0
            current_run = 0
            for entry in heatmap:
                if entry['count'] > 0:
                    ...
                    current_run = 0
            max_streak = max(max_streak, streak)  # текущий стрик может быть длиннее
            
            # Определяем ачивки
            achievements = [
                {'id': 'first_step', 'icon': '🌱', 'title': 'Первый шаг', 'desc': 'Создать первую задачу', 'unlocked': total_tasks >= 1, 'progress': min(total_tasks, 1), 'target': 1},
                {'id': 'five_done', 'icon': '✅', 'title': 'Пятёрочка', 'desc': 'Выполнить 5 задач', 'unlocked': completed_tasks >= 5, 'progress': min(completed_tasks, 5), 'target': 5},
                {'id': 'ten_done', 'icon': '🎯', 'title': 'Десятка', 'desc': 'Выполнить 10 задач', 'unlocked': completed_tasks >= 10, 'progress': min(completed_tasks, 10), 'target': 10},
                {'id': 'twenty_five', 'icon': '🏅', 'title': '25 побед', 'desc': 'Выполнить 25 задач', 'unlocked': completed_tasks >= 25, 'progress': min(completed_tasks, 25), 'target': 25},
                {'id': 'fifty_done', 'icon': '💪', 'title': 'Полтинник', 'desc': 'Выполнить 50 задач', 'unlocked': completed_tasks >= 50, 'progress': min(completed_tasks, 50), 'target': 50},
                {'id': 'hundred_done', 'icon': '🏆', 'title': 'Сотня', 'desc': 'Выполнить 100 задач', 'unlocked': completed_tasks >= 100, 'progress': min(completed_tasks, 100), 'target': 100},
                {'id': 'streak_3', 'icon': '🔥', 'title': 'Разогрев', 'desc': '3 дня подряд', 'unlocked': max_streak >= 3, 'progress': min(max_streak, 3), 'target': 3},
                {'id': 'streak_7', 'icon': '🔥', 'title': 'Неделя огня', 'desc': '7 дней подряд', 'unlocked': max_streak >= 7, 'progress': min(max_streak, 7), 'target': 7},
                {'id': 'streak_14', 'icon': '🔥', 'title': 'Две недели', 'desc': '14 дней подряд', 'unlocked': max_streak >= 14, 'progress': min(max_streak, 14), 'target': 14},
                {'id': 'streak_30', 'icon': '🔥', 'title': 'Месяц в деле', 'desc': '30 дней подряд', 'unlocked': max_streak >= 30, 'progress': min(max_streak, 30), 'target': 30},
                {'id': 'delegator', 'icon': '📤', 'title': 'Делегатор', 'desc': 'Делегировать 5 задач', 'unlocked': delegated_by_me >= 5, 'progress': min(delegated_by_me, 5), 'target': 5},
                {'id': 'helper', 'icon': '🤝', 'title': 'Помощник', 'desc': 'Принять 5 делегированных', 'unlocked': delegated_to_me >= 5, 'progress': min(delegated_to_me, 5), 'target': 5},
                {'id': 'author', 'icon': '📝', 'title': 'Автор', 'desc': 'Создать 5 постов', 'unlocked': total_posts >= 5, 'progress': min(total_posts, 5), 'target': 5},
                {'id': 'blogger', 'icon': '✍️', 'title': 'Блогер', 'desc': 'Создать 20 постов', 'unlocked': total_posts >= 20, 'progress': min(total_posts, 20), 'target': 20},
                {'id': 'lightning', 'icon': '⚡', 'title': 'Молниеносный', 'desc': 'Выполнить за час', 'unlocked': lightning_count >= 1, 'progress': min(lightning_count, 1), 'target': 1},
                {'id': 'early_bird', 'icon': '🌅', 'title': 'Ранняя пташка', 'desc': 'Задача до 7 утра', 'unlocked': early_count >= 1, 'progress': min(early_count, 1), 'target': 1},
                {'id': 'night_owl', 'icon': '🦉', 'title': 'Сова', 'desc': 'Задача после 23:00', 'unlocked': night_count >= 1, 'progress': min(night_count, 1), 'target': 1},
                {'id': 'busy_bee', 'icon': '🐝', 'title': 'Пчёлка', 'desc': '10+ активных задач', 'unlocked': active_now >= 10, 'progress': min(active_now, 10), 'target': 10},
                {'id': 'networker', 'icon': '👥', 'title': 'Сетевик', 'desc': '5 избранных контактов', 'unlocked': fav_count >= 5, 'progress': min(fav_count, 5), 'target': 5},
                {'id': 'social', 'icon': '💬', 'title': 'Общительный', 'desc': '50 сообщений AI', 'unlocked': total_interactions >= 50, 'progress': min(total_interactions, 50), 'target': 50},
                {'id': 'chatterbox', 'icon': '🗣️', 'title': 'Болтун', 'desc': '200 сообщений AI', 'unlocked': total_interactions >= 200, 'progress': min(total_interactions, 200), 'target': 200},
                {'id': 'pro_profile', 'icon': '💼', 'title': 'Профессионал', 'desc': 'Заполнить весь профиль', 'unlocked': profile_fields_filled >= 7, 'progress': profile_fields_filled, 'target': 7},
                {'id': 'recommended', 'icon': '⭐', 'title': 'Рекомендован', 'desc': 'Рейтинг 8+/10', 'unlocked': avg_rating >= 8, 'progress': min(avg_rating, 10), 'target': 10},
                {'id': 'goal_setter', 'icon': '🎯', 'title': 'Целеустремлённый', 'desc': 'Создать 3 цели', 'unlocked': total_goals >= 3, 'progress': min(total_goals, 3), 'target': 3},
                {'id': 'goal_achiever', 'icon': '🏅', 'title': 'Триумфатор', 'desc': 'Достичь цели', 'unlocked': completed_goals >= 1, 'progress': min(completed_goals, 1), 'target': 1},
                {'id': 'veteran_7', 'icon': '📅', 'title': 'Новичок', 'desc': '7 дней в системе', 'unlocked': days_since_registration >= 7, 'progress': min(days_since_registration, 7), 'target': 7},
                {'id': 'veteran_30', 'icon': '🎪', 'title': 'Ветеран', 'desc': '30 дней в системе', 'unlocked': days_since_registration >= 30, 'progress': min(days_since_registration, 30), 'target': 30},
                {'id': 'veteran_90', 'icon': '🏛️', 'title': 'Старожил', 'desc': '90 дней в системе', 'unlocked': days_since_registration >= 90, 'progress': min(days_since_registration, 90), 'target': 90},
            ]
            
            # Находим последнюю открытую ачивку для отображения в профиле
            unlocked_achievements = [a for a in achievements if a['unlocked']]
            featured_achievement = unlocked_achievements[-1] if unlocked_achievements else None
        ...
        logger.error(f"Error in api_gamification: {e}", exc_info=True)
        return web.json_response({'error': 'Internal server error'}, status=500)
...
app.router.add_get('/api/gamification', api_gamification_handler)
