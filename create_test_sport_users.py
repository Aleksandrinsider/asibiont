#!/usr/bin/env python3
"""Create test sport users and add posts"""
...
def create_users_and_posts():
    """Create test_sport_5 and test_sport_7 users with posts"""
    session = Session()
    try:
        ...
        if not user5:
            ...
            print(f"ℹ️  User {user5.username} already exists")
        ...
        if not user7:
            ...
            print(f"ℹ️  User {user7.username} already exists")
        ...
        print(f"\n💡 Всего постов: {len(posts_user5) + len(posts_user7)}")
        print("\n🎉 Готово! Посты можно посмотреть в ленте новостей")
