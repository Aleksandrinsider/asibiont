...
def generate_user_response(ai_message, conversation_context, scenario_step):
    ...
    prompt = f"""Ты Алекс, продакт-менеджер из Москвы, работаешь в Яндекс. Общаешься с AI-ассистентом по задачам.

Твои данные (если спросят):
- Имя: Алекс
- Город: Москва
- Компания: Яндекс
- Должность: Продакт-менеджер
- Навыки: Python, управление проектами, аналитика
- Интересы: Стартапы, AI, продуктовый дизайн
- Цели: Запустить свой продукт

История диалога:
{context_summary}

AI сказал: "{ai_message}"

ВЕДИ СЕБЯ КАК ЖИВОЙ ЧЕЛОВЕК (шаг {scenario_step}):

1. Если AI спросил конкретный вопрос (имя? город? должность?) → ответь ПРЯМО
2. Если AI показал пустой список → добавь задачу с деталями
3. Если AI показал задачи → попроси действие (делегируй, удали, отметь выполненной)
4. Если AI добавил задачу → иногда "спасибо", иногда сразу новая команда
5. Если AI просит профиль → либо заполни, либо "давай сначала с задач"
6. Иногда (20%) просто поболтай: "как дела?", "что посоветуешь?", "окей"

ВАРЬИРУЙ команды:
- "Добавь встречу с клиентом завтра в 15:00"
- "Покажи задачи на сегодня"
- "Делегируй @maria подготовку презентации"
- "Удали задачу про звонок"
- "Найди мне дизайнера в Москве"
- "Отметь первую задачу выполненной"

Отвечай ЕСТЕСТВЕННО, 3-20 слов, БЕЗ кавычек:"""
...
async def test_ai_dialogue():
    ...
    try:
        ...
        print("="*80)
        print("DIALOGUE START - 10 ITERATIONS (AI-GENERATED RESPONSES)")
        print("="*80 + "\n")
