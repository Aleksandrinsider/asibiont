...
# Настроим логирование для перехвата tool calls
logging.basicConfig(level=logging.INFO)

async def test_ai_delete_with_logging():
    ...
    if user:
        ...
        test_task = Task(
            user_id=user.id,
            title="уникальная тестовая задача для удаления",
        ...
        user_message = "удали уникальную тестовую задачу для удаления"

        print(f"\nОтправляем AI команду: '{user_message}'")

        # Перехватим tool calls через логирование
        try:
...
# Запустим тест
asyncio.run(test_ai_delete_with_logging())