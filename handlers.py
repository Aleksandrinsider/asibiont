...
Настроим под вас
Напишите нам — и мы настроим агентов под ваш бизнес: подберём интеграции, пропишем задачи, обучим под вашу нишу. Работает с первого дня без долгого онбординга.
...
Set up for you
Write to us — and we'll configure agents for your business: select integrations, define tasks, train for your niche. Works from day one, no lengthy onboarding required.
...
async def refer_handler(message: Message):
    ...
    if lang == 'en':
        text = (
            ...
            f"For every invited user who tops up their balance — you get 20% of their payment in tokens.\n\n"
        ...
        text = (
            ...
            f"За каждого приглашённого, кто пополнит баланс — ты получаешь 20% от суммы пополнения токенами.\n\n"
...
async def _process_text_message_inner(user_id, text, message, state, user_lock):
    ...
    try:
        ...
        try:
            if lang == 'en':
                err_text = "AI server error while processing your request. Try again or rephrase it shorter."
            else:
                err_text = "Сбой при обработке запроса — AI-сервер вернул ошибку. Попробуй написать ещё раз или переформулировать короче."
