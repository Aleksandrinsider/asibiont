...
async def subscribe_handler(message: Message):
    ...
    await message.reply(f"Отлично! 😊 Подписка на месяц стоит 3000 рублей и дает полный доступ ко всем возможностям:\n\n• Чат с ИИ для планирования задач, напоминаний и мотивации\n• Поиск единомышленников по интересам\n• Персонализированные советы и рекомендации\n• Управление задачами с дедлайнами и приоритетами\n\nОплати по ссылке: {payment_url}\nПосле оплаты подписка активируется автоматически — и вперед к продуктивности! 🚀")
    session.close()
...
async def chat_handler(message: Message):
    ...
    try:
        ...
        if not subscription or subscription.status != 'active':
            await message.reply("Привет! 😊 Я умный ИИ-помощник для управления задачами и поиска единомышленников. С подпиской за 3000 рублей в месяц ты получишь:\n\n• Полный доступ к чату с ИИ для планирования дел, напоминаний и мотивации\n• Поиск партнеров по интересам (дизайн, программирование, спорт и т.д.)\n• Персонализированные советы и рекомендации\n• Управление задачами с напоминаниями и приоритетами\n\nОформи подписку командой /subscribe и начни продуктивно работать! 🚀")
            return
