...
async def find_partners_handler(message: Message):
    ...
    if os.getenv("LOCAL") == "1":
        ...
        try:
            ...
            context = []
    response = chat_with_ai("Найди людей", context, user_id)
    await message.bot.send_message(message.chat.id, response)
    # Сохранить контекст
    context.append({"user": "Найди людей", "agent": response})
    if len(context) > 10:
        context = context[-10:]
    if os.getenv("LOCAL") == "1":
        context_store[f"context:{user_id}"] = context
