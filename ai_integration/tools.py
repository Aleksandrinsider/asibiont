...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "generate_image",
            "description": "🎨 Сгенерировать изображение через AI (Flux). Вызывай когда пользователь просит: нарисуй, создай картинку, сгенерируй изображение, нужна иллюстрация, акварельная иллюстрация, нарисуй мне — и любые похожие формулировки. Промпт пиши на английском, детально. Картинка автоматически отправится пользователю в чат. Для поста — URL из результата передавай как image_url в create_post.",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
    ...
    },
    {
        ...
        "function": {
            "name": "generate_video",
            "description": "🎬 Сгенерировать короткое видео через Replicate (Wan-2.1). Ключевые слова: сними ролик, создай видео, reels, тикток-видео, для инстаграм, для тикток. Промпт пиши на английском. Требует REPLICATE_API_TOKEN. ⚠️ Одна генерация = максимум 5–10 сек. Если пользователь просит 15–30 сек — используй num_clips=2 или 3: система сгенерирует несколько сцен и даст инструкцию по склейке в CapCut. Для Instagram Reels и TikTok используй aspect_ratio='9:16' (вертикальный, по умолчанию).",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
