...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "📇 СОХРАНИТЬ email-контакт в справочник пользователя. Вызывай когда нашёл email человека через IMAP/RSS/поиск или после получения ответа. Сохранённые контакты учитываются в прогрессе целей.\nПеред сохранением спроси себя: 'Этот адрес ведёт к человеку, который принял бы решение по цели?' Личный адрес (firstname@, name.surname@) — прямая линия к нужному человеку. Общая почта (info@, contact@, team@) обрабатывается секретарём или автоответчиком — ответ маловероятен. Если нашёл только общий адрес — сделай доп. шаг: web_search('[Имя Фамилия] [компания] email') часто даёт личный.",
            "parameters": {
                "type": "object",
                "properties": {
                    "email": {
