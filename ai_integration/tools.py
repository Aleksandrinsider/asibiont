...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "update_profile",
        "description": "ОБЯЗАТЕЛЬНОЕ ОБНОВЛЕНИЕ ПРОФИЛЯ ПОЛЬЗОВАТЕЛЯ! Вызывай АВТОМАТИЧЕСКИ при ЛЮБОМ упоминании личной информации в разговоре. КЛЮЧЕВЫЕ СЛОВА: город, компания, должность, навыки, интересы, цели, работаю, занимаюсь, люблю, хочу, интересует, увлекаюсь, нравятся. ПРИМЕРЫ ОБЯЗАТЕЛЬНОГО ВЫЗОВА: 'я из Лондона' → update_profile(city='Лондон'), 'работаю в Google' → update_profile(company='Google'), 'дизайнер' → update_profile(skills='дизайнер'), 'люблю музыку' → update_profile(interests='музыка'), 'хочу заняться спортом' → update_profile(interests='спорт'), 'интересует программирование' → update_profile(interests='программирование'), 'хочу открыть бизнес' → update_profile(goals='открыть бизнес'). НИКОГДА не используй update_user_memory для этой информации!",
            "parameters": {
                "type": "object",
                "properties": {
