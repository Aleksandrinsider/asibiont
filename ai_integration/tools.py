...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "send_email",
            "description": "📧 ОТПРАВИТЬ EMAIL — универсальный инструмент. Отправь письмо на любой адрес: предложение, вопрос, напоминание, благодарность, приглашение, что угодно. НЕ связан с кампаниями — одиночная отправка. Письмо уйдёт с личной почты пользователя (Gmail/Яндекс/Mail.ru SMTP) — в приоритете. Если личной почты нет — через его личный Resend ключ. При нескольких личных почтах — система спросит, с какой отправить. Платформенный email НЕ используется.",
            "parameters": {
                "type": "object",
                "properties": {
                    "to": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "📨 НАЧАТЬ EMAIL-ПЕРЕГОВОРЫ для достижения конкретной цели. "
                "Агент сам ведёт переписку: отправляет первое письмо, отслеживает ответы и продолжает диалог "
                "до достижения цели (встреча, согласование условий, подтверждение, договорённость и т.п.). "
                "Используй вместо send_email когда нужен ДИАЛОГ с несколькими итерациями, а не разовая отправка. "
                "НЕ вызывай если пользователь хочет просто отправить одно письмо — тогда send_email."
            ),
            "parameters": {
                "type": "object",
                "properties": {
