...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "add_task",
            "description": "Создать НОВУЮ задачу. 'Напомни [что-то]' БЕЗ времени → вызвать add_task с title, reminder_time НЕ传; инструмент сам спросит время. С временем → вызвать сразу. Для изменения существующей задачи — edit_task, не add_task.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
                    ...
                    "reminder_time": {
                        "type": "string",
                        "description": "Время напоминания. Если время есть в запросе — используй сразу (форматы: 'завтра в 9:00', 'через 2 часа', 'YYYY-MM-DD HH:MM'). Если время НЕ указано — НЕ передавай это поле: инструмент автоматически попросит время у пользователя.",
                    },
                    "is_recurring": {
                        "type": "boolean",
