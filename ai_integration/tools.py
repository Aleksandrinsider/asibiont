...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_notes",
            "description": "Найти и прочитать заметки команды. Используй когда другой агент сохранил заметку и тебе нужно её прочитать, или когда пользователь спрашивает о сохранённых материалах. Возвращает полный текст заметок. Для конкретной заметки передай ключевые слова в query и limit=1 чтобы сэкономить токены.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Ключевые слова для поиска в заголовках и тексте заметок. Оставь пустым чтобы получить последние заметки.",
                    },
                    "limit": {
                        "type": "integer",
                        "description": "Максимальное количество заметок для возврата (1-20, по умолчанию 5).",
                    },
                },
                "required": [],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "save_note",
            "description": "Сохранить заметку — запись БЕЗ напоминания и дедлайна. ВСЕГДА используй когда пользователь говорит: 'запиши', 'в заметки', 'запомни', 'сохрани в заметки', 'добавь в заметки', 'заметка', 'запиши себе'. НЕ путай с add_task — заметки не имеют времени напоминания. По умолчанию source='chat' (личная заметка). source='blog' — только если пользователь явно попросил опубликовать в блог ('напиши в блог', 'опубликуй статью') или у агента есть задача вести блог. Никогда не ставь source='blog' по своей инициативе.",
            "parameters": {
                "type": "object",
                "properties": {
                    "content": {
    ...
    },
    {
        ...
        "function": {
            "name": "list_tasks",
            "description": "Показать все активные задачи пользователя. Используй когда: пользователь спрашивает о задачах, нужно проверить контекст перед добавлением, или нужен обзор текущей загрузки.",
            "parameters": {
                "type": "object",
                "properties": {
                    "include_completed": {
    ...
    },
    {
        ...
        "function": {
            "name": "research_topic",
            "description": "🔍 ПОИСК И АНАЛИЗ любой темы: тренды, конкуренты, конференции/ивенты/мероприятия, сообщества по теме, контакты организаторов. Единственный инструмент для поиска в интернете. Используй для: поиск ссылок/ресурсов, анализ рынка/ниши, тренды, стратегии, факты, цифры, программы тренировок, курсы, рецепты, маршруты путешествий, советы экспертов. depth='basic' для быстрого поиска ссылок, 'full' для анализа, 'deep' для глубокого исследования.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Тема для исследования. Примеры: 'рынок AI-ботов в России 2026', 'план подготовки к марафону для начинающих', 'лучшие книги по инвестициям 2026', 'курсы игры на гитаре с нуля'"
                    },
                    "depth": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "content": {
                        "type": "string",
                        "description": "Текст поста для блога. Для SEO-статей целевой объём 1700-2200 символов (мягкий коридор), структура: короткий лид + 3-5 смысловых блоков + вывод/CTA. Пиши живо, по делу, без воды и маркетинговых клише."
                    },
                    "image_url": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            "name": "edit_note",
            "description": "✏️ РЕДАКТИРОВАТЬ ЗАМЕТКУ / БЛОГ-СТАТЬЮ: Изменяет содержимое существующей заметки или статьи в блоге на сайте. Если note_id не указан — редактирует последнюю блог-статью. Используй когда пользователь говорит: 'измени статью', 'исправь заметку', 'обнови блог', 'отредактируй пост в блоге', 'измени содержание', 'перепиши статью'. НЕ путай с edit_post (который редактирует посты в ленте, а не блог).",
            "parameters": {
                "type": "object",
                "properties": {
                    "note_id": {
                        "type": "integer",
                        "description": "ID заметки/статьи для редактирования. Если не указан — редактирует последнюю блог-статью."
                    },
                    "new_content": {
                        "type": "string",
                        "description": "Новый текст заметки или статьи"
                    },
                    "new_title": {
                        "type": "string",
                        "description": "Новый заголовок (опционально, если нужно изменить заголовок)"
                    }
                },
                "required": ["new_content"]
            }
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "📢 Опубликовать пост в ПУБЛИЧНЫЙ Telegram-канал. Ключевые слова: пост в канал, опубликуй в телеграм.",
            "parameters": {
                "type": "object",
                "properties": {
                    "content": {
                        "type": "string",
                        "description": "Текст поста для ПУБЛИЧНОГО Telegram-канала. Поддерживает Markdown. Пиши для широкой аудитории: экспертный контент, кейсы, инсайты. БЕЗ обращения к конкретному пользователю. Стиль: профессиональный пост, НЕ личное сообщение."
                    },
                    "image_url": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "📢 Опубликовать пост в ПУБЛИЧНЫЙ Discord канал через webhook. Ключевые слова: пост в дискорд, опубликуй в discord.",
            "parameters": {
                "type": "object",
                "properties": {
                    "content": {
                        "type": "string",
                        "description": "Текст поста для ПУБЛИЧНОГО Discord-канала. Пиши для широкой аудитории: экспертный контент, аналитика, кейсы, инсайты. БЕЗ обращения к конкретному пользователю (НЕ 'для тебя', 'твои задачи', 'я нашёл тебе'). Стиль: профессиональный пост в канале, а НЕ личное сообщение."
                    },
                    "image_url": {
                        "type": "string",
    ...
    # ═════ EMAIL — отправка писем конкретному адресату ═════
    {
        ...
        "function": {
            ...
            "description": "📤 Отправить персонализированное email конкретному адресату. Используй для outreach: пиши первое письмо конкретному человеку с персонализированным текстом. ⚠️ Только для ВНЕШНИХ контактов. Внутренние агенты/коллеги команды (имена из \"ТВОЯ КОМАНДА\", адреса @asibiont.com) — НЕ email: используй DELEGATE[Имя].",
            "parameters": {
                "type": "object",
                "properties": {
                    "recipient_email": {
                    ...
                    "body": {
                        "type": "string",
                        "description": "Текст письма — персонализированный. Пиши от первого лица, коротко (3-5 абзацев), с конкретным предложением и CTA. Первая строка — конкретная деталь о работе/проекте получателя (НЕ общий комплимент). Запрещено: 'Я был впечатлён', 'I was impressed by your work in [широкая область]'. ⛔ ЗАПРЕЩЕНО выдумывать цифры: 'экономит 15 часов в неделю', 'сократил рутину на 20 часов', 'конверсия 12%', '+32% за месяц' — если нет реальных данных, не пиши числа вообще."
                    }
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "💬 ОТВЕТИТЬ на входящее письмо от конкретного человека. Вызывается автономно при входящем reply или когда пользователь просит ответить. ⚠️ КАЧЕСТВО: отвечай ТОЛЬКО если ответ действительно нужен — не пиши «спасибо за ответ» без содержательного продолжения. Каждый ответ должен продвигать диалог к цели.",
            "parameters": {
                "type": "object",
                "properties": {
                    "outreach_id": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "📩 FOLLOW-UP EMAIL — повторное письмо получателю, который не ответил. Используй когда якорь email_follow_up сработал или пользователь просит отправить follow-up. Учитывай оригинальное письмо, чтобы не повторяться. ⚠️ КАЧЕСТВО: follow-up имеeт смысл ТОЛЬКО если есть что сказать нового — новая ценность, уточнение, другой angle. Не отправляй follow-up «просто напомнить о себе» без новой информации.",
            "parameters": {
                "type": "object",
                "properties": {
                    "outreach_id": {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "sender_name": {
                        "type": "string",
                        "description": "Имя отправителя. По умолчанию — имя пользователя (владельца аккаунта). НЕ пиши имя агента — ты действуешь от лица пользователя."
                    },
                    "from_account": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "📇 СОХРАНИТЬ email-контакт в справочник пользователя. Вызывай когда нашёл email человека через IMAP/RSS/поиск или после получения ответа. Сохранённые контакты учитываются в прогрессе целей. ⚠️ Сохраняй только ВНЕШНИЕ контакты; внутренних агентов/коллег команды (в т.ч. @asibiont.com) не сохраняй как лиды — для них используй DELEGATE[Имя].\nПеред сохранением спроси себя: 'Кто читает этот ящик и подходит ли этот человек для моей цели?'\nЛичный (firstname@, name.surname@) — прямая линия к нужному человеку, хорошо для любой цели.\nКорп. ящик иногда уместен: press@ / media@ если цель — публикация; partners@ если цель — B2B-партнёрство; info@ маленького стартапа (до 10 чел) часто читает основатель. Но info@ крупной компании = автоответ или колл-центр.\nЕсли адрес не соответствует цели — попробуй найти личный через web_search('[Имя Фамилия] [компания] email').",
            "parameters": {
                "type": "object",
                "properties": {
                    "email": {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "post_time": {
                        "type": "string",
                        "description": "Время публикации (HH:MM). Если пользователь указал — передай. Если нет — не передавай, система предложит свободный слот из расписания."
                    },
                    "max_posts": {
                        "type": "integer",
    ...
    },
    {
        ...
        "function": {
            "name": "generate_image",
            "description": "🎨 Сгенерировать изображение через AI (Flux). Вызывай когда пользователь просит: нарисуй, создай картинку, сгенерируй изображение, нужна иллюстрация, нарисуй мне — и любые похожие формулировки. Картинка автоматически отправится пользователю в чат. Для поста — URL из результата передавай как image_url в create_post. ❌ НЕ используй когда просят видео, ролик, Reels-видео, TikTok-видео — для этого есть generate_video.",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
    ...
    },
    {
        ...
        "function": {
            "name": "generate_video",
            "description": "🎬 ВИДЕО-ГЕНЕРАТОР. Вызывай ВСЕГДА когда просят: сделай видео, сними ролик, создай видео, reels, тикток-ролик, видеоролик для инстаграм/тикток — любые варианты слов ВИДЕО и РОЛИК. ❌ НЕ заменяй на generate_image если просят видео — пользователь ждёт именно видео. Промпт пиши на английском. Требует REPLICATE_API_TOKEN. ⚠️ Одна генерация = максимум 5–10 сек. Если пользователь просит 15–30 сек — используй num_clips=2 или 3: система сгенерирует несколько сцен. Для Instagram Reels и TikTok используй aspect_ratio='9:16' (вертикальный, по умолчанию).",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
                    ...
                    "aspect_ratio": {
                        "type": "string",
                        "description": "Соотношение сторон. 9:16 = вертикальный (Instagram Reels, TikTok, Stories) — по умолчанию. 16:9 = горизонтальный (YouTube). 1:1 = квадрат (Instagram Feed).",
                        "enum": ["9:16", "16:9", "1:1"]
                    },
                    "duration": {
                        "type": "integer",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "⚡ Выполнить действие через подключённую интеграцию агента (любые внешние API и сервисы). Используй творчески — одна интеграция может решать разные задачи. Если у пользователя несколько агентов — укажи agent_name чтобы выбрать нужного.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
                    ...
                    "agent_name": {
                        "type": "string",
                        "description": "Имя агента, через которого выполнить действие. Указывай когда у пользователя несколько агентов с разными интеграциями."
                    }
                },
                "required": ["action"]
            }
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "🐛 Создать issue в GitHub репозитории. Используй когда пользователь просит: 'создай issue', 'открой задачу в GitHub', 'зафиксируй баг в репозитории', 'добавь feature request'. GITHUB_TOKEN подставляется автоматически из настроек агента. ⚠️ КАЧЕСТВО: ПРЕЖДЕ ЧЕМ СОЗДАТЬ — проверь что такой issue ещё не создавался (похожий заголовок/тема). Не дублируй существующие задачи. Создавай issue только если это действительно новая, конкретная, выполнимая задача — не «на всякий случай».",
            "parameters": {
                "type": "object",
                "properties": {
                    "repo_url": {
    ...
    },
    {
        ...
        "function": {
            "name": "ask_agent",
            "description": "🗣️ Задать ВОПРОС другому агенту из твоей команды и ДОЖДАТЬСЯ ответа. Используй когда тебе нужна информация, которой у тебя нет — например, спросить у аналитика данные, у кодеров статус разработки, у исследователя результаты поиска. Агент получит твой вопрос, подумает и вернёт ответ. НЕ используй для сообщений без ожидания ответа — для этого есть tell_agent.",
            "parameters": {
                "type": "object",
                "properties": {
                    "agent_name": {
                        "type": "string",
                        "description": "Имя агента-получателя (из списка команды). Пиши имя в точности как оно указано в секции КОМАНДА КОЛЛЕГ."
                    },
                    "question": {
                        "type": "string",
                        "description": "Чёткий вопрос агенту. Укажи что именно нужно узнать или сделать. Пример: 'Какой статус по задаче с email-рассылкой?'"
                    }
                },
    ...
    },
    {
        ...
        "function": {
            "name": "tell_agent",
            "description": "📨 Отправить СООБЩЕНИЕ/ФАКТ/РЕЗУЛЬТАТ другому агенту БЕЗ ожидания ответа. Используй когда нужно поделиться информацией: сообщить о завершении задачи, передать найденные данные, предупредить о чём-то. Агент увидит это сообщение при следующем запуске. НЕ используй для вопросов — для этого есть ask_agent.",
            "parameters": {
                "type": "object",
                "properties": {
                    "agent_name": {
                        "type": "string",
                        "description": "Имя агента-получателя (из списка команды)."
                    },
                    "message": {
                        "type": "string",
                        "description": "Сообщение/факт/результат для другого агента. Пример: 'Я завершил анализ рынка, отчёт сохранён в заметках под заголовком \"Анализ рынка 13.05\"'"
                    }
                },
