...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "add_task",
            "description": "Создать НОВУЮ задачу. Явные команды с временем → создавай сразу. Если время не указано — спроси. Для изменения существующей задачи — edit_task, не add_task.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
    ...
    },
    {
        ...
        "function": {
            "name": "complete_task",
            "description": "Отметить задачу выполненной. Ключевые слова: сделал, готово, завершил, разобрался. Заполняй completion_note.",
            "parameters": {
                "type": "object",
                "properties": {
                    "task_id": {
    ...
    },
    {
        ...
        "function": {
            "name": "delegate_task",
            "description": "Делегировать задачу другому пользователю ИЛИ активированному агенту команды. Используй когда видишь 'делегируй', 'поручи', 'передай', 'попроси агента'. Примеры: 'делегируй Ивану X', 'поручи @maria X завтра в 10:00', 'поручи Кристине проверить почту', 'передай Марку задачу'.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
                    ...
                    "delegated_to_username": {
                        "type": "string",
                        "description": "Имя получателя — реального пользователя (@username без @) ИЛИ имя активированного агента из команды. Примеры: 'maria', 'Иван', 'Кристина', 'Марк'",
                    },
                    "reminder_time": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            "name": "create_post",
            "description": "📝 Создать пост в общую ленту новостей платформы (не TG-канал). Публикуй только после явного согласия пользователя.",
            "parameters": {
                "type": "object",
                "properties": {
                    "content": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "🤝 Запустить кампанию внешнего поиска исполнителей: агент находит реальных людей в интернете (не внутренних агентов) и рассылает им приглашения взять задачу на тестирование/выполнение. По сути — это аутрич-кампания: бот ищет подходящих людей, пишет им, они принимают или отклоняют. Когда отчитываться пользователю: называй это 'поиск тестировщиков' или 'рассылка приглашений', не 'делегирование' — последнее звучит как внутреннее поручение. Ключевые слова: найди тестировщиков, найди волонтёров, разошли приглашения.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
    ...
    },
    {
        ...
        "function": {
            "name": "generate_image",
            "description": "🎨 Сгенерировать изображение (Flux). Ключевые слова: нарисуй, создай изображение, сгенерируй картинку. Пиши промпт на английском.",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
