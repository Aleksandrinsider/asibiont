...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "get_task_advice",
            "description": "Получить AI-совет по выполнению конкретной задачи. Используй когда: пользователь спрашивает КАК сделать задачу, просит помощи в решении, не знает с чего начать, застрял. Примеры: 'как лучше подготовить презентацию', 'помоги с запуском проекта', 'не знаю как начать изучать Python', 'что делать с задачей X'. AI проанализирует задачу и даст конкретные практические шаги, инсайты, ресурсы.",
            "parameters": {
                "type": "object",
                "properties": {
                    "task_id": {
                    ...
                    "task_title": {
                        "type": "string",
                        "description": "Название задачи (если task_id неизвестен)",
                    },
                },
                "required": [],
            },
