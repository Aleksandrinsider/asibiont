...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "delete_post",
            "description": "🗑 УДАЛИТЬ ПОСТ: Удаляет пост пользователя из ленты. Ключевые слова: 'удали пост', 'убери пост', 'удали публикацию'. Если пользователь говорит 'удали' сразу после публикации — удаляет последний пост. Можно указать post_id для удаления конкретного поста.",
            "parameters": {
                "type": "object",
                "properties": {
                    "post_id": {
