...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "add_task",
            "description": "Создать НОВУЮ задачу. ТОЛЬКО по явному согласию пользователя — сначала предложи в тексте, потом создавай. НИКОГДА не придумывай reminder_time — если пользователь не указал время, СПРОСИ и дождись ответа, не создавай без времени. Для ИЗМЕНЕНИЯ или ДОПОЛНЕНИЯ существующей задачи (время, описание, название) — ВСЕГДА edit_task, НЕ add_task. Если задача уже создана и пользователь даёт время/детали — это edit_task.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
                    ...
                    "description": {
                        "type": "string",
                        "description": "Краткие доп. детали (макс 100 символов), НЕ повторяющие название. Оставляй пустым если деталей нет. НЕ пиши длинные описания.",
                    },
                    "reminder_time": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "content": {
                        "type": "string",
                        "description": "Текст поста для ленты новостей. 2-5 абзацев, живой стиль, без маркетинговых клише. Пиши от лица пользователя."
                    }
                },
                "required": ["content"]
            }
