...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "save_user_rule",
            "description": "Сохранить поведенческое правило или предпочтение пользователя в долгосрочную память. ВЫЗЫВАЙ ОБЯЗАТЕЛЬНО при любой фразе которая задаёт поведение на будущее: 'не трогай X', 'ищи только Y', 'не привлекай тех кто...', 'всегда делай Z', 'при задаче A — используй B', 'агенты должны...', 'не нужно C', 'только D'. Примеры ОБЯЗАТЕЛЬНОГО вызова: 'контакты уже в ASI Biont не нужны' → правило об исключениях; 'не пиши на русском' → языковое правило; 'сначала всегда проверяй X' → порядок действий. Правило автоматически передаётся всем агентам пользователя в каждом цикле. НЕ вызывай для одноразовых просьб — только для инструкций на будущее.",
            "parameters": {
                "type": "object",
                "properties": {
                    "rule": {
                        "type": "string",
                        "description": "Чёткая формулировка правила — что нужно делать или не делать, в каком контексте. Максимально конкретно и без двусмысленности."
                    }
                },
                "required": ["rule"]
            }
