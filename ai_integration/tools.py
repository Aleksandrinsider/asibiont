...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "ПЕРЕНЕСТИ ВРЕМЯ СУЩЕСТВУЮЩЕЙ ЗАДАЧИ. ОБЯЗАТЕЛЬНО используй когда пользователь хочет ИЗМЕНИТЬ ВРЕМЯ задачи. Ключевые слова: 'перенеси', 'давай перенесем', 'измени время', 'поставь на другое время'. ВСЕГДА используй reschedule_task для переноса, НИКОГДА не создавай новую задачу через add_task. Если название не указано, будет перенесена последняя активная задача. Примеры: 'перенеси встречу на завтра в 16:00' → reschedule_task(task_title='встреча', new_time='завтра в 16:00'), 'давай перенесем на 15 минут' → reschedule_task(new_time='через 15 минут'), 'перенеси её через 15 минут' → reschedule_task(new_time='через 15 минут')",
            "parameters": {
                "type": "object",
                "properties": {
                    "task_title": {
