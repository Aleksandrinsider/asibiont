...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "send_email",
            "description": "📧 ОТПРАВИТЬ EMAIL — универсальный инструмент. Отправь письмо на любой адрес: предложение, вопрос, напоминание, благодарность, приглашение, что угодно. НЕ связан с кампаниями — одиночная отправка. Если у пользователя подключён Gmail/Яндекс/Mail.ru — письмо уйдёт с его личного ящика. Если подключено несколько почт — система попросит уточнить, с какой отправить. Используй когда пользователь даёт email и просит написать/отправить что-то конкретное.",
            "parameters": {
                "type": "object",
                "properties": {
                    "to": {
