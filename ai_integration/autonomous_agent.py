    ...
    def _fix_tool_params(self, tool_name, params, user_message=None):
        ...
        if tool_name == 'find_relevant_contacts_for_task':
            ...
            if not has_any:
                ...
                # Словарь творительный → именительный падеж
                _position_cases = {
                    'маркетологом': 'Маркетолог', 'программистом': 'Программист',
                    'разработчиком': 'Разработчик', 'дизайнером': 'Дизайнер',
                    'менеджером': 'Менеджер', 'директором': 'Директор',
                    'инженером': 'Инженер', 'аналитиком': 'Аналитик',
                    'тимлидом': 'Тимлид', 'фрилансером': 'Фрилансер',
                    'предпринимателем': 'Предприниматель', 'продюсером': 'Продюсер',
                    'консультантом': 'Консультант', 'архитектором': 'Архитектор',
                    'тестировщиком': 'Тестировщик', 'администратором': 'Администратор',
                    'редактором': 'Редактор', 'копирайтером': 'Копирайтер',
                    'стратегом': 'Стратег', 'специалистом': 'Специалист',
                    'руководителем': 'Руководитель', 'координатором': 'Координатор',
                    'основателем': 'Основатель', 'партнёром': 'Партнёр',
                    'бухгалтером': 'Бухгалтер', 'юристом': 'Юрист',
                    'экономистом': 'Экономист', 'логистом': 'Логист',
                    'методистом': 'Методист', 'преподавателем': 'Преподаватель',
                }
                for pat in position_patterns:
                    ...
                    if m:
                        val = m.group(1).strip()
                        # Нормализуем падеж
                        val_norm = _position_cases.get(val.lower())
                        if val_norm:
                            val = val_norm
                        elif val.endswith('ом') or val.endswith('ем'):
                            # Общее правило: убираем -ом/-ем, но это неточно
                            pass  # Лучше не трогать, если нет в словаре
                        if len(val) > 1:
                            params['position'] = val
                        break
