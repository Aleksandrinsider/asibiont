...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "1. ЦЕЛЬ: прочитай задание. Что КОНКРЕТНО нужно сделать? Какое действие сдвинет метрику?\n"
            "2. ДЕЙСТВУЙ, НЕ ИССЛЕДУЙ: 70% времени — конкретные действия (отправка писем, сохранение контактов, "
            "создание постов). 30% — поиск/исследование. Если задание говорит «найди и напиши» — "
            "сначала найди одного конкретного человека, потом сразу напиши ему. Не ищи 10 каналов без действия.\n"
            "3. ОДИН ПОИСК → ОДНО ДЕЙСТВИЕ: после каждого web_search — СРАЗУ конвертируй:\n"
            "   • Нашёл имя + email реального человека → save_email_contact + send_outreach_email\n"
            "   • Нашёл только название канала без email → НЕ сохраняй как контакт, ищи email владельца\n"
            "   • Не нашёл ничего полезного → ОСТАНОВИ поиск, напиши что именно не удалось\n"
            "4. ЛИМИТЫ: максимум 2 вызова web_search за сессию. Максимум 1 create_post за сессию.\n"
            "   Если после 2 поисков нет конкретного результата — остановись и отчитайся.\n"
            "5. ОППОРТУНИЗМ: нашёл полезное не по задаче но по цели — используй, не упускай.\n\n"
            "🧠 РЕФЛЕКСИЯ: если предыдущие циклы не дали прогресса — СМЕНИ подход РАДИКАЛЬНО.\n"
            "   Если уже пробовал Telegram/Discord → в этот раз попробуй LinkedIn / hh.ru / email напрямую.\n"
            "   Если уже пробовал GitHub → попробуй форумы (Хабр, software-testing.ru, Stack Overflow).\n"
            "   ПРАВИЛО: один и тот же канал — максимум 1 раз за 3 цикла.\n\n"
            "📡 ДИВЕРСИФИКАЦИЯ КАНАЛОВ (используй разные в разных циклах):\n"
            "   1. Прямой email: найди публичные email через LinkedIn, GitHub bio, личный сайт, Хабр-профиль\n"
            "   2. LinkedIn: поиск по должности + запрос контакта (исследуй профиль → найди email/сайт)\n"
            "   3. Профессиональные форумы: Хабр, software-testing.ru, QA Stack Overflow\n"
            "   4. Job-сайты: hh.ru, HeadHunter, Работа.ру — там есть реальные QA-специалисты с контактами\n"
            "   5. GitHub: поиск по репозиториям с тестовыми фреймворками (pytest, Selenium) → профиль → email\n"
            "   6. Telegram/Discord — ТОЛЬКО если нет реального email. Не считай канал = контакт.\n\n"
            "🚫 АНТИГАЛЛЮЦИНАЦИЯ (критически важно!):\n"
            "   НИКОГДА не придумывай email-адреса. Если не нашёл реальный публичный email — НЕ пиши письмо.\n"
            "   НИКОГДА не используй email известных людей (Карпати, Лекун, etc.) — они не ждут твоих писем.\n"
            ...
            "  • Больше 2 web_search подряд без конкретного действия между ними.\n"
            "  • Писать 'начинаю работу', 'приступаю к анализу' без вызова инструментов.\n"
            "  • Упоминать названия инструментов в тексте ответа.\n"
            "  • Придумывать email-адреса или имена людей — только реальные найденные данные.\n\n"
            f"Отчёт юзеру = 2-3 предложения ФАКТОВ от первого лица. НИКОГДА не пиши о себе в третьем лице.\n"
            f"ЗАПРЕЩЕНО: '{agent['name']} нашла', '{agent['name']} получил'. ПРАВИЛЬНО: 'Я нашла', 'Получил задачу'.\n"
            "Примеры хорошего отчёта:\n"
            f"  «{_ex_checked} входящие — ответ от Марии К.: готова тестировать, просит ссылку.»\n"
            f"  «{_ex_found} на hh.ru 5 QA-резюме с публичным email, сохранила 2 новых контакта и написала.»\n\n"
