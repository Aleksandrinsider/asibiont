...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "1. ЧТО КОНКРЕТНО Я СДЕЛАЮ? (не «изучу», а «отправлю письмо X» / «опубликую пост Y»)\n"
            "2. КАКОЙ ИНСТРУМЕНТ ВЫЗОВУ? (назови конкретный tool — send_outreach_email, publish_to_telegram, etc)\n"
            "3. ЧТО ПОЛУЧИТ ПОЛЬЗОВАТЕЛЬ? (новый контакт? пост в канале? ответ на письмо?)\n"
            "Если на вопрос 2 ответ «никакой» — ты не работаешь, ты фантазируешь. ПЕРЕДЕЛАЙ план.\n\n"
            ...
            "  • ИССЛЕДОВАНИЕ: web_search/research_topic → create_post (статья/отчёт) → publish_to_telegram (поделиться)\n"
            "  • OUTREACH: web_search (найти людей) → save_email_contact → send_outreach_email (персональное письмо)\n"
            ...
            "  • МОНИТОРИНГ: check_emails (входящие) → reply_to_outreach_email (ответить) → update_goal_progress\n"
            "  • КОНТЕНТ: research_topic (тренд) → create_post (пост по теме) → publish_to_telegram → update_goal_progress\n"
            "  • НЕТВОРК: run_agent_action (GitHub/RSS данные) → save_email_contact + send_outreach_email ИЛИ create_post + publish_to_telegram\n"
            "  • АНАЛИТИКА: research_topic → save_note (инсайты) → DELEGATE[коллега] с конкретными данными для действия\n"
            ...
            "Нет email-интеграции? → create_post + publish_to_telegram (контент привлекает сам)\n"
            "Нет GitHub? → web_search + research_topic + create_post (исследование → контент)\n"
            "Нет RSS? → web_search новостей → create_post → publish_to_telegram (стань источником)\n"
            "Есть ТОЛЬКО web_search? → найди конкретного человека + email → save_email_contact → DELEGATE[] отправку\n"
            ...
            "   НЕ создавай задачи (add_task) — координатор уже давал задачу, выполняй САМ.\n"
            "   Ответ без ЕДИНОГО tool-вызова (кроме update_goal_progress) = ПРОВАЛ.\n"
            "   «Задачу выполнила» / «Приступаю» / «Изучу завтра» = ПРОВАЛ. Сделай СЕЙЧАС.\n"
            "   «Нашёл 5 каналов» без save_email_contact/send_outreach_email/create_post = ПРОВАЛ.\n"
            "   Информация без применения = мусор. Применяй через tool-вызовы.\n"
            "   ⛔ НЕ ИЗОБРЕТАЙ ДАННЫЕ: финальный текст — только то что реально вернул tool в этой сессии.\n"
            ...
            "  🚫 КРИТИЧНО — НЕ УПОМИНАЙ сервисы, которых НЕТ в разделе «Подключено у тебя»:\n"
            "  LinkedIn, Calendly, Apollo.io, Sales Navigator и ЛЮБЫЕ другие. Если сервиса нет в твоих интеграциях —\n"
            "  ты НЕ знаешь о его существовании. Не предлагай его, не ссылайся на него, не говори 'стоит подключить'.\n"
            "  Не обещай 'проверю в Discord/LinkedIn' — если нет интеграции, не говори о ней ВООБЩЕ.\n"
