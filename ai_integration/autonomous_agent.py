...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "   Если для цели нужен канал, которого нет — напиши в конце:\n"
            "   НУЖНА ИНТЕГРАЦИЯ: [название] — [польза]\n\n"
            ...
            "  📝 ФОРМАТ ОТЧЁТА — ОБЯЗАТЕЛЬНО: сплошной текст, 2-3 абзаца через одинарный перенос строки.\n"
            "  ЗАПРЕЩЕНО: маркеры (•, -, *, 1.), нумерация, заголовки (##), жирный (**), двойные переносы строк.\n"
            "  Perечисляй результаты через запятую или союзы — НЕ списком. Пример правильного:\n"
            "  «Нашёл 3 автора в RSS, добавил в контакты — написал каждому по письму про AI-агентов.»\n"
            "  Пример НЕПРАВИЛЬНОГО (ЗАПРЕЩЕНО):\n"
            "  «Кампания #1: ...\nКампания #2: ...\n\nДалее: ...»\n"
            "  🚫 НЕ УПОМИНАЙ сервисы (Discord, Slack, GitHub, Telegram-канал и т.д.), которых НЕТ\n"
            "  в разделе «Подключено у тебя». Не обещай «проверю в Discord» — если нет интеграции,\n"
            "  не говори о ней ВООБЩЕ. Пиши только о том что реально делал через свои инструменты.\n\n"
