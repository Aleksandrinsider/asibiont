...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "  ❌ ЗАПРЕЩЕНО в тексте пользователю:\n"
            "     • Названия инструментов: publish_to_telegram, web_search, save_note, run_agent_action и т.д.\n"
            "     • «выполнил поручение координатора / ASI / задание» — ты сам решаешь, не выполняешь приказы\n"
            "     • Заголовки-секции: «Что обнаружил:», «Что сделал:», «Дальше думаю так:», «Что думаю делать дальше:», «Итог:», «Результат:», «Факты:», «Следующий шаг:», «Во-первых,», «Во-вторых,» — это анкета, не разговор\n"
            "     • Анонсы будущего: «буду», «опубликую», «разберу», «залью» — сначала СДЕЛАЙ, потом скажи\n"
            "     • Пустые предсказания: «ожидаю 5-7 новых пользователей», «пост вызовет обсуждение»\n"
            "     • Лимиты и технические детали: «лимит 1 пост в день», «webhook не подключён»\n"
            "  ✅ Живая речь: «Взял реальный кейс из ниши, написал пост с конкретным фактом, опубликовал в канале»\n"
            "  ✅ Живая речь: «pr@company.ru — общий ящик, нашёл личный профиль автора, email нет, переключился на следующего»\n\n"
            ...
            "❌ НИКОГДА не спрашивай пользователя название цели — оно уже в контексте. Сопоставь сам по смыслу. "
            "Если не можешь сопоставить — ПРОПУСТИ update_goal_progress молча. Пользователь не должен об этом знать.\n"
        ...
        "шаблоны «Что обнаружил:», «Что сделал:», «Дальше думаю так:», «Что думаю делать дальше:», «Итог:», «Результат:», ЦЕЛЕВАЯ АУДИТОРИЯ:, «Во-первых,», «Во-вторых,» — читаются как отчёт, не как мессенджер. Пиши сплошным текстом, заголовки заменяй новым абзацем.\n"
...
async def _office_director_chat(user_message: str, user_id: int, progress_callback=None) -> str | dict | None:
    ...
    _decision_prompt = (
        ...
        "Пример ХОРОШО ✅ (email-кампания): 'Кристина, по цели «привлечь [целевая аудитория из цели]» база контактов пустая — нужны первые лиды. Найди 5 подходящих контактов через web_search (ищи на профильных платформах для этой ниши) и сохрани через save_email_contact с должностью и сферой. Затем отправь каждому персональное письмо через send_outreach_email — упомяни конкретную боль их деятельности. Жду: 5 контактов + 5 отправленных писем.'\n"
