...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            f"  {'Нашла' if _is_fem else 'Нашёл'} ник/логин без email → run_agent_action search_github_users с этим логином (возвращает email из commit/профиля GitHub).\n"
            "  ⚠️ web_search «логин email contact» НИКОГДА не находит личный email — это потеря хода. Используй GitHub API.\n"
