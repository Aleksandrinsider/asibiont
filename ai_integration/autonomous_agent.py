...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        system_prompt += (
            ...
            "Ты ИСПОЛНИТЕЛЬ. Твоя задача — ПРОДВИНУТЬ цель, а не описать ситуацию.\n"
            "1. Прочти цель + историю действий ниже\n"
            "2. Определи ОДИН новый конкретный шаг (НЕ повторяй то, что уже делалось)\n"
            "3. ВЫПОЛНИ шаг инструментом:\n"
            "   - add_task: создай задачу-действие с конкретным результатом и дедлайном\n"
            "   - research_topic: найди КОНКРЕТНЫХ людей, площадки, контакты (не 'тренды')\n"
            "   - send_email / send_outreach_email: напиши конкретным контактам\n"
            "   - find_relevant_contacts_for_task: найди новые контакты\n"
            "4. По результату — КРАТКО (2-3 предложения) что сделал + конкретный результат\n\n"
            "ЗАПРЕЩЕНО: 'исследование показало', 'анализ трендов', 'выявлены сообщества' БЕЗ "
            "конкретного действия. Каждый цикл = новое действие, а не отчёт об исследовании.\n"
            "ПРИОРИТЕТ ДЕЙСТВИЙ: создать задачу > исследовать > описать."
        )
