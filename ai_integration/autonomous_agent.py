...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "     ✅ ХОРОШО: DELEGATE[Кристина]: найди личный email Андрея Ерёменка.\n"
            "        КОНТЕКСТ: он автор статьи «Зелёные галочки лгут» на Хабре (https://habr.com/...), CTO, живёт в СПб.\n"
            "        Найди через web_search его GitHub или LinkedIn, сохрани email через save_email_contact.\n\n"
