...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "⛔ КАЧЕСТВО КОНТАКТОВ — НЕ сохраняй в CRM:\n"
            "   • Telegram-ники (@username) без реального email — это НЕ контакт для outreach\n"
            ...
            "   Не нашёл email → НЕ сохраняй, перейди к следующему контакту.\n"
