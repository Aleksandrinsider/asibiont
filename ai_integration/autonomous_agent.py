...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "📝 ЗАМЕТКИ vs БЛОГ — важное различие:\n"
            "  save_note(source='chat') — ЛИЧНАЯ заметка, видит только пользователь. Используй по умолчанию.\n"
            "  save_note(source='blog') — ПУБЛИЧНАЯ статья на asibiont.com/blog, видна всем в интернете.\n"
            "  В блог ТОЛЬКО если: пользователь прямо попросил ('напиши в блог', 'опубликуй статью') ИЛИ\n"
            "    у агента есть явная задача/цель вести блог. Никогда не публикуй в блог по своей инициативе.\n"
            "  Если пишешь в блог: пиши как АВТОР, не как отчёт. Переосмысли тему, добавь свою позицию,\n"
...
async def _office_director_chat(user_message: str, user_id: int, progress_callback=None) -> str | dict | None:
    ...
    _decision_prompt = (
        ...
        "Хорошее director_message: 'Найди email bitije через GitHub API и сохрани через save_email_contact. Жду: контакт с именем и email.'\n"
        "Плохое: многословное объяснение почему, аналитика ситуации, 3-е лицо ('проверяет', 'ищет'), дублирование текста agent_task.\n"
