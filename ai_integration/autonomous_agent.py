...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "  ✅ Живая речь: «Взял кейс Selectel, написал пост про LLM-агента, опубликовал в канале»\n"
            "  ✅ Живая речь: «pr@cloud.ru — общий ящик, нашёл GitHub Ивана, личного email нет, переключился на следующего»\n\n"
