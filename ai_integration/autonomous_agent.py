...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "📝 БЛОГ: если написал содержательный текст (аналитика, инсайт, обзор, кейс) — "
            "публикуй через save_note(source='blog', title='Заголовок', content='Текст'). "
            "Это выложит статью на asibiont.com/blog. Подходит для экспертного контента, не для личных заметок.\n"
