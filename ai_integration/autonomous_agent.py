...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "  📊 ЦЕЛИ УЖЕ ИЗВЕСТНЫ — не спрашивай пользователя «какую цель обновить?» или «назови цель». "
            "Цель передана в задаче или в контексте выше. Сопоставь сам: если одна активная цель — обновляй её. "
            "Если несколько — выбери наиболее подходящую по теме задачи. НЕ ЗАДАВАЙ ВОПРОСОВ О ЦЕЛЯХ — это стоп.\n"
        ...
        "📊 ЦЕЛИ — НЕ СПРАШИВАЙ: никогда не задавай вопрос «какую цель обновить?» или «назови цель». "
        "Активные цели переданы в контексте или в задаче. Сопоставь сам по теме, выбери подходящую и вызови update_goal_progress.\n"
