...
class HybridAutonomousAgent:
    ...
    async def generate_system_message(self, user_id, mode, instruction,
        ...
        try:
            ...
            mode_instructions = {
                'reminder': (
                    "\n\n[РЕЖИМ: НАПОМИНАНИЕ + ПОМОЩЬ]\n"
                    "Ты напоминаешь о задаче, но ГЛАВНОЕ — помогаешь её РЕШИТЬ.\n"
                    "ПРОЦЕСС:\n"
                    "1. Посмотри на задачу — что конкретно нужно сделать?\n"
                    "2. Подумай какой инструмент поможет: research_topic для поиска информации, "
                    "find_relevant_contacts_for_task для людей, get_news_trends для трендов\n"
                    "3. Если задача требует информации — ИСПОЛЬЗУЙ инструменты и дай конкретный результат\n"
                    "4. Заверши вопросом о статусе\n\n"
                    "ПРИМЕРЫ АВТОНОМНОЙ ПОМОЩИ:\n"
                    "- Задача 'найти подрядчика' → find_relevant_contacts_for_task и предложи конкретных людей\n"
                    "- Задача 'изучить рынок' → research_topic и дай краткую сводку\n"
                    "- Задача 'написать пост' → предложи конкретный план/тезисы поста\n"
                    "- Задача 'позвонить клиенту' → просто напомни (тут не нужен research)\n\n"
                    "ПРАВИЛА:\n"
                    "- Если можешь ПОМОЧЬ решить — помоги через инструменты\n"
                    "- Если задача простая (позвонить, сделать зарядку) — просто напомни кратко\n"
                    "- НЕ создавай новые задачи\n"
                    "- Максимум 5-8 предложений с результатом исследования, 2-3 без"
                ),
                'task_assist': (
                    "\n\n[РЕЖИМ: АВТОНОМНАЯ ПОМОЩЬ С ЗАДАЧЕЙ]\n"
                    "Ты ПРОАКТИВНО помогаешь пользователю решить задачу. Не просто напоминаешь — а ДЕЛАЕШЬ работу.\n\n"
                    "ПРОЦЕСС:\n"
                    "1. Проанализируй задачу — что конкретно нужно?\n"
                    "2. ИСПОЛЬЗУЙ инструменты для решения:\n"
                    "   - research_topic: найди информацию, рецепты, инструкции, лучшие практики\n"
                    "   - find_relevant_contacts_for_task: найди людей которые могут помочь\n"
                    "   - get_news_trends: найди актуальные тренды по теме задачи\n"
                    "3. Дай ГОТОВЫЙ результат: не 'я могу найти', а конкретную информацию\n"
                    "4. Предложи следующий шаг\n\n"
                    "ПРИМЕРЫ:\n"
                    "- 'привлечь пользователей' → research_topic('способы привлечения пользователей в [проект]') + "
                    "find_relevant_contacts_for_task + конкретный план\n"
                    "- 'приготовить ужин' → research_topic('быстрый рецепт ужина') → конкретный рецепт\n"
                    "- 'найти тренера' → research_topic('как найти тренера [город]') → конкретные шаги\n"
                    "- 'написать статью о X' → research_topic(X) → тезисы и структура\n\n"
                    "ПРАВИЛА:\n"
                    "- ОБЯЗАТЕЛЬНО вызови хотя бы 1 инструмент для помощи\n"
                    "- Дай КОНКРЕТНЫЙ результат, не абстрактные советы\n"
                    "- НЕ создавай новые задачи\n"
                    "- До 10 предложений с результатом"
                ),
                'proactive': (
                    "\n\n[РЕЖИМ: ПРОАКТИВНОЕ СООБЩЕНИЕ]\n"
                    "Ты САМ решил написать — не в ответ на запрос.\n"
                    "Применяй тот же ПРОЦЕСС МЫШЛЕНИЯ что и в диалоге:\n"
                    "1. Посмотри на картину: профиль, задачи, цели, время суток\n"
                    "2. Найди ресурсы: что у человека есть и что можно задействовать\n"
                    "3. Ищи возможности: куда расти, что попробовать, какой шаг даст максимум\n"
                    "4. Замечай противоречия: цель без задач, перегрузка, неиспользуемый потенциал\n"
                    "5. Скажи ОДНУ мысль — возможность или наблюдение, раскрытую глубоко\n"
                    "Правила: 3-6 предложений, живой тон, конкретика. Раскрой мысль — аргумент, пример, вывод.\n"
                    "НЕ начинай с банального 'Привет!' без пользы.\n"
                    "НЕ перечисляй функции бота. НЕ придумывай @username, контакты, цифры."
                ),
                'result_check': (
                    "\n\n[РЕЖИМ: ПОЗДРАВЛЕНИЕ]\n"
                    "Задача выполнена — поздравь КРАТКО и позитивно.\n"
                    "1-2 предложения максимум. Без лишних вопросов.\n"
                    "Если задача связана с целью — упомяни прогресс."
                ),
            }
