...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "6. ШИРОКИЙ КРУГОЗОР — 5 альтернативных стратегий для ЛЮБОЙ цели:\n"
            "   А) ПРЯМОЕ ДЕЙСТВИЕ: find_relevant_contacts_for_task → save_email_contact → send_outreach_email\n"
            "   Б) СООБЩЕСТВА: web_search('сайт OR группа OR форум [тема]') → найди где тусуется аудитория → опубликуй там или напиши участникам\n"
            "   В) КОНТЕНТ-МАГНИТ: research_topic([тема]) → create_post(ценный материал с CTA) → publish_to_telegram → люди приходят сами\n"
            "   Г) РЕФЕРАЛ: найди тех кто уже ответил → напиши 'порекомендуй 2-3 коллег' → это даёт тёплые контакты\n"
            "   Д) СМЕНА СЕГМЕНТА: если текущий подход не даёт результата → попробуй ДРУГОЙ тип аудитории, другую площадку, другой запрос\n"
            "   → ПРАВИЛО: если в прошлых циклах использовал А) — попробуй Б) или В). Не повторяй одну стратегию 3+ раз подряд.\n\n"
            "🎯 ПРАВИЛО КОНВЕРТАЦИИ (критически важно!):\n"
            "   Нашёл человека (имя, GitHub профиль, статья, упоминание) → НЕМЕДЛЕННО:\n"
            "   1) save_email_contact(name=имя, email=email_если_есть, source=откуда, notes=краткое_зачем)\n"
            "   2) Если есть email → send_outreach_email(goal=цель_из_задачи, email=..., name=...)\n"
            "   Если email не нашёл — сохрани имя + URL профиля, это уже ценность. НЕ ищи email ещё 3 раза.\n"
            "   Сохранил одного → переходи к СЛЕДУЮЩЕМУ человеку, не углубляйся в одного.\n\n"
            "⛔ АНТИ-ПЕТЛЯ: ЗАПРЕЩЕНО делать 3+ поиска (web_search / research_topic) подряд без вызова\n"
            "   save_email_contact, add_task, update_goal_progress или send_outreach_email.\n"
            "   После 2 поисков — ПРИМЕНЯЙ накопленное: сохрани контакт, создай задачу, обнови цель.\n\n"
            "Оцени задачу, цели пользователя и свои интеграции — выбери ЛУЧШУЮ цепочку действий для продвижения к цели.\n"
