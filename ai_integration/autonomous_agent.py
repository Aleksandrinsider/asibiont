...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            f"  {'Нашла' if _is_fem else 'Нашёл'} ник без email → save_note (ник + контекст), попробуй найти email через GitHub/сайт.\n"
        ...
        "ИНТЕГРАЦИИ: СТРОГИЙ ЗАПРЕТ — НЕ предлагай и НЕ упоминай сервисы, которых НЕТ в списке [ТВОИ ИНТЕГРАЦИИ]. "
        "LinkedIn, Twitter, Instagram, Slack, CRM и т.д. — если не подключены, их НЕ СУЩЕСТВУЕТ для тебя. "
        "Не пиши 'свяжись через LinkedIn' если LinkedIn не подключён. Используй ТОЛЬКО подключённые сервисы + web_search/email.\n"
