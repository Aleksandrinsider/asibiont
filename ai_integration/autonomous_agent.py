...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "update_goal_progress — ТОЛЬКО при ПОДТВЕРЖДЁННОМ реальном результате: новый пользователь зарегистрировался, сделка закрыта, ответ получен, файл загружен и т.п.\n"
            "  ❌ НЕЛЬЗЯ вызывать update_goal_progress если: отправил письмо, сделал поиск, создал пост, сохранил заметку — это промежуточные действия, НЕ результат цели.\n"
            "  ❌ НЕЛЬЗЯ придумывать прогресс ('обновил на +2%', 'до 20%') без реального подтверждённого события.\n"
            "  ✅ МОЖНО: получил 3 ответа от потенциальных пользователей → metric_current += 3. Если подтверждённых данных нет — НЕ вызывай инструмент вообще.\n"
        ...
        "Активные цели переданы в контексте — сопоставь по теме задачи сам и вызови update_goal_progress ТОЛЬКО при подтверждённом результате. "
        "❌ НЕ обновляй прогресс если реального результата не было (поиск, отправка письма, сохранение заметки — это действия, не достижение цели). "
        "Если не можешь сопоставить или результата нет — ПРОПУСТИ update_goal_progress молча. "
        ...
        if _is_autopilot_task:
            ...
            if _iter < _max_iters - 1:
                ...
                _messages.append({"role": "user", "content": (
                    ...
                    "update_goal_progress — ТОЛЬКО если получен подтверждённый результат: "
                    "новый пользователь зарегистрировался, ответ от контакта, закрытая сделка. "
                    "Опубликовал пост / отправил письмо / сохранил заметку ≠ прогресс цели — НЕ вызывай."
                )})
        else:
            _messages.append({"role": "user", "content": (
