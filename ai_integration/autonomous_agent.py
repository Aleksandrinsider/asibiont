...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            f"  {'Нашла' if _is_fem else 'Нашёл'} ник/логин без email → run_agent_action search_github_users с этим логином (работает и без GitHub токена, возвращает email из commit-истории/профиля).\n"
            "  Если GitHub вернул пустой результат → web_search «имя + сайт/блог» → найди страницу с контактом. Не ищи «логин email contact» — это всегда 0.\n"
            ...
            "        Используй run_agent_action search_github_users с его именем/логином, сохрани email через save_email_contact.\n\n"
