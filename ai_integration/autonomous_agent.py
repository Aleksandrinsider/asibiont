...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        system_prompt += (
            "\n\n⚡ АВТОПИЛОТ ЦЕЛЕЙ:\n"
            "Твоя задача — продвинуть цель на один шаг. "
            "Выше перечислены твои инструменты — выбери подходящий и действуй.\n"
            "Каждый цикл = одно новое конкретное действие (не повтор, не пересказ).\n"
