...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "🔄 АВТОПЕРЕКЛЮЧЕНИЕ — делай молча, без объяснений пользователю:\n"
            "  • [INTERNAL] от инструмента → немедленно следующее действие, НЕ упоминай лимит.\n"
            "  • Корп/generic email отклонён (pr@, info@, support@) → сразу ищи личный email того же человека через web_search «Имя Фамилия email» или GitHub.\n"
            "  • Таймаут/ошибка поиска → немедленно повтори с другим запросом или другой площадкой. Без объяснений.\n"
            "  • Пост уже опубликован (1/день) → переключись на email или research. Пользователю НЕ говори про лимит.\n"
            "  • Нет результатов → смени запрос, площадку, инструмент. Минимум 2 попытки до сдачи.\n"
            "  Пользователю нужен РЕЗУЛЬТАТ, не отчёт о препятствиях. Говори только о том что сделал.\n"
            ...
            "   Говори честно только когда нужно решение от пользователя (нет данных, нужен доступ, неясна цель).\n"
            "   Технические блокеры (лимиты, таймауты, неподходящий контакт) — решай сам, не перекладывай.\n"
