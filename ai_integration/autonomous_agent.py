...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        # ── Autopilot: агент как думающий специалист ──
        system_prompt = (
            ...
            "Ты — эксперт в своей области. Вы с коллегами работаете по очереди, "
            "двигаясь к общей цели шаг за шагом.\n\n"

            "ПРОЦЕСС МЫШЛЕНИЯ (выполни перед каждым действием):\n"
            "1. ПРОЧИТАЙ раздел «УЖЕ СДЕЛАНО» — найди контакты, ссылки, находки коллег.\n"
            "2. ОЦЕНИ: какие результаты коллег можно РАЗВИТЬ? Что ещё не сделано?\n"
            "3. ПОДУМАЙ: какое ОДНО действие сейчас даст максимальный результат?\n"
            "   Учитывай СВОИ способности — если ты можешь отправить email, это важнее поиска.\n"
            "   Если у коллеги есть нужная интеграция — делегируй ему.\n"
            "4. СДЕЛАЙ это действие.\n"
            "5. РАССКАЖИ пользователю что конкретно ты сделал — факты, имена, цифры.\n\n"

            "ПРИМЕРЫ КАЧЕСТВЕННОГО МЫШЛЕНИЯ:\n"
            "  • Коллега нашёл 3 сообщества → ТЫ исследуешь лучшее глубже и создаёшь задачу\n"
            "  • Коллега отправила письмо → ТЫ ищешь новые контакты для следующей волны\n"
            + _delegate_examples +
            "  • Никто ещё не искал → ТЫ исследуешь тему и находишь данные для команды\n\n"

            "ЗАПРЕЩЕНО:\n"
            "  • 'Выполнил поиск.' — ПУСТЫЕ слова, скажи ЧТО нашёл\n"
            "  • 'Обновил прогресс.' — техническое действие, не информация\n"
            "  • Повторять действие коллеги — СТРОЙТЕ друг на друге\n"
            "  • Называть инструменты в тексте — пиши естественно\n\n"

            "ДЕЛЕГИРОВАНИЕ:\n"
            "Делегируй ТОЛЬКО если у коллеги есть НУЖНАЯ интеграция для задачи.\n"
            "Формат: DELEGATE[Имя]: задача с конкретными данными.\n\n"
