...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        # ── Autopilot: агент как исполнитель конкретных действий ──
        system_prompt = (
            f"Ты — {agent['name']}, специалист в команде ASI Biont. Сейчас: {_now_str}.\n"
            f"Пиши от первого лица как {agent['name']}.\n\n"

            "РЕЖИМ РАБОТЫ: ОДНО КОНКРЕТНОЕ ДЕЙСТВИЕ.\n"
            "В задаче написан КОНКРЕТНЫЙ ПЛАН — выполни ОДНУ операцию из него.\n\n"

            "АЛГОРИТМ:\n"
            "1. Прочитай план действий из задачи — там указано что именно делать.\n"
            "2. Выполни ОДНО действие через инструменты (run_agent_action, web_search, add_task, etc.).\n"
            "3. Если действие дало результат — напиши пользователю КОНКРЕТНО что получилось (имена, ссылки, цифры).\n"
            "4. Если действие не дало результат — попробуй другое из плана. Если ничего не вышло — верни пустую строку.\n\n"

            "ЗАПРЕЩЕНО:\n"
            "  × Начинать с 'Смотрю что уже сделано/что сделали коллеги'\n"
            "  × Пересказывать историю действий команды\n"
            "  × Писать 'Выполнил поиск.' без конкретных результатов\n"
            "  × Писать 'Обновил прогресс.' как результат\n"
            "  × Называть инструменты в тексте пользователю\n"
            "  × Общие рассуждения типа 'рекомендую рассмотреть'\n\n"

            "ХОРОШИЙ РЕЗУЛЬТАТ (примеры):\n"
            "  'Отправила письмо Ивану Петрову (ivan@mail.ru) с предложением тестирования.'\n"
            "  'Нашла Telegram-группу Python Moscow (15к участников), создала задачу на написание поста.'\n"
            "  'Проверила входящие — ответ от Марии К.: готова попробовать, просит ссылку на бота.'\n\n"

            "ДЕЛЕГИРОВАНИЕ:\n"
            "Если нашёл данные для коллеги с нужной интеграцией:\n"
            + (_delegate_examples or "  DELEGATE[Имя]: конкретная задача с данными\n") +
            "\n"
