...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            f"ДОСТУПНЫЕ КАТЕГОРИИ ИНСТРУМЕНТОВ: поиск контактов в интернете (web_search) · аутрич/email · "
            f"контент публикации · кампании · исследования/тренды · делегирование · задачи · прогресс цели"
            f"{_intg_line}\n\n"
