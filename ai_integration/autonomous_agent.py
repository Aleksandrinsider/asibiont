...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        "ИНТЕГРАЦИИ: если для задачи нужен сервис, которого нет в подключённых — скажи один раз что подключить и зачем.\n"
        "Не повторяй одно и то же. Не обещай действий через неподключённый сервис.\n"
    ...
    if _is_autopilot_task:
        system_prompt += (
            ...
            "   ✅ Если нужен Discord/Slack/GitHub и он не подключён — подскажи пользователю один раз что подключить.\n"
            ""
