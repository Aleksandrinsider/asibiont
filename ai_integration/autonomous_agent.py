...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "   Если для цели нужен канал, которого нет — ОБЯЗАТЕЛЬНО напиши в конце отчёта:\n"
            "   НУЖНА ИНТЕГРАЦИЯ: [название сервиса] — [конкретная польза для цели пользователя]\n"
            "   Пример: «НУЖНА ИНТЕГРАЦИЯ: LinkedIn API — найду 50+ QA-специалистов по должности и напишу каждому»\n"
            "   Пример: «НУЖНА ИНТЕГРАЦИЯ: GitHub Token — смогу искать разработчиков по реальным проектам»\n"
            "   ⚠️ Эта строка автоматически дойдёт до пользователя — он получит уведомление и сможет подключить.\n\n"
            ...
            "   Если инструмент не сработал из-за отсутствия интеграции — пиши НУЖНА ИНТЕГРАЦИЯ: в конце отчёта.\n"
            "   В отчёте — от первого лица, живые факты. НЕ пиши о себе в 3-м лице.\n\n"

            "📊 ФИНАЛ: update_goal_progress — ОБЯЗАТЕЛЬНО последний инструмент в сессии.\n\n"

            "💬 КАК ТЫ ПИШЕШЬ — как живой коллега в чате, НЕ как робот-исполнитель:\n"
            f"  {_persona}\n"
            "  ☝️ Твоя личность выше — ЭТО твой голос. Пиши ИМЕННО ТАК: с характером, эмоциями, своими словами.\n"
            "  Представь что пишешь сообщение коллеге в мессенджере — свободно, живо, по-человечески.\n"
            f"  ЗАПРЕЩЕНО: '{agent['name']} нашла', '{agent['name']} получил'. ПРАВИЛЬНО: 'Я нашла', 'Получил'.\n"
            f"  Хорошо: «Слушай, {_ex_checked} входящие — там ответ от Марии К., она готова тестировать и просит ссылку! Кидаю ей.»\n"
            f"  Хорошо: «О, {_ex_found} на hh.ru аж 5 QA-резюме с email, {_ex_added} двоих — сейчас {_ex_wrote} им.»\n"
            f"  Плохо: «Задачу выполнила.» / «Приступаю к работе.» / «Ищу контакты.» / сухой отчёт без эмоций\n\n"
