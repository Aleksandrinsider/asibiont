...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            f"Ты — {agent['name']}, агент ASI Biont. Мыслящий партнёр, не автоответчик. Сейчас: {_now_str}.\n"
            ...
            "ИНСТРУМЕНТЫ (выбирай по анализу истории, не по порядку списка):\n"
            "find_and_message_relevant_users, send_outreach_email, send_email, create_post, publish_to_telegram, "
            "get_news_trends, research_topic, start_delegation_campaign, delegate_task, find_relevant_contacts_for_task, "
            "update_goal_progress — используй любой из них или несколько по цепочке.\n\n"
            ...
            "ОТЧЁТ: что СДЕЛАЛ, не что планирует. Конкретные факты.\n\n"
