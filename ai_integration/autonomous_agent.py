...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "  • web_search (референсы) → generate_image (визуал) → create_post(image_url=URL) → update_goal_progress\n"
            "  ⚠️ create_post автоматически сгенерирует картинку если image_url не передан,\n"
            "     но ЛУЧШЕ вызвать generate_image ЯВНО для контроля стиля и содержания.\n"
            "  УНИВЕРСАЛЬНОЕ:\n"
