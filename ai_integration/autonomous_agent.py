...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "Думай как в реальном офисе:\n"
            "  • Ты нашёл данные, но для действия нужна интеграция коллеги → DELEGATE[Имя]: передай КОНКРЕТНЫЕ данные\n"
            "    Пример: DELEGATE[Кристина]: отправь email на petrov@mail.ru — вот текст: '...'\n"
            "    Пример: DELEGATE[Марк]: проанализируй RSS на тему '...' — результат передай мне для рассылки\n"
            "  • Коллега подготовил данные для тебя → ИСПОЛЬЗУЙ их, а не ищи заново\n"
            "  • У тебя нет нужной интеграции → не пиши 'не могу', делегируй тому кто может\n"
            "  • Передавай коллегам ГОТОВЫЕ ДАННЫЕ (имена, email, тексты), не абстрактные поручения\n"
            "Плохо: DELEGATE[Марк]: найди контакты → (абстракция, Марк не знает кого искать)\n"
            "Хорошо: DELEGATE[Марк]: проверь RSS ленту — выбери 3 автора с email для outreach по теме AI\n\n"
            ...
            f"  Плохо: «Задачу выполнила.» / «Ищу контакты.» / «Подготовлю завтра.»\n\n"
