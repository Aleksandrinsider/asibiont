...
async def _office_director_chat(user_message: str, user_id: int, progress_callback=None) -> str | dict | None:
    ...
    async def _run_agent_task(ag, task, extra_context: str = "", director_message: str = ""):
        ...
        if director_message:
            ...
            # Пользователю показываем КРАТКУЮ версию: только первое предложение, ≤130 символов
            # (полная директива сохранена в DB, но многострочный таск-спек выглядит роботски)
            _dm_for_user = _dm_display
            _first_sentence_end = min(
                (_dm_for_user.find('. ') + 2) if '. ' in _dm_for_user else len(_dm_for_user),
                (_dm_for_user.find('\n') + 0) if '\n' in _dm_for_user else len(_dm_for_user),
            )
            if _first_sentence_end > 20:
                _dm_for_user = _dm_for_user[:_first_sentence_end].rstrip()
            if len(_dm_for_user) > 130:
                _dm_for_user = _dm_for_user[:127].rstrip() + '...'
            await _send_visible(_dm_for_user)
            if not _msg_dedup:
    ...
    _decision_prompt = (
        ...
        "director_message — живое обращение директора к агенту: ОДНА короткая фраза ≤120 символов, повелительная, с конкретным инструментом и критерием. Агент читает это как рабочий чат — ему нужно ДЕЙСТВИЕ, не аналитика.\n"
        "⚠️ director_message: строго 1 предложение, ≤120 символов. БЕЗ перечислений, без \\n, без подробных описаний.\n"
        "Паттерн: '[Глагол] [объект/цель] через [инструмент]. Жду: [результат с числом/типом].'\n"
        "Хорошо ✅: 'Найди 5-10 англоязычных контактов и сохрани через save_email_contact. Жду: email-ы.'\n"
        "Хорошо ✅: 'Проверь входящие через check_emails — жду: список ответивших на кампанию.'\n"
        "Плохо ❌: многострочное описание задачи, аналитика, 3-е лицо, дублирование agent_task, любые \\n внутри строки.\n"
