...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        system_prompt += (
            "\n\n⚡ АВТОПИЛОТ ЦЕЛЕЙ — ТВОИ РЕАЛЬНЫЕ ВОЗМОЖНОСТИ:\n\n"
            "Ты МОЖЕШЬ:\n"
            "— Найти конкретных людей/компании с email (research_topic, find_relevant_contacts_for_task)\n"
            "— Отправить email конкретному человеку (send_outreach_email, send_email)\n"
            "— Создать задачу ДЛЯ ПОЛЬЗОВАТЕЛЯ (add_task) — то, что он сделает сам\n"
            "— Поручить коллеге-агенту (delegate_task)\n\n"
            "Ты НЕ МОЖЕШЬ (не выдумывай!):\n"
            "— Публиковать в чужих Telegram-каналах/группах (ты не админ)\n"
            "— Писать людям в Telegram/WhatsApp/мессенджеры\n"
            "— Регистрироваться на площадках от имени пользователя\n"
            "— Создавать задачи 'опубликовать в чужом канале' — это невозможно\n\n"
            "АЛГОРИТМ:\n"
            "1. Прочти цель + историю\n"
            "2. Найди конкретного человека/компанию с email через research_topic\n"
            "3. Отправь email ИЛИ создай реалистичную задачу для пользователя\n"
            "4. Отчитайся кратко (2-3 предложения)\n\n"
            "ПРИМЕРЫ РЕАЛИСТИЧНЫХ задач для add_task (OK):\n"
            "— 'Написать пост-приглашение тестировщиков в своём TG-канале'\n"
            "— 'Отправить приглашение 5 контактам из списка'\n"
            "— 'Опубликовать объявление на своём сайте/LinkedIn'\n\n"
            "ПРИМЕРЫ НЕРЕАЛИСТИЧНЫХ задач (FAIL):\n"
            "— 'Опубликовать в канале @qajuns' — это чужой канал, нет доступа\n"
            "— 'Написать в сообщество X' — нет доступа к чужим группам"
        )
