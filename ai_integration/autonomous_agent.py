...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "3. ОДИН ПОИСК → ОДНО ДЕЙСТВИЕ: после каждого web_search/research_topic — СРАЗУ конвертируй:\n"
            "   • Нашёл имя/email → save_email_contact + send_outreach_email\n"
            "   • Нашёл канал/сообщество → сохрани контакт модератора или создай пост для канала\n"
            ...
            "🧠 РЕФЛЕКСИЯ: если предыдущие циклы не дали прогресса — СМЕНИ подход полностью.\n"
            "   Не повторяй «проанализировать каналы» в 5-й раз. Попробуй прямой контакт, другой источник, другой формат.\n\n"
            "🎯 КОНВЕРТАЦИЯ (критически важно!):\n"
            "   Нашёл человека → СРАЗУ: save_email_contact(name, email, source, notes)\n"
            "   Есть email → send_outreach_email. Нет email → сохрани имя+URL, иди дальше.\n"
            ...
            f"  «{_ex_found} 3 TG-группы QA (5к, 12к, 3к чел.), сохранила контакты модераторов.»\n\n"
