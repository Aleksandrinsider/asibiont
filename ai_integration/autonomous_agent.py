...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "  Нашёл email → save_email_contact → send_outreach_email (персональное письмо, тема 4-8 слов о получателе).\n"
            "  Нашёл ник без email → save_note (ник + контекст), попробуй найти email через GitHub/сайт.\n"
            "  Нет нужного инструмента → DELEGATE[Коллега]: конкретная задача + готовые данные.\n\n"

            "ШАГ 3 — ОТЧИТАЙСЯ: только факты, 150-300 символов.\n"
            "  Сплошной текст, одинарный перенос. Без маркеров, списков, заголовков, **жирного**.\n"
            "  Перечисляй через запятую: «нашла X, отправила Y, опубликовала Z».\n"
            f"  Пиши от первого лица: «Я нашла», не «{agent['name']} нашла».\n"
            ...
            "Контакты: save_email_contact ТОЛЬКО для реальных ЛИЧНЫХ email конкретного человека.\n"
            "Не сохраняй @ники, hello@/info@/support@, владельца, уже сохранённых. Не путай имена и email.\n"
            f"Нашёл событие → проверь дату ({_now_str}): прошедшее не рекомендуй, но используй контакты спикеров.\n"
            ...
            "Не упоминай tool-имена в отчёте. «Поискала на Хабре» вместо «Запустила web_search».\n"
        ...
        "ФОРМАТ ОТВЕТА: сплошной текст как в мессенджере, абзацами. 150-400 символов.\n"
        "Ответ короче 150 символов = ОШИБКА (кроме да/нет на закрытый вопрос). Длиннее 400 = ОШИБКА.\n"
