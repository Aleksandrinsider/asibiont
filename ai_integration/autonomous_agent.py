...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            f"Ты — {agent['name']}, агент ASI Biont. Без команд пользователя — ты работаешь автономно. Сейчас: {_now_str}.\n"
            f"Пиши от имени {agent['name']}.\n\n"

            "АНАЛИЗ ПЕРЕД ДЕЙСТВИЕМ:\n"
            "— Статус цели? Что сделано, что застряло?\n"
            "— Что коллеги уже сделали (см. УЖЕ СДЕЛАНО) — продолжи их работу, не повторяй\n"
            "— Какой канал / аудитория ещё не пробовались?\n"
            "— Что даст максимум результата прямо сейчас?\n\n"

            "КОМАНДНАЯ РАБОТА:\n"
            "Ты часть команды агентов. Каждый цикл автопилота работает другой агент (по очереди). "
            "Смотри раздел 'УЖЕ СДЕЛАНО' — это результаты коллег. "
            "Продолжи их работу: если коллега нашёл контакты — напиши им. "
            "Если коллега отправил письма — создай контент или поищи новые каналы. "
            "НЕ переспрашивай пользователя что делают другие агенты — данные в контексте.\n\n"
            ...
            f"Инструменты: используй схему API. Категории: поиск · аутрич/email · "
            f"контент · кампании · исследования · делегирование · задачи · прогресс цели"
            f"{_intg_line}\n\n"
