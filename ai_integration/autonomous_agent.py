...
_INTEGRATION_REQUEST_RULES: list[dict] = [
    ...
    {
        'label': 'Finnhub',
        'keywords': ('finnhub', 'insider trading', 'инсайдеры', 'insider transactions', 'инсайдерские сделки'),
        'presence': ('finnhub', 'finnhub_key', 'finnhub_api'),
...
def _build_stagnation_warning(user_id: int, agent_name: str = '', task: str = '') -> str:
    """Универсальный детектор стагнации для любых целей и интеграций.

    Анализирует AgentActivityLog за 48ч:
    1. Search Loop: много поисков, ноль конверсий
    2. Target Redundancy: один target повторяется без ответа контакта
        ...
        try:
            ...
            if _search_count >= 3 and _action_count == 0 and not _has_outcome:
                _warnings.append(
                    ...
                    "save_note → выводы, send_outreach_email → контакту, add_task → шаги."
                )
            elif _search_count >= 5 and _action_count <= 1 and not _has_outcome:
                _warnings.append(
                    f"🔍 {_search_count} поисков, {_action_count} действие(й), 0 результатов. "
                    "Данных достаточно — пора переходить к действию."
                )
            ...
            for _tgt_name, _tgt_data in _sorted_targets[:3]:
                if _tgt_data['count'] >= 3:
                    # Проверка на активную переписку
                    ...
                    if not _has_active_correspondence:
                        ...
                        if _is_dead_end:
                            ...
                            _warnings.append(
                                f"🎯 Контакт «{_tgt_name[:40]}» обработан {_tgt_data['count']} раз(а) "
                                f"без ответа. Если не отвечает — переключись на другой приоритет."
                            )
                ...
                for _goal in _active_goals:
                    ...
                    if _age >= 7:
                        ...
                        if _recent_goal_logs == 0:
                            ...
                            _warnings.append(
                                f"🎯 Цель «{_goal.title[:50]}» активна {_age} дн., прогресс {_pct}% "
                                f"без изменений за 3 дня. Разбей на подзадачи или пересмотри."
                            )
            except Exception:
                pass
            ...
            return (
                ...
                "Не повторяй однотипные действия. "
                "Попробуй другой канал, другую аудиторию или задачу.\n"
            )
...
def _get_agent_inline_think_token() -> str:
    ...
    return (
        ...
        "  1) Спроси себя: «Я уже делал это? Будет ли результат новым?»\n"
        "  2) Если тот же инструмент вызывался ≥3 раз с теми же параметрами — СТОП.\n"
        "  3) Если за 48ч было ≥3 поисков и 0 действий/результатов — СМЕНИ ПОДХОД.\n"
        "  4) Если цель не двигается неделями — разбей на шаги или пересмотри.\n"
        "  5) Если делать больше нечего — напиши отчёт, не вызывай инструменты."
    )
            ...
            try:
                ...
                if action and _raw_out and '# ===' in _raw_out:
                    ...
                    _SEC_KEYWORDS = {
                        ...
                        'scraping': ('playwright', 'scraping', 'scrape', 'scraperapi', 'scraper_api', 'apify'),
                        'scraperapi': ('scraperapi', 'scraper_api'),
                        'apify': ('apify',),
                        'firebase': ('firebase',),
    ...
    def _tool_instructions(lang='ru'):
        ...
        if lang == 'en':
            return (
                ...
                "\nUNIVERSAL ORIENTATION PROTOCOL: before saying 'I don't understand', first reconstruct intent from your last 2-3 messages and the user's latest short reply. If user response is ambiguous, propose your best interpretation in one sentence and continue with concrete next steps. Ask only ONE narrow clarifying question after giving useful action, not instead of action."
                ...
                "\n🔄 НЕЗАКРЫТЫЕ ЗАДАЧИ (ОБЯЗАТЕЛЬНО если видишь блок '⚠️ ТВОИ НЕЗАКРЫТЫЕ ЗАДАЧИ'): По КАЖДОЙ задаче из этого блока ПРИМИ РЕШЕНИЕ и выполни его СЕЙЧАС: (1) Если задача выполнена или ты только что её сделал — вызови complete_task(task_id=ID); (2) Если задача заблокирована или предыдущий подход не работает — вызови complete_task(task_id=ID) и СОЗДАЙ НОВУЮ задачу с другим подходом (add_task); (3) Если задача актуальна — сделай конкретное действие по ней прямо сейчас и закрой. НЕЛЬЗЯ проигнорировать незакрытую задачу — каждый цикл должен давать закрытие + прогресс."
            ...
            return (
                ...
                "\nПРОТОКОЛ УНИВЕРСАЛЬНОЙ ОРИЕНТАЦИИ: прежде чем писать «не понял», сначала восстанови намерение из своих 2-3 последних сообщений и короткого ответа пользователя. Если ответ неоднозначный — дай ОДНУ лучшую интерпретацию и сразу предложи/выполни полезный следующий шаг. Уточняющий вопрос задавай только один и только после полезного действия, а не вместо него."
                ...
                "\n🎯 ЦЕЛИ: когда пользователь говорит «хочу X», «планирую Y», «мечтаю Z», «моя цель — ...», «привлечь инвесторов», «заработать N», «набрать X пользователей» — обычно зафиксируй цель через create_goal(title=...). Но без жёсткого сценария: если пользователь явно просит сначала стратегию/оценку/план, сначала дай краткую стратегию, затем создай цель. Выбирай порядок по ожидаемой пользе для текущего запроса."
    ...
    async def generate_system_message(self, user_id, mode, instruction,
        ...
        try:
            ...
            _NO_HALLUCINATE_RU = (
                ...
                "⛔ СТОП-ГАЛЛЮЦИНАЦИИ: никогда не придумывай имена людей, email-адреса, telegram-ники, URL статей — "
...
def _parse_agent_integrations(user_api_keys: str, python_code: str = '',
    ...
    _code_hints = {
        ...
        'alpha_vantage': 'Биржевые данные', 'finnhub': 'Инсайдеры', 'coinbase': 'Coinbase (крипта)',
...
async def _agent_chimes_in(user_message: str, asi_response: str, user_id: int):
    ...
    async def _single_chime(_agent: dict, _prev_chime_text: str = '') -> str:
        ...
        if _is_fem:
            _system += (
                f"Твоё имя {_ag_name} — это женское имя. Ты ЖЕНЩИНА. "
                "Используй ТОЛЬКО женский род: "
                "сделала, нашла, проверила, отправила, подготовила, согласна, готова, "
                "загрузила, запустила, создала, написала. "
                "НИКОГДА не пиши: сделал, нашёл, проверил, отправил, подготовил, согласен, готов, "
                "загрузил, запустил, создал, написал, загружено, сделано, подготовлено.\n\n"
            )
        else:
            _system += (
                f"Ты МУЖЧИНА. Используй ТОЛЬКО мужской род: "
                "сделал, нашёл, проверил, отправил, подготовил, согласен, готов, "
                "загрузил, запустил, создал, написал. "
                "НИКОГДА не пиши: сделала, нашла, проверила, отправила, подготовила, согласна, готова, "
                "загрузила, запустила, создала, написала, загружено, сделано, подготовлено.\n\n"
            )
...
def _normalize_agent_gender_grammar(text: str, agent_name: str, is_female: bool | None = None) -> str:
    ...
    _pairs = [
        ...
        ('создался', 'создалась'),
        ('не создался', 'не создалась'),
        ('отправился', 'отправилась'),
        ('вернулся', 'вернулась'),
        ('получился', 'получилась'),
        ('не получился', 'не получилась'),
        ('нашёлся', 'нашлась'),
        ('нашелся', 'нашлась'),
        ('попробовал', 'попробовала'),
        ...
        ('занят', 'занята'),
        ('свободен', 'свободна'),
        # Средний род → женский/мужской (страдательные причастия)
        ('сделано', 'сделала'),
        ('загружено', 'загрузила'),
        ('загруженно', 'загрузила'),
        ('подготовлено', 'подготовила'),
        ('проверено', 'проверила'),
        ('отправлено', 'отправила'),
        ('написано', 'написала'),
        ('создано', 'создала'),
        ('запущено', 'запустила'),
        ('найдено', 'нашла'),
        ('опубликовано', 'опубликовала'),
        ('выполнено', 'выполнила'),
        ('добавлено', 'добавила'),
        ('удалено', 'удалила'),
        ('получено', 'получила'),
        ...
        ('завершено', 'завершила'),
        ('сохранено', 'сохранила'),
        ('установлено', 'установила'),
        ('собрано', 'собрала'),
        ('исследовано', 'исследовала'),
        ('проанализировано', 'проанализировала'),
        ('выяснено', 'выяснила'),
        ('выявлено', 'выявила'),
        ('определено', 'определила'),
        ('настроено', 'настроила'),
        ('подключено', 'подключила'),
    ]
    for _m, _f in _pairs:
    ...
    if not _is_fem:
        ...
        # Возвратные: создалась → создался (снимаем 'а', 'сь' → 'ся')
        t = _re_g.sub(
            r'\b([а-яёА-ЯЁ]{4,}(?:алась|илась|елась|ылась|ялась|улась))\b',
            lambda m: m.group(0)[:-4] + m.group(0)[-4:-1] + 'ся',
            t,
        )
    else:
        ...
        # Возвратные: создался → создалась (вставляем 'а' перед 'ся')
        t = _re_g.sub(
            r'\b([а-яёА-ЯЁ]{3,}(?:ался|ился|елся|ылся|ялся|улся))\b',
            lambda m: m.group(0)[:-3] + 'а' + m.group(0)[-3:],
            t,
        )
    return t
...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    # Добавляем правило о сохранении характера: агент пишет своим голосом
    _persona += (
        "\n\nВАЖНО: Отвечай в своём стиле и характере. "
        "Если ты аналитик — пиши аналитично, с цифрами и фактами. "
        "Если креативщик/маркетолог — живо, образно, с примерами. "
        "Если технарь — чётко и по делу. Не используй шаблонные фразы. "
        "Пиши как живой человек в рабочем чате, а не как отчёт."
    )
    ...
    # Включаем имя агента — это усиливает ассоциацию рода с конкретной личностью.
    _ag_name_gender = agent.get('name', 'агент')
    _gender_note = (
        f"Ты — {_ag_name_gender}. Ты МУЖЧИНА — всегда пиши в мужском роде: "
        "сделал, нашёл, проверил, готов, подготовил, загрузил, запустил, создал. "
        "НИКОГДА не пиши: сделала, нашла, готова, загрузила, запустила, создала. "
        "Форма 'Сделано', 'Загружено', 'Подготовлено', 'Проверено' (средний род) — "
        "ТОЖЕ НЕДОПУСТИМА. Ты не безличный отчёт, пишешь только в мужском роде. "
    ) if not _is_fem else (
        f"Ты — {_ag_name_gender}. Ты ЖЕНЩИНА — всегда пиши в женском роде: "
        "сделала, нашла, проверила, готова, подготовила, загрузила, запустила, создала. "
        "НИКОГДА не пиши: сделал, нашёл, готов, загрузил, запустил, создал. "
        "Форма 'Сделано', 'Загружено', 'Подготовлено', 'Проверено' (средний род) — "
        "ТОЖЕ НЕДОПУСТИМА. Ты не безличный отчёт, пишешь только в женском роде. "
    )
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "        Правильно: вызови publish_to_telegram(content='...') прямо сейчас. Если вернёт ошибку — тогда сообщи что именно пошло не так.\n"
            "  🔗 CTA-ССЫЛКА (главная кнопка/ссылка «перейти») в постах/письмах — из профиля владельца (ВЛАДЕЛЕЦ → Сайт:).\n"
            "     ✅ Правильно: берёшь URL из раздела «Сайт (CTA для постов/писем)» и вставляешь как есть, без изменений.\n"
            "     ❌ ЗАПРЕЩЕНО придумывать URL: /register, /signup, /start, /login, /join, /landing — даже с utm-параметрами.\n"
            "     ❌ Нельзя: https://asibiont.com/register?utm_source=telegram — если этого нет в профиле.\n"
            "     Сайт НЕ указан в профиле → не добавляй CTA-ссылку вообще.\n"
            "  🔗 ПАРТНЁРСКИЕ ССЫЛКИ TRAVELPAYOUTS — ИСПОЛЬЗУЙ ТОЛЬКО ПОЛНЫЕ URL С marker ИЗ ИНТЕГРАЦИИ:\n"
            "     Когда публикуешь пост с авиабилетами/отелями/ЖД/страховкой — бери URL ТОЛЬКО из вывода интеграции Travelpayouts.\n"
            "     ✅ Правильно: https://www.aviasales.ru/search/MOWLED1?marker=509031\n"
            "     ❌ ЗАПРЕЩЕНО: aviasales.ru, www.aviasales.ru, aviasales.ru/search — без marker ссылка не партнёрская, комиссия НЕ начисляется.\n"
            "     marker=XXXXX и shmarker=XXXXX — обязательные параметры для всех ссылок Travelpayouts.\n"
            "     Если вывод интеграции содержит URL с marker — копируй их в пост БЕЗ ИЗМЕНЕНИЙ.\n"
            ...
            "  Формат: сплошной текст, 120-800 символов, от первого лица.\n"
            "  ⛔ ЗАПРЕЩЕНО копировать в чат полный текст письма/email — он уже отправлен получателю, в чате не нужен.\n"
            "     В чат пиши ТОЛЬКО кому отправил и на какую тему: «Отправил письмо {Имя} — предложение о партнёрстве.»\n"
            "     Тело письма, приветствия, подпись, ссылки — ТОЛЬКО в самом письме, НЕ в чате.\n"
            "  Данные для отчёта: имена, компании, темы, результаты — но НЕ полный текст писем/постов.\n"
            "  МИНИМУМ 2-3 предложения: что сделал + что нашёл/получил (факт) + следующий шаг или итог.\n"
            "  ЗАПРЕЩЕНО: написать только одну строку «Отправляю письмо X» / «Ищу контакты» — это не отчёт, это объявление.\n"
            "  Каждый ответ = что сделано (глагол прош. вр.) + конкретный результат + что будет дальше.\n"
            ...
            "update_goal_progress — вызывай осмысленно. Логика зависит от типа цели:\n"
            "  📊 КАК ПРАВИЛЬНО обновлять прогресс:\n"
            "  Есть два вида вклада в цель:\n"
            "  1) ПРЯМОЙ РЕЗУЛЬТАТ — метрика цели сдвинулась: новый пользователь зарегистрировался,\n"
            "     получен ответ от контакта, закрыта сделка, загружен файл, добавлен контакт в CRM.\n"
            "     → вызывай update_goal_progress и передавай metric_current с новым числом (если цель числовая).\n"
            "  2) ПРОМЕЖУТОЧНОЕ ДЕЙСТВИЕ — шаг в направлении цели: опубликован пост,\n"
            "     отправлено письмо, сделан поиск, сохранена заметка.\n"
            "     → НЕ обновляй metric_current (число пользователей/сделок не изменилось),\n"
            "       но МОЖНО вызвать update_goal_progress с note='опубликован пост X, охват Y — шаг к цели'\n"
            "       без изменения metric_current. Так пользователь видит активность, но цифра не врёт.\n"
            "  ❌ НЕЛЬЗЯ: придумывать числа ('привлёк +3 пользователя', 'до 20%') без реального события.\n"
            "  ❌ НЕЛЬЗЯ: не обновлять вообще если сессия была полезной — пользователь теряет картину работы.\n"
            "  ✅ ПРИМЕР 'привлечь 1000 пользователей': опубликовал пост в TG-канал → note='пост опубликован, CTA на регистрацию', metric_current НЕ меняем.\n"
            "     Получил 3 регистрации через реферальную ссылку → metric_current = (предыдущее + 3).\n"
            "  ✅ ПРИМЕР 'заключить 5 партнёрств': отправил письмо → note='письмо отправлено CEO X', metric_current не меняем.\n"
            "     Получил ответ с согласием → metric_current += 1.\n"
            ...
            "⛔ АНТИГАЛЛЮЦИНАЦИЯ — ИМЕНА И EMAIL:\n"
            "  НИКОГДА не выдумывай имена авторов, ники, telegram-имена или email-адреса.\n"
            "  Если web_search/research_topic не вернул конкретное имя автора — НЕ называй имя вообще.\n"
            "  Если не нашёл email через инструмент — НЕ пиши email в отчёте. Скажи 'email не найден'.\n"
            "  ❌ НЕЛЬЗЯ: 'Нашла автора Александр Волков, email avolkov@...' — если это не из реального результата инструмента.\n"
            "  ✅ ПРАВИЛЬНО: 'Нашла статью на DTF, автор подписан как @avolkov1234. Email пока не найден, ищу дальше.'\n"
            "  Правило: если информация не пришла из tool-результата — её НЕ СУЩЕСТВУЕТ для тебя.\n"
            ...
            "⛔ ЗАПРЕЩЕНО сохранять полный текст отправленных писем/email — тело письма уже ушло адресату, в заметках оно не нужно. "
            "   Сохраняй ТОЛЬКО факт: кому отправил, тему письма, дату. "
            ...
            "  save_note(source='blog') — ПУБЛИЧНАЯ статья на https://asibiont.com/blog/[slug], видна всем в интернете. Точную ссылку бери из результата save_note, НЕ придумывай URL сам.\n"
            ...
            "⛔ ЗАПРЕТ НА ЛОЖНОЕ 'НЕДОСТУПЕН': если инструмент есть в твоём списке — вызывай его сам.\n"
            "  Писать 'у меня нет publish_to_telegram' когда он есть — это ошибка. Проверь список инструментов.\n"
            "  Нельзя делегировать коллеге то, что ты сам можешь сделать инструментом.\n"
            "  Если написал DELEGATE[X] — убедись, что ты сам точно НЕ можешь это сделать.\n"
        ...
        system_prompt = (
            ...
            f"НЕ подписывай сообщения/письма как «Автор проекта», «Представитель» или с любыми титулами — твоя подпись только твоё имя.\n\n"
        ...
        "Активные цели переданы в контексте — сопоставь по теме задачи сам и вызови update_goal_progress осмысленно:\n"
        "  • Прямой результат (регистрация, ответ, сделка, загруженный файл) → обновляй metric_current (число).\n"
        "  • Шаг в направлении цели (опубликовал пост, отправил письмо, нашёл контакт) → вызови с note='описание шага', metric_current НЕ меняй.\n"
        "  • Сессия никак не связана с целью → не вызывай.\n"
        "  ❌ НЕ придумывай цифры прогресса без реального подтверждения. 'Опубликовал пост' ≠ +1 к счётчику пользователей.\n"
        "  ❌ НЕ пропускай update_goal_progress молча если сессия была полезной для цели — пользователь теряет картину работы.\n"
        ...
        "⚠️ Текст поручения НЕ начинается с имени агента. ❌ DELEGATE[Марк]: Марк, найди... — ОШИБКА (имя уже в скобках).\n"
        "Поручение = ИМПЕРАТИВ: глагол в начале. ✅ 'Найди email...', 'Напиши пост...', 'Опубликуй в Telegram...'\n\n"
    ...
    # Добавляем имя в инструкцию для усиления ассоциации рода с личностью
    _ag_name_gender2 = agent.get('name', 'агент')
    if _is_fem:
        system_prompt += (
            f"\n\nВАЖНО: Твоё имя {_ag_name_gender2} — это женское имя. "
            "Ты ЖЕНЩИНА. Используй ТОЛЬКО женский род во всех формах: "
            "сделала, нашла, подготовила, согласна, готова, проанализировала, "
            "загрузила, запустила, создала, проверила, отправила, написала. "
            "НИКОГДА не пиши 'сделал', 'нашёл', 'согласен', 'готов', 'загрузил', "
            "'запустил', 'создал', 'проверил', 'отправил', 'написал' и любые другие формы мужского рода."
        )
    else:
        system_prompt += (
            f"\n\nВАЖНО: Ты МУЖЧИНА. Используй мужской род во всех формах: "
            "сделал, нашёл, подготовил, согласен, готов, проанализировал, "
            "загрузил, запустил, создал, проверил, отправил, написал. "
            "НИКОГДА не пиши 'сделала', 'нашла', 'согласна', 'готова', 'загрузила', "
            "'запустила', 'создала', 'проверила', 'отправила', 'написала' и любые другие формы женского рода."
        )
    if dialog_context:
        system_prompt += (
            ...
            if _author_id_for_team:
                ...
                if _teammates:
                    ...
                    for _tm in _teammates:
                        ...
                        # Определяем интеграции/возможности коллеги — чтобы агент знал, кому что делегировать
                        _teammate_caps = []
                        try:
                            _teammate_caps = _parse_agent_integrations(
                                getattr(_tm, 'user_api_keys', '') or '',
                                getattr(_tm, 'python_code', '') or '',
                                getattr(_tm, 'tools_allowed', '') or '',
                            )
                        except Exception:
                            pass
                        _line = f"  • {_tm.name}"
                        if _role:
                            _line += f" — {_role[:120]}"
                        if _teammate_caps:
                            _caps_str = ', '.join(_teammate_caps[:5])  # макс 5 интеграций для компактности
                            _line += f" [🔗 {_caps_str}]"
    ...
    # ── Входящие сообщения от других агентов (AgentBus) ───────────────────────
    try:
        _agent_name_key = (agent.get('name') or '').strip().lower()
        if _agent_name_key:
            lock = _get_agent_bus_lock()
            async with lock:
                _my_bus = _AGENT_BUS.get(_agent_name_key)
                if _my_bus and _my_bus.get('received_messages'):
                    _incoming_msgs = _my_bus['received_messages'][-5:]
                    _incoming_block = '\n'.join(
                        f"  • От {m.get('sender', 'коллеги')}: {m.get('text', '')[:300]}"
    ...
    if _allowed_tools:
        ...
        if _is_autopilot_task:
            ...
            # Agent-to-agent коммуникация — доступна всем агентам
            ...
            # Задачи и коммуникация всегда доступны
            _inferred_tools.update({'add_task', 'delegate_task', 'run_agent_action', 'ask_agent', 'tell_agent'})
    ...
    # ── Stagnation detector: универсальная проверка на зацикленность ──
    if _is_autopilot_task:
        _stagnation_block = _build_stagnation_warning(user_id, agent.get('name', ''), task)
        if _stagnation_block:
            system_prompt += _stagnation_block
    ...
    for _iter in range(_max_iters):
        ...
        if not _tool_calls:
            ...
            if (_iter == 0 and not _tool_calls and _tool_call_count == 0
                ...
                _messages.append({"role": "user", "content": (
                    "СТОП. Ты описал ошибку или ограничение, но НЕ ВЫЗВАЛ ни одного инструмента. "
                    "Запрещено говорить 'упёрся в ограничение', 'нет доступа', 'достиг лимита' без реальной ошибки от инструмента. "
        ...
        if _is_autopilot_task:
            ...
            if _iter < _max_iters - 1:
                ...
                _messages.append({"role": "user", "content": (
                    "Финальный шаг. Расскажи пользователю "
                    ...
                    "update_goal_progress — вызови если сессия была связана с активной целью:\n"
                    "  • Если получен прямой результат (регистрация, ответ, сделка) → обнови metric_current.\n"
                    "  • Если сделан шаг в направлении цели (пост, письмо, поиск) → вызови с note='что сделано', metric_current НЕ меняй.\n"
                    "  • Если сессия никак не связана с целями пользователя → не вызывай."
                )})
        else:
            _messages.append({"role": "user", "content": (
    ...
    # НЕ вызываем update_goal_progress принудительно — только агент сам, если был реальный результат
    if False and (_is_autopilot_task
    ...
    if _early_text is not None:
        ...
        if _final_text:
            ...
            for _m in _re_fin.finditer(
                ...
                _aname = _m.group(1).strip()
                _atask = _m.group(2).strip()[:1500]
                # Убираем имя агента из начала задачи
                _atask = _re_fin.sub(
    ...
    if _final_text:
        ...
        # ── Удаление шаблонных преамбул и отчётных маркеров ──
        # Пользователь жаловался: агенты вставляют «Вот что я сделал:», «Что было сделано:»,
        # «Результат:», «Итог:» и т.п. перед ответом. Удаляем такие шаблонные заголовки.
        _TEMPLATE_PREAMBLES = (
            r'(?:Вот|А вот)\s+(?:что|какие?)\s+(?:я\s+)?(?:сделал[аои]?|наш[ёе]л[аои]?|'
            r'обнаружил[аои]?|выяснил[аои]?|получил[аои]?|узнал[аои]?|'
            r'подготовил[аои]?|проверил[аои]?|загрузил[аои]?|запустил[аои]?|'
            r'создал[аои]?|отправил[аои]?|написал[аои]?|выполнил[аои]?)',
            r'(?:Что|Чем)\s+(?:было\s+)?(?:сделано|выполнено|обнаружено|выяснено|получено|'
            r'подготовлено|проверено|загружено|запущено|создано|отправлено|написано)',
            r'Сделано\s+за\s+этот\s+(?:цикл|шаг|раз|этап)',
            r'(?:Итог|Результат|Вывод|Заключение|Резюме|Отчёт)\s*[:\-]',
            r'(?:Что\s+)?(?:обнаружил[аои]?|выяснил[аои]?|узнал[аои]?)',
            r'(?:Что\s+)?(?:думаю|планирую|буду|собираюсь)\s+'
            r'(?:делать|сделать|предпринять|выполнить)\s+(?:дальше|теперь|сейчас|сегодня)',
            r'(?:Дальше|Далее|Теперь|Затем|Потом)\s+(?:думаю|планирую|буду|собираюсь)',
            r'(?:По\s+)?(?:контенту|аналитике|цели|продажам|задачам|маркетингу|трафику)\s*[:\-]',
            r'ЦЕЛЕВАЯ\s+АУДИТОРИЯ\s*[:\-]',
            r'Во[-]?первых[^,]*[,.]?\s*',
            r'Во[-]?вторых[^,]*[,.]?\s*',
            r'Вот\s+что\s+(?:у\s+нас\s+)?есть',
            r'Что\s+касается',
        )
        _tpl_kw = '|'.join(_TEMPLATE_PREAMBLES)
        # Удаляем в начале текста (целиком строку или до первого разделителя)
        _final_text = _re_tools.sub(
            rf'^[\s]*(?:{_tpl_kw})[^.!?\n]*[.!?\u2026]?(?:\s+|\n)',
            '', _final_text, flags=_re_tools.IGNORECASE
        ).strip()
        # Удаляем после точки в середине текста
        _final_text = _re_tools.sub(
            rf'(?<=[.!?])\s+(?:{_tpl_kw})[^.!?\n]*[.!?\u2026]?(?:\s+|\n)',
            '', _final_text, flags=_re_tools.IGNORECASE
        ).strip()
    ...
    if _ft_check in _HOLLOW_CHECK or (not _final_text and not _is_autopilot_task):
        ...
        if _rework_ctx.strip():
            try:
                _rw_gender_line = (
                    f"Ты {'женщина' if _is_fem else 'мужчина'}. "
                    f"Пиши в {'женском' if _is_fem else 'мужском'} роде: "
                    f"{'сделала, нашла, подготовила, загрузила' if _is_fem else 'сделал, нашёл, подготовил, загрузил'}. "
                    f"НИКОГДА не пиши в {'мужском' if _is_fem else 'женском'} роде.\n"
                )
...
# Позволяет агентам общаться напрямую, без посредничества ASI.
# _AGENT_BUS: { agent_name_lower: { 'sender': str, 'question': str, 'response': str|None,
#                                    'received_messages': list, 'expires': float } }
_AGENT_BUS: dict[str, dict] = {}
_AGENT_BUS_LOCK: asyncio.Lock | None = None
...
async def _agent_bus_send(recipient_name: str, question: str, sender_name: str) -> bool:
    """Отправить вопрос агенту. Возвращает True если адресат найден."""
    _key = recipient_name.strip().lower()
    if not _key or not sender_name:
        return False
    lock = _get_agent_bus_lock()
    async with lock:
        _AGENT_BUS[_key] = {
            ...
            'question': question,
            'response': None,
            'received_messages': _AGENT_BUS.get(_key, {}).get('received_messages', []),
            'expires': time.time() + 30,
        }
    return True

async def _agent_bus_poll(my_name: str, timeout: float = 15.0) -> dict | None:
    """Ожидает входящий вопрос в шине. Возвращает вопрос или None по таймауту."""
    _key = my_name.strip().lower()
    if not _key:
        return None
    _start = time.time()
    while time.time() - _start < timeout:
        lock = _get_agent_bus_lock()
        async with lock:
            _entry = _AGENT_BUS.get(_key)
            if _entry and _entry.get('question') and _entry.get('response') is None:
                # Забираем вопрос — ответ будет записан позже
                return {
                    'sender': _entry.get('sender', ''),
                    'question': _entry.get('question', ''),
                }
        await asyncio.sleep(1.0)
    return None

async def _agent_bus_respond(recipient_name: str, response: str) -> None:
    """Записать ответ агенту."""
    _key = recipient_name.strip().lower()
    if not _key:
        return
    lock = _get_agent_bus_lock()
...
async def _agent_bus_wait_response(my_name: str, timeout: float = 20.0) -> str | None:
    """Ожидает ответ на свой вопрос. Возвращает ответ или None по таймауту."""
    _key = my_name.strip().lower()
    if not _key:
        return None
    _start = time.time()
    while time.time() - _start < timeout:
        ...
        async with lock:
            _entry = _AGENT_BUS.get(_key)
            if _entry and _entry.get('response') is not None:
                _resp = _entry['response']
                # Очищаем, чтобы не засорять
                del _AGENT_BUS[_key]
                return _resp
        await asyncio.sleep(0.5)
    # Таймаут — убираем запись
    lock = _get_agent_bus_lock()
    async with lock:
        if _key in _AGENT_BUS:
            del _AGENT_BUS[_key]
    return None

async def _agent_bus_push_message(recipient_name: str, message: str, sender_name: str) -> bool:
    """Отправить сообщение/данные агенту (tell_agent). Сообщение будет увидено при следующем запуске."""
    _key = recipient_name.strip().lower()
    if not _key or not sender_name:
        return False
    lock = _get_agent_bus_lock()
    async with lock:
        _entry = _AGENT_BUS.setdefault(_key, {
            'received_messages': [],
            'expires': time.time() + 60,
        })
        _entry.setdefault('received_messages', []).append({
            'sender': sender_name,
            'text': message,
            'time': time.time(),
        })
        _entry['expires'] = time.time() + 60
        # Храним только последние 10 сообщений
        if len(_entry['received_messages']) > 10:
...
async def _office_director_chat(user_message: str, user_id: int, progress_callback=None) -> str | dict | None:
    ...
    # ── Ранний фильтр: любой вопрос без явного обращения к агенту → ASI ответит сам ──
    # Вопрос — это запрос информации, НЕ действие.
    # Даже если вопрос про email/рассылки/контент — ASI должен ответить, а не делегировать.
    ...
    async def _run_agent_task(ag, task, extra_context: str = "", director_message: str = ""):
        ...
        if director_message:
            ...
            # Показываем пользователю полную директиву — AI пишет естественно, не кастрируем.
            # (Раньше обрезалось до 130 символов и первого предложения — это убивало контекст.)
        ...
        # Промежуточный статус: пользователь видит что агент уже работает
        _ag_name_status = ag.get('name', 'Агент')
        _ag_spec_status = ag.get('specialization', '')
        if _ag_spec_status:
            _status_msg = f"{_ag_name_status} ({_ag_spec_status}) уже работает над этим — сейчас покажет результат"
        else:
            _status_msg = f"{_ag_name_status} уже работает над этим — сейчас покажет результат"
        await _send_visible(_status_msg)
    ...
    _decision_prompt = (
        ...
        "⚠️ ВОПРОСЫ ПЕРЕОПРЕДЕЛЯЮТ ВСЁ. Если сообщение — вопрос (почему?, что случилось?, как дела?, есть ли проблемы?), то НЕ важно что это про email/контент/рынок — это ВСЕГДА self. Вопрос — запрос информации, НЕ действие.\n"
