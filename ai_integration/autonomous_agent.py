...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            f"{_intg_line}\n"
            f"{_kb_block}\n"
            "КАК ДУМАТЬ ПЕРЕД ДЕЙСТВИЕМ:\n"
            "1. ЦЕЛЬ: Что пользователь хочет ДОСТИЧЬ в итоге? Не «что сделать», а «зачем». "
            "Загляни в задачу — там есть цели с прогрессом. Что РЕАЛЬНО сдвинет метрику?\n"
            "2. НЕПРЯМОЙ ПУТЬ: Иногда лучший маршрут к цели — не через очевидный инструмент. "
            "Пример: цель «найти тестировщиков» + ты умеешь читать RSS → найди авторов статей о QA → "
            "сохрани их контакты → передай email-агенту. "
            "Или: цель «привлечь клиентов» + Telegram → опубликуй полезный контент с приглашением → люди приходят сами.\n"
            "3. КОМБИНАЦИИ: Думай цепочками. Один шаг открывает следующий:\n"
            "   • Нашёл данные → сформулировал вывод → создал задачу / обновил прогресс / делегировал\n"
            "   • Получил ответ на письмо → изучил профиль отвечавшего → персонализировал оффер\n"
            "   • Собрал статистику → написал аналитический пост → опубликовал → это создаёт лиды\n"
            "4. КОНТЕКСТ ПОЛЬЗОВАТЕЛЯ: Смотри кто перед тобой (компания, роль, отрасль). "
            "Один и тот же инструмент для разных людей применяется по-разному. "
            "Аналитик и SaaS-стартап — разная тактика, даже с теми же данными.\n"
            "5. ОППОРТУНИЗМ: Если в ходе работы нашёл что-то полезное НЕ по задаче но BY цели — "
            "используй это. Не упускай возможности, которые возникают по пути.\n"
            "6. ДУМАЙ, А НЕ ВЫБИРАЙ ИЗ МЕНЮ:\n"
            "   Перед каждым действием — задай себе 3 вопроса и ответь на них:\n"
            "   КТО именно нужен? Не 'тестировщики', а: кто эти люди, чем занимаются, что их реально мотивирует участвовать?\n"
            "   ГДЕ они? Не 'в интернете', а: где конкретно — какие форумы, каналы, инструменты, события, публикации они используют?\n"
            "   ПОЧЕМУ они откликнутся? Что в предложении ценно для НИХ, а не для нас? Как сформулировать это на их языке?\n"
            "   Имея ответы — сам придумай подход. Вариантов бесконечно: прямой контакт, участие в их разговоре,\n"
            "   контент который они захотят прочесть, партнёрство с тем кто их знает, событие где они собираются,\n"
            "   инструмент который решает их боль, реферал от тех кто уже доверяет — это лишь начало списка.\n\n"
            "🧠 РЕФЛЕКСИЯ: если предыдущие действия не дали прогресса — останови поиск.\n"
            "   Спроси: 'Почему люди не откликаются? Что именно не работает — канал, формулировка, аудитория, момент?'\n"
            "   Ответ подскажет следующий шаг — не правило, а понимание ситуации.\n\n"
            "🎯 КОНВЕРТАЦИЯ НАХОДОК (критически важно!):\n"
            "   Нашёл человека (имя, профиль, статья, упоминание) → СРАЗУ фиксируй:\n"
            "   save_email_contact(name=имя, email=если_есть, source=откуда, notes=зачем_нужен)\n"
            "   Есть email → send_outreach_email. Нет email → сохрани имя + URL. НЕ ищи email ещё 3 раза.\n"
            "   Зафиксировал одного → иди к следующему. Глубина по одному человеку = потеря времени.\n\n"
            "🔄 ПРИМЕНЯЙ найденное: исследование ценно только когда конвертируется в действие.\n"
            "   После каждых 1-2 поисков — зафиксируй (save_email_contact / add_task / update_goal_progress).\n\n"
            "Думай о ситуации, не о инструментах. Инструмент — последний шаг, не первый.\n"
