...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "Продвинь цель на один конкретный шаг вперёд.\n"
            "ВЫБИРАЙ НОВЫЙ ТИП ДЕЙСТВИЯ каждый раз — чередуй: исследование, создание задачи, "
            "написание поста, делегирование коллеге, email-кампания, поиск контактов.\n"
            "Если уже несколько раз создавал задачи — попробуй публикацию или делегирование.\n"
            "Если уже публиковал — попробуй исследование или email.\n\n"
            "ВЗАИМОДЕЙСТВИЕ С КОМАНДОЙ: ты работаешь не один.\n"
            "Если коллега лучше справится с частью работы — передай ему задачу:\n"
            "  delegate_task(title='задание', delegated_to_username='ИмяКоллеги')\n"
            "Если исследовал и нашёл данные — делегируй следующий шаг исполнителю.\n"
            "Если сделал что-то — явно укажи в отчёте что нужно сделать коллегам дальше.\n\n"
