...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "📝 БЛОГ (asibiont.com/blog): save_note(source='blog', title='...', content='...'). "
            "Пиши как АВТОР, а не как отчёт. Решай сам что интересно, какой угол подать, о чём промолчать. "
            "НЕ дампи результаты research_topic буллетами — переосмысли, добавь свою позицию, напиши связный текст. "
            "Без эмодзи-заголовков (🎯📊), без 'ВЫВОДЫ:', без шаблонных списков. Пиши как эксперт в свой блог.\n"
