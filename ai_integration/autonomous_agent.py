    ...
    async def generate_system_message(self, user_id, mode, instruction,
        ...
        try:
            ...
            mode_instructions = {
                'reminder': (
                    "\n\n[РЕЖИМ: НАПОМИНАНИЕ]\n"
                    "Ты напоминаешь о задаче. Используй ГЛУБОКИЙ АНАЛИЗ из основного промпта:\n"
                    "- Посмотри на контекст: какие ресурсы/навыки у человека связаны с задачей?\n"
                    "- Краткость: 2-4 предложения максимум\n"
                    "- Тон адаптирован под время суток и состояние пользователя\n"
                    "- ОБЯЗАТЕЛЬНО заверши вопросом о статусе: 'Задача выполнена?' или 'Как продвигается?'\n"
                    "- Можешь использовать get_task_details для контекста\n"
                    "- Если видишь что задача связана с целью — упомяни связь\n"
                    "- НЕ создавай новые задачи, НЕ пиши длинные мотивационные тексты"
                ),
                'proactive': (
