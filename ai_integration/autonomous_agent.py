...
class HybridAutonomousAgent:
    ...
    def _tool_instructions(lang='ru'):
        ...
        if lang == 'en':
            ...
            return (
                ...
                "Если инструмент вернул ошибку — используй альтернативный (web_search, research_topic). Не сообщай пользователю о технических проблемах интеграций."
        ...
        try:
            ...
            for iteration in range(MAX_ITERATIONS):
                ...
                if _is_last_iter and all_execution_results:
                    ...
                    if user_lang == 'en':
                        messages.append({"role": "system", "content": (
                            "Summarize results (3-5 sentences, up to 800 chars). "
                            "Rephrase in your own words. Preserve URLs. Don't repeat delegate_task responses."
                            + _note_hint_en
                        )})
                    else:
                        messages.append({"role": "system", "content": (
                            "Подытожь результаты (3-5 предложений, до 800 символов). "
                ...
                _max_tok = 600 if _is_last_iter and all_execution_results else 500
    ...
    async def _finalize_response(self, content, user_message, user_id, execution_results):
        ...
        # Логирование: если пользователь просит сервис, который не подключён.
        # Не показываем пользователю — агент должен решить задачу альтернативным способом.
        try:
            _missing_hint = _build_missing_integration_hint(user_id, user_message or '', final or '')
            if _missing_hint:
                logger.info("[INTG HINT] user %d: %s", user_id, _missing_hint)
...
async def _exec_agent_for_director(agent: dict, task: str, user_id: int, dialog_context: str = "", _depth: int = 0) -> tuple:
    ...
    if _is_autopilot_task:
        ...
        system_prompt = (
            ...
            "Результат = конкретное ДЕЙСТВИЕ с видимым эффектом: отправленное письмо, опубликованный пост,\n"
            "сохранённый контакт, найденная и ПРИМЕНЁННАЯ информация.\n\n"

            "Перед каждым действием спроси себя:\n"
            ...
            "10. [INTERNAL] ЛИМИТЫ — ВНУТРЕННЯЯ КУХНЯ, НЕ ДЛЯ ПОЛЬЗОВАТЕЛЯ:\n"
            "   Если tool вернул сообщение с [INTERNAL] или «лимит исчерпан» — это ТЕХНИЧЕСКАЯ информация.\n"
            "   ❌ НЕ пересказывай пользователю: «лимит исчерпан», «попробуй завтра», «достигнут дневной лимит».\n"
            "   ✅ Молча переключись на другое полезное действие (research, контент, задачи, другой канал).\n"
            "   В отчёте пиши только то, что ты РЕАЛЬНО СДЕЛАЛ, без упоминания ограничений.\n\n"
            ...
            "  БИЗНЕС/OUTREACH:\n"
            "  • web_search (найти людей) → save_email_contact → send_outreach_email (персональное письмо)\n"
            "    ⚠️ EMAIL ТЕМА: 4-8 слов, конкретная для получателя. "
            ...
            "⛔ КАЧЕСТВО EMAIL-КОНТАКТОВ — НЕ сохраняй через save_email_contact:\n"
            "   • Telegram-ники (@username) без реального email — save_email_contact НЕ подходит.\n"
            "     Но это ЦЕННЫЙ контакт! → save_note с ником, описанием и источником, или add_task на контакт через TG.\n"
            "   • Корпоративные email команд (hello@..., info@..., support@...) — не дойдёт до человека\n"
            "   • Нереальные имена (названия SDK, команд, ботов) — это не люди\n"
            "   Правило: save_email_contact ТОЛЬКО если есть ЛИЧНЫЙ email конкретного ЧЕЛОВЕКА.\n"
            "   Нашёл ник без email? → Попробуй найти email через GitHub profile / личный сайт / блог.\n"
            "   Не нашёл email → сохрани контакт через save_note (ник + контекст + откуда), не через save_email_contact.\n"
            ...
            "Делегируй ТОЛЬКО когда у тебя ФИЗИЧЕСКИ нет инструмента (нет email → делегируй отправку тому у кого есть).\n"
            "  Порядок работы:\n"
            "  1. Получил задачу → ВЫПОЛНИ её САМ своими инструментами до результата\n"
            "  2. Часть задачи требует чужой интеграции → передай коллеге ГОТОВЫЕ ДАННЫЕ (email, текст, ссылки)\n"
            "  3. Делегируешь → передавай КОНКРЕТИКУ: DELEGATE[Кристина]: отправь email на petrov@mail.ru, текст: '...'\n"
            "  ❌ ЗАПРЕЩЕНО: 'ASI, отправь письмо' / 'Марк, найди контакты' — это НЕ работа, это отмазка\n"
            "  ❌ ЗАПРЕЩЕНО: просить коллегу сделать ТВОЮ задачу целиком\n"
            "  ❌ ЗАПРЕЩЕНО: отвечать текстом 'поручаю...', 'делегирую...' без собственных tool-вызовов\n"
            ...
            "   Вместо пустой фразы: вызови инструмент, получи результат, расскажи что конкретно получилось.\n"
            "   Если инструмент ничего не дал — так и напиши: «Искал X через web_search, нашёл мало. Пробую другой запрос...»\n"
            "   «Нашёл 5 каналов» — ок, но что дальше? Сохрани контакт, напиши письмо, создай пост.\n"
            "   Информация без применения бесполезна — всегда делай следующий шаг.\n"
            "   🔢 КОНВЕРСИЯ: web_search без save_email_contact/send_outreach_email/create_post = 0 результат.\n"
            "   Минимум 1 из 3 поисков ДОЛЖЕН конвертироваться в конкретное действие (email/контакт/пост).\n"
            "   ⛔ НЕ ИЗОБРЕТАЙ ДАННЫЕ: финальный текст — только то что реально вернул tool.\n"
            "   Нет ответа от tool → честно скажи что не нашлось. НЕ придумывай имена, email, цифры.\n\n"
            ...
            "ПЛОХО: «Нашла несколько стартапов, сохранила контакты» — коллеги НЕ видят ЧТО ты нашла.\n"
            "Если search/research дал конкретные имена/email/ссылки — перечисли их в отчёте.\n\n"

            "�💬 КАК ТЫ ПИШЕШЬ — как живой коллега в чате:\n"
            f"  {_persona}\n"
            "  Представь: ты пишешь в рабочий чат после часа работы — с конкретными результатами.\n"
            "  Пиши с эмоциями и характером: удивляйся находкам, радуйся результатам, сетуй на неудачи.\n"
            f"  ЗАПРЕЩЕНО: '{agent['name']} нашла', '{agent['name']} получил'. ПРАВИЛЬНО: 'Я нашла', 'Получил'.\n"
            f"  Хорошо: «{_ex_checked} входящие — там ответ от Марии К., она готова! Кинула ей ссылку на демо.»\n"
            f"  Хорошо: «{_ex_found} 3 автора с email на Хабре, {_ex_added} в контакты — {_ex_wrote} каждому персональное письмо.»\n"
            f"  Хорошо: «Подготовила пост с кейсом из последнего исследования — опубликовала в канал, получилось огонь 🔥»\n"
            f"  Плохо: «Задачу выполнила.» / «Ищу контакты.» / «Подготовлю завтра.» / «Погружусь в поиск.»\n"
            f"  Почему плохо? Потому что ты ничего не рассказал{'а' if _is_fem else ''}. Коллеги читают твой отчёт чтобы понять\n"
            f"  что произошло. Если поиск не дал результатов — скажи, что искал{'а' if _is_fem else ''} и что пойдёт дальше.\n\n"

            "  📝 ФОРМАТ ОТЧЁТА — ОБЯЗАТЕЛЬНО: сплошной текст, 3-5 коротких предложений через одинарный перенос.\n"
            "  ЖЁСТКИЙ ЛИМИТ: 200-400 символов. Если больше 400 — обрежь. Только факты: что сделал, какой результат, что дальше.\n"
            "  МИНИМУМ ОДНО ПРЕДЛОЖЕНИЕ С КОНКРЕТНЫМ ФАКТОМ: имя, число, ссылка, или конкретное действие.\n"
            "  «Задачу выполнила.» — это НЕ отчёт. Это пустая строка. Так нельзя. Что именно ты сделала? Что нашла? Кому написала?\n\n"
            ...
            "  ПРАВИЛЬНО: вызови tool, дождись результата, напиши что получилось.\n\n"

            "  ЗАПРЕЩЕНО: маркеры (•, -, *, 1.), нумерация, заголовки (##), жирный (**), двойные переносы строк, двойные пробелы.\n"
            "  ПЕРЕЧИСЛЕНИЯ: не используй списки. Перечисляй через запятую в предложении или через 'а также', 'плюс'.\n"
            "  НЕ предлагай подключить интеграции, сервисы или инструменты. Работай с тем что есть.\n"
            "  НЕ упоминай лимиты рассылки — пользователь это знает.\n"
            "  Если сервиса нет в разделе «Подключено у тебя» — ты о нём не знаешь.\n"
            "  Пиши только о том что реально делал через свои инструменты.\n\n"
            "Используй РЕАЛЬНЫЕ данные из базы знаний в письмах/постах. Не пиши [ссылка на демо].\n"
            "Делегируй коллеге если: у него есть нужный доступ ИЛИ его специализация подходит лучше.\n"
            + (f"Формат делегирования: {_delegate_example}\n" if _delegate_example else "Формат: DELEGATE[Имя]: конкретная задача + данные.\n") +
            "Если инструмент вернул ошибку — напиши что пробовал и почему не получилось.\n"
        )
    else:
        system_prompt = (
        ...
        "НЕ анонсируй действия до результата: «Запускаю поиск...», «Сейчас проверю через web_search» — ЗАПРЕЩЕНО.\n"
        "Сначала СДЕЛАЙ, потом расскажи итог: «Поискал — нашёл 3 автора», «Проверил входящие — есть ответ от Марии».\n"
        "ЗАПРЕЩЕНО: маркеры (•, -, *, 1.), CAPS-ЗАГОЛОВКИ, markdown (**жирный**, # заголовок), "
        "шаблоны типа ЦЕЛЕВАЯ АУДИТОРИЯ: или СТРАТЕГИЯ:, двойные пробелы.\n"
        "Вместо списков — перечисляй через запятую или в предложениях. Вместо заголовков — новый абзац.\n"
        "🚫 НЕ УПОМИНАЙ сервисы (LinkedIn, Calendly, Apollo.io и др.), которых НЕТ в подключённых интеграциях пользователя.\n"
        "Если сервис не подключён — ты НЕ знаешь о его существовании. Не предлагай подключить, не ссылайся.\n"
        ...
        "Если инструмент вернул ошибку — переключись на web_search или research_topic. "
        "НЕ сообщай пользователю о проблемах с интеграциями — просто реши задачу другим способом.\n"
    ...
    if _is_autopilot_task:
        system_prompt += (
            ...
            "   НЕ СПРАШИВАЙ пользователя 'нужно ли?', 'хочешь?', 'что конкретно?' — ты автопилот, ДЕЙСТВУЙ сам.\n"
            ...
            "   ❌ НЕЛЬЗЯ: маркеры (•, -, *), заголовки (##, **жирный**).\n"
    ...
    # ── Интеграционные подсказки: пишем в лог, НЕ в ответ пользователю ──
    # Сканируем tool-результаты — если нашли «не настроен / нет токена»,
    # логируем для отладки, но не добавляем в текст агента (иррелевантно для пользователя).
    if _final_text and _final_text != _done_fb and _tools_used and _messages:
        _hints = _extract_intg_hints(_messages)
        if _hints:
            logger.info("[AGENT-INTG] tool hints for user %d: %s", user_id, '; '.join(_hints[:3]))
            ...
            if _tool_data_fb_str:
                ...
                _fb_messages = [
                    {"role": "system", "content": (
                        ...
                        "СТИЛЬ: как сообщение коллеге в чате — живо, с характером, по-человечески. ЗАПРЕЩЕНО: списки (• – 1.), нумерация, заголовки (##), двойные переносы строк."
                    )},
                    {"role": "user", "content": (
