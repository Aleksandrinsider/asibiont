        ...
        try:
            ...
            try:
                ...
                base_prompt += (
                    ...
                    "\n⚠️ ЗАДАЧИ БЕЗ ВРЕМЕНИ: Если в списке задач пользователя есть задачи с пометкой '⚠️ БЕЗ ВРЕМЕНИ' — это ПРОБЛЕМА. При КАЖДОМ взаимодействии с пользователем, если есть такие задачи, НАПОМНИ: 'Кстати, у задачи X нет времени напоминания — на когда её поставить?'. Предложи конкретное время. Задача без напоминания = задача которую забудут."
                )
            except Exception as e:
