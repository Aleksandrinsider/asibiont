...
Если пользователь говорит "сделал/готово/выполнил" → complete_task()"""
        ...
        task_section = """\nАКТИВНАЯ ЗАДАЧА: НЕТ. У пользователя НОЛЬ задач в базе данных.
⛔ СТРОГОЕ ПРАВИЛО: НЕ упоминай никакие задачи в ответе. Даже если в истории разговора, в памяти или в другом контексте есть слова про задачи — это СТАРЫЕ ДАННЫЕ. Сейчас задач НЕТ. Не говори "у тебя задача на завтра", "ты планировал", "напомню про". Задач = 0."""
...
Ответь как хочешь (шутки, юмор — ок), но ОБЯЗАТЕЛЬНО заверши вопросом о человеке.
Не давай советов вслепую. Не предлагай задачи/цели.
ВАЖНО: данные о задачах и профиле бери ТОЛЬКО из разделов АКТИВНАЯ ЗАДАЧА и ПРОФИЛЬ выше. История разговора — НЕ источник фактов о задачах пользователя.
ГЛАВНОЕ: в конце ответа ВСЕГДА спроси кто он, чем занимается, что интересно.
"""
        else:
            profile_instruction = f"""
