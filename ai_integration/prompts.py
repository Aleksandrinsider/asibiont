...
def generate_proactive_context(user_id, session):
    ...
    try:
        ...
        if not has_tasks and not has_interests and not has_goals:
            ...
            proactive_hints.append("💡 РЕКОМЕНДУЙ КОНТЕНТ: предложи полезные книги, курсы, активности по погоде/времени суток")
            proactive_hints.append("📚 ПРИМЕРЫ КНИГ: 'Атомные привычки' (продуктивность), 'Поток' (психология), популярные новинки LitRPG/фантастики")
...
- ЕСЛИ ПРОФИЛЬ ПУСТОЙ (нет задач/целей/интересов):
  ...
  * Предлагай полезный контент: книги (например, "Атомные привычки", "Поток", популярные новинки жанра который обсуждается)
