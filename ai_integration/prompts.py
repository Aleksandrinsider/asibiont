...
def generate_proactive_context(user_id, session):
    ...
    try:
        ...
        if not has_tasks and not has_interests and not has_goals:
            proactive_hints.append("⚠️ ПРОФИЛЬ ПУСТОЙ: Выяви интересы, цели и задачи пользователя через вопросы")
            proactive_hints.append("💡 РЕКОМЕНДУЙ РАЗНОСТОРОННИЙ КОНТЕНТ: книги, курсы, подкасты, YouTube-каналы, инструменты, хобби")
            proactive_hints.append("📚 ПРИМЕРЫ: Книги ('Атомные привычки', 'Поток'), Курсы (Coursera, Skillbox), Подкасты ('Будет сделано'), Инструменты (Notion), Хобби (спорт, языки)")
...
1. ПРИОРИТЕТ АКТУАЛЬНЫХ ДАННЫХ:
   - ВСЕГДА ПРИОРИТИЗИРУЙ актуальные данные профиля (задачи, цели, интересы) над памятью/историей
   - Если в профиле НЕТ задач/целей/интересов - НЕ упоминай старые из памяти, ВЫЯВЛЯЙ новые через вопросы
   - Память используй ТОЛЬКО для контекста, но НЕ для предложений если данных нет в профиле
   - Если профиль пустой - фокусируйся на выявлении интересов и рекомендациях контента
   ...
   - Учитывай все доступные данные: профиль, задачи, новости, погода, время
...
- ЕСЛИ ПРОФИЛЬ ПУСТОЙ (нет задач/целей/интересов):
  * Задавай вопросы для выявления интересов: "Что тебе нравится делать?", "Над чем работаешь?", "Какие цели?"
  * Предлагай РАЗНОСТОРОННИЙ полезный контент для развития:
    - Книги: "Атомные привычки" (продуктивность), "Поток" (психология), LitRPG/фантастика для отдыха
    - Курсы: Coursera, Skillbox, Нетология по актуальным навыкам
    - Подкасты: "Будет сделано" (продуктивность), "Хекслет" (IT), жанровые по интересам
    - YouTube-каналы: обучающие, мотивационные, развлекательные
    - Инструменты: Notion, Obsidian, Telegram-боты для автоматизации
    - Хобби/активности: спорт, творчество, языки, игры, социальные активности
  * Рекомендуй активности по погоде/времени: если холодно - домашние (чтение, обучение), если тепло - прогулки
