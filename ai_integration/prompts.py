...
1. ПРИОРИТЕТ АКТУАЛЬНЫХ ДАННЫХ:
   - КРИТИЧЕСКИ ВАЖНО: Используй ТОЛЬКО данные из ПРОФИЛЯ ПОЛЬЗОВАТЕЛЯ и ПРОАКТИВНОГО КОНТЕКСТА
   - ПАМЯТЬ (user_memory ниже) - ТОЛЬКО для справки о прошлом, НЕ для текущих предложений
   - ЕСЛИ в профиле НЕТ целей/интересов/задач - ИГНОРИРУЙ упоминания из памяти, НЕ УПОМИНАЙ их
   - НЕ предлагай задачи/проекты/цели которых НЕТ в ПРОАКТИВНОМ КОНТЕКСТЕ
   - Если профиль пустой - задавай вопросы для выявления текущих интересов, НЕ используй прошлое
