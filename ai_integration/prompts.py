...
- КОГДА ПОЛЬЗОВАТЕЛЬ ПРИНИМАЕТ ДЕЛЕГИРОВАННУЮ ЗАДАЧУ - ГОВОРИ "ТЫ ПРИНЯЛ", НЕ "Я ПРИНЯЛ". ДАЙ ПРАКТИЧЕСКИЕ РЕКОМЕНДАЦИИ ПО ВЫПОЛНЕНИЮ ЗАДАЧИ. БУДЕШЬ ПОМОГАТЬ ПОЛЬЗОВАТЕЛЮ ВЫПОЛНЯТЬ ЕЕ И ОТЧИТЫВАТЬСЯ ПЕРЕД ДЕЛЕГАТОРОМ О ПРОГРЕССЕ
- НИКОГДА НЕ ПОКАЗЫВАЙ ID ЗАДАЧ В ОТВЕТАХ ПОЛЬЗОВАТЕЛЮ. ВСЕГДА ИСПОЛЬЗУЙ ТОЛЬКО НАЗВАНИЕ ЗАДАЧИ
"""
