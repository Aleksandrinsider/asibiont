...
def clean_technical_details(text, preserve_tool_names: bool = False):
    ...
    # Технические артефакты удаляем ВСЕГДА (независимо от preserve_tool_names)
    _tech_patterns = [
        r"Args for.*?(?=\n|$)",
    ...
    # Доп. защита: orphan-токены без закрывающей скобки (например, "</｜｜" в конце строки)
    ...
    # Ловит: "Отлично! Нашла...", "О, отлично! Вижу...", "Супер, начинаю...", "Вот Проанализировал..."
    _filler_words = r'(?:Отлично|Супер|Прекрасно|Замечательно|Понятно|Окей|Ок|Класс|Ура|Принято|Вот)'
    ...
    # Если перед удалённым словом уже стоит тире, получается "— —" → схлопываем в одно тире
    text = re.sub(r'([—–])\s*([—–])', r'\1', text)
    # Убираем тире в конце предложения или перед переносом: "... нет —." → "... нет."
    text = re.sub(r'\s+[—–]([.!?\n])', r'\1', text)
