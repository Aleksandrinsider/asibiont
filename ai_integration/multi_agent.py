...
class Analyst(AgentRole):
    ...
    def analyze(self, user_message, profile_data, tasks_data, memory_context):
        ...
        # Анализ задач
        if not tasks_data:
            analysis.append("ЗАДАЧ НЕТ. Проактивность: предложи идеи, тренды, цели по интересам.")
...
class Strategist(AgentRole):
    ...
    def plan(self, intent, emotion, profile_data, has_tasks):
        ...
        intent_strategies = {
            'greeting': 'ПРИВЕТСТВИЕ → Если профиль пустой: один тёплый вопрос. Если есть: research по интересам или тренды.',
        ...
        if not has_tasks:
            strategies.append('ЗАДАЧ НЕТ → Предложи идею/цель. Не "чем помочь?", а конкретное предложение.')
