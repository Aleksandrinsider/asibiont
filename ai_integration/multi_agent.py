...
class Analyst(AgentRole):
    ...
    def analyze(self, user_message, profile_data, tasks_data, memory_context):
        ...
        if msg_len < 15:
            analysis.append("КОРОТКОЕ сообщение → ответ по существу, но НЕ отписка. Конкретика важнее длины.")
        elif msg_len > 100:
...
class Strategist(AgentRole):
    ...
    def plan(self, intent, emotion, profile_data, has_tasks):
        ...
        intent_strategies = {
            'greeting': 'ПРИВЕТСТВИЕ → Если профиль пустой: один тёплый вопрос. Если задач нет: спроси чем занят СЕЙЧАС, предложи помощь в текущем моменте.',
