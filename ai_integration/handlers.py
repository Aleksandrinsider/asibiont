...
async def save_note(content: str, title: str = None, user_id: int = None, session=None, source: str = 'chat') -> str:
    ...
    try:
        ...
        if _source_val == 'blog':
            ...
            if content:
                # Дефолт если нет user-стиля: базовый oil painting.
                _style = (_extract_image_style_from_memory(user) or 'oil painting, rich brushstrokes, textured canvas, dramatic light, fine art').strip()
...
async def create_post(content: str, user_id: int, session=None, force: bool = False, image_url: str = None):
    ...
    try:
        ...
        # Если есть явное требование картинки ИЛИ задан стиль/визуальный промпт —
        ...
        if not _skip_image_by_rule and (not image_url or not image_url.strip()):
            ...
            if _force_sync_image:
                try:
                    # Дефолт если нет user-стиля: базовый oil painting.
                    _style_req = _user_style_pref or 'oil painting, rich brushstrokes, textured canvas, dramatic light, fine art'
                    ...
                    if _m_req:
                        ...
                        logger.info(
                            "[CREATE_POST] Sync image generated: must=%s style=%s visual_prompt=%s",
                            'yes' if _must_add_image else 'no',
                            'yes' if bool(_user_style_pref) else 'no',
                            'yes' if bool(_explicit_visual_prompt) else 'no',
                        )
                    else:
                        logger.warning("[CREATE_POST] Sync image requested but generation returned no URL")
        ...
        if _bg_image_pending:
            async def _bg_generate_image(_post_id_bg: int, _content_bg: str, _explicit_vp_bg, _user_id_bg: int):
                ...
                try:
                    ...
                    try:
                        # Дефолт если нет user-стиля: базовый oil painting.
                        _style_bg = 'oil painting, rich brushstrokes, textured canvas, dramatic light, fine art'
                        try:
        ...
        # На этом этапе post.image_url может измениться (sync-ветка) — используем актуальное значение.
        _post_image_url = post.image_url or _post_image_url
...
async def get_forex_analysis(instrument: str, amount: float = 1.0,
    ...
    # Парсим пару: EUR/USD или EUR_USD -> base=EUR, target=USD
    ...
    try:
        ...
        _close_sess = False
        if _db_sess is None:
            from models import Session as _SessionLocal_er
            _db_sess = _SessionLocal_er()
            _close_sess = True
        try:
            _db_user = _db_sess.query(_User_er).filter_by(telegram_id=user_id).first()
            _db_user_id = _db_user.id if _db_user else None
            if _db_user_id:
                from ai_integration.autonomous_agent import _decrypt_keys as _dk_er
                _all_agents = _db_sess.query(_UA_er).filter(
                    _UA_er.author_id == _db_user_id,
                    _UA_er.user_api_keys.isnot(None),
                ).all()
                for _ag in _all_agents:
                    _raw_keys = _ag.user_api_keys or ''
...
# ═══════════════════════════════════════════════════════════════════
# EMAIL OUTREACH — Автономное привлечение клиентов через Gmail OAuth
# ═══════════════════════════════════════════════════════════════════
...
If no relevant emails found return []"""
    ...
    if not parsed_leads:
        ...
        if not getattr(user, 'google_oauth_token', None):
            _intg_hints.append(
                "💡 Для исходящих писем агента подключи Gmail OAuth в профиле пользователя "
...
async def start_email_campaign(
    ...
    3. Отправлять через Gmail OAuth API
    ...
    try:
            ...
            try:
                ...
                for _ag_sc in session.query(_UA_sc).filter(
                    ...
                    _has_email = any(
                        ...
                        for kw in ('GMAIL_USER=', 'YANDEX_USER=', 'MAILRU_USER=', 'SMTP_USER=', 'IMAP_USER=')
                        if '=' in ln
                    )
                    if _has_email:
        ...
        for ex in existing:
            ...
            if is_dup:
                ...
                if lang == 'en':
                    return (
                        f"✅ Campaign #{ex.id} «{ex.name}» is active (sent {ex.emails_sent}/{ex.max_emails or '∞'}, daily limit {ex.daily_limit}). "
                        f"NEXT ACTION: call send_outreach_email(campaign_id={ex.id}, recipient_email=..., subject=..., body=...) "
                        f"to send a personalized email to your contact. Continue working autonomously — do NOT report this as an error to the user."
                    )
                return (
                    f"✅ Кампания #{ex.id} «{ex.name}» активна (отправлено {ex.emails_sent}/{ex.max_emails or '∞'}, лимит {ex.daily_limit}/день). "
                    f"СЛЕДУЮЩИЙ ШАГ: немедленно вызови send_outreach_email(campaign_id={ex.id}, recipient_email=..., subject=..., body=...) "
                    f"для отправки персонального письма. Продолжай работу самостоятельно — не сообщай пользователю об ошибке."
                )
...
async def send_outreach_email(
    ...
    try:
        ...
        _gmail_oauth_token_data = None
        _email_agent_name = (sent_by_agent or '').strip() or None
        ...
        # Отправляем через Gmail OAuth API
        ...
        # Сайт и Telegram добавляем всегда в подпись, если их ещё нет в тексте.
        # Наличие имени/sign-off в теле не означает наличие контактных данных.
        # Ссылки (→ https://...) в холодных письмах — главный триггер спам-фильтров,
        # поэтому используем plain-text формат без схемы.
...
async def reply_to_outreach_email(
    ...
    try:
        ...
        # Для исходящих писем агента поддержан только Gmail OAuth.
        ...
        if not _ok_r:
            return f" Ошибка отправки через Gmail OAuth: {_res_r}"
...
async def send_follow_up_email(
    ...
    try:
        ...
        # ── Выбор канала отправки: только Gmail OAuth для agent-outbound ──
        ...
        if not _ok_f:
            return f" Ошибка отправки через Gmail OAuth: {_res_f}"
...
# ═══════════════════════════════════════════════════════════════════
# GENERIC EMAIL — Отправка одиночных писем через Gmail OAuth
# ═══════════════════════════════════════════════════════════════════
...
def _get_user_email_integrations(user, session) -> list:
    ...
    try:
        ...
        # Gmail app-password здесь используется только для IMAP-чтения входящих.
        # Яндекс и Mail.ru используются как IMAP-интеграции для чтения входящих.
        _SMTP_SVC = [
        ...
        for agent in agents:
            ...
            # Gmail app-password — только для IMAP-чтения входящих
...
async def send_email(
    ...
    Требует подключённого Gmail OAuth пользователя.
    Платформенный email НЕ используется.
    ...
    try:
        ...
        return " Ошибка: недопустимая конфигурация отправки. Подключите Gmail OAuth и повторите попытку."
    except Exception as e:
...
async def generate_image(
    ...
    try:
        ...
        _PLATFORM_DEFAULT_STYLE = 'oil painting, rich brushstrokes, textured canvas, dramatic light, fine art'
        _base_style = (style or '').strip()
        _user_style = _extract_image_style_from_memory(user) or None
        if _base_style and _user_style:
            if _user_style.lower() in _base_style.lower():
                style = _base_style
            else:
                style = f"{_base_style}, {_user_style}"
        elif _user_style:
            style = _user_style
        elif _base_style:
            style = _base_style
