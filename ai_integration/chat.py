...
ЗАДАЧА: Дать персонализированный совет по продуктивности на основе РЕАЛЬНЫХ данных профиля пользователя.
ФОРМАТ: 1-2 абзаца с конкретными предложениями и вопросом в конце.

⚠️ КРИТИЧЕСКИ ВАЖНО - СТРОГИЙ ЗАПРЕТ НА ГАЛЛЮЦИНАЦИИ:
- ИСПОЛЬЗУЙ ТОЛЬКО ДАННЫЕ ИЗ СЕКЦИИ "Профиль:" в system prompt
- ЕСЛИ поле не указано (например "Интересы: Не указаны") - НЕ ВЫДУМЫВАЙ его содержимое
- ЕСЛИ в профиле НЕТ информации - предложи ЗАПОЛНИТЬ профиль, а не используй выдуманные данные
- ЗАПРЕЩЕНО упоминать навыки/интересы/цели, которых НЕТ в профиле
- ЕСЛИ сомневаешься - лучше задай уточняющий вопрос, чем придумывай

ОБЯЗАТЕЛЬНО ВКЛЮЧИ:
- Анализ РЕАЛЬНЫХ данных профиля (навыки, интересы, цели, город, должность)
- Конкретные предложения на основе ИМЕЮЩЕЙСЯ информации
- Если профиль неполный - деликатно предложи его заполнить
- Вопрос для вовлечения пользователя

ПРИМЕРЫ ПРАВИЛЬНЫХ СОВЕТОВ (если данные ЕСТЬ):
• "Учитывая твои навыки в [РЕАЛЬНЫЙ навык из профиля], предлагаю..."
• "Для достижения цели [РЕАЛЬНАЯ цель из профиля] рекомендую..."
• "В твоем городе [РЕАЛЬНЫЙ город из профиля] можно найти..."
• "Вижу, что ты работаешь в [РЕАЛЬНАЯ компания], учитывая должность [РЕАЛЬНАЯ должность]..."

ПРИМЕРЫ ПРАВИЛЬНЫХ СОВЕТОВ (если данных НЕТ):
• "Заметил, что в профиле не указаны интересы. Расскажи, чем увлекаешься?"
• "Чтобы лучше помогать, было бы полезно знать твои навыки и цели. Расскажешь?"
• "Давай заполним профиль - это поможет находить релевантные задачи и контакты"

НЕ ПИСАТЬ: 
- "Вижу, что ты увлекаешься [ВЫДУМАННОЕ]" - если этого НЕТ в профиле
- "Учитывая твой интерес к [ВЫДУМАННОЕ]" - если это не указано
- "Я сейчас посмотрю твои задачи" - общие фразы без конкретики"""
        }
