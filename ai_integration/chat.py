...
Мягко напомни. НЕ ругай. Предложи: перенести, разбить на подзадачи, или отменить если неактуально.""")
    ...
    # Тип 5: Предложение задач (если задач 0 — но только иногда, не каждый раз!)
    if task_count == 0:
        message_types.append({
            'type': 'plan',
            'instruction': 'Предложи 2-3 КОНКРЕТНЫЕ задачи с временем, основываясь на профиле/целях. Спроси "Создать?". Но НЕ начинай с "вижу у тебя нет задач" — сразу к делу.'
        })
