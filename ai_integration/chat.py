...
3. ДЛИНА И СТРУКТУРА:
   - ПРИВЕТСТВИЯ: 4-6 предложений МИНИМУМ, проявляй интерес, задавай наводящие вопросы, заполняй профиль, вовлекай в диалог, будь персонализированным
...
async def chat_with_ai(message, context=None, user_id=None, file_content=None, db_session=None, message_type=None):
    ...
    try:
        ...
        if user_id:
            ...
            if user:
                tz_str = user.timezone if user.timezone else "Europe/Moscow"
            ...
            if fresh_tasks_info and "У вас есть" in fresh_tasks_info:  # Только если есть активные задачи
