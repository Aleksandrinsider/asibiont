...
3. ДЛИНА И СТРУКТУРА:
   - ПРИВЕТСТВИЯ: минимум 4-6 предложений + 2-3 вопроса для вовлечения
        ...
        if user_id:
            ...
            if profile:
                ...
                if not profile_filled and (len(context) if context else 0 < 2):
                    user_memory += """\n👋 ПЕРВОЕ ЗНАКОМСТВО - СПЕЦИАЛЬНЫЕ ИНСТРУКЦИИ:
Пользователь НОВЫЙ или профиль пустой. При приветствии (привет, здравствуй, hi):

1. ОБЯЗАТЕЛЬНО поприветствуй ТЕПЛО и ПЕРСОНАЛЬНО (не шаблонно)
2. ЗАДАЙ 2-3 ВОВЛЕКАЮЩИХ ВОПРОСА чтобы узнать:
   • Чем занимается сейчас / что планирует на день
   • Из какого города (для персонализации)
   • Какие интересы/хобби (для подбора контактов)
3. ПОКАЖИ ЦЕННОСТЬ: Намекни что бот поможет с задачами и найдет единомышленников
4. ФОРМАТ: 4-6 предложений, живой диалог, НЕ робот

ПРИМЕР ХОРОШЕГО ОТВЕТА:
'Привет! Рад познакомиться 👋 Я помогу тебе организовать задачи и найти интересных людей по твоим увлечениям. Расскажи - чем обычно занимаешься? Из какого ты города? А какие у тебя интересы - может спорт, программирование или что-то еще?'

ПРИМЕР ПЛОХОГО (ЗАПРЕЩЕНО):
'Привет! Как дела? Что планируешь на сегодня?' ← слишком коротко и шаблонно!"""
            else:
