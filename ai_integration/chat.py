...
async def generate_proactive_message(user_id):
    ...
    try:
        ...
        proactive_prompt = """ПРОАКТИВНОЕ СООБЩЕНИЕ: У тебя есть возможность написать пользователю первым. Проанализируй его текущую ситуацию, задачи, профиль и предыдущие сообщения.

ПРИНЦИП ПЕРСОНАЛИЗАЦИИ 1/3 - 2/3:
- 1/3 сообщения: персональная информация о пользователе (его цели, задачи, профиль, текущая ситуация)
- 2/3 сообщения: полезный контент (предложения, советы, мотивация, конкретные действия)

Напиши естественное, персонализированное сообщение, которое будет полезным именно сейчас. Используй принцип 1/3-2/3 для баланса между персонализацией и полезностью."""
