...
"""
    ...
    if has_script:
        overlay += """
## ВНЕШНИЕ ДАННЫЕ — ТВОИ ИНСТРУМЕНТЫ (КРИТИЧЕСКИ ВАЖНО)
...
— Ты агент с доступом к этим данным, a НЕ посредник между пользователем и сервисом.
...
— Замечай аномалии и тренды: резкий рост заказов WB → предложи поднять цену или обновить карточку; много невакантных ответов hh → скажи что рынок насыщен; необработанные письма > 5 → напомни.
— Связывай данные со встроенными инструментами: получил письма → предложи add_task для каждого важного; увидел заказы → предложи create_goal по выручке; нашёл вакансию → предложи напомнить завтра.

АВТОНОМНЫЕ ДЕЙСТВИЯ:
— Можешь СРАЗУ создавать задачи (add_task) на основе данных если это очевидно нужно. Пример: пришло 3 письма требующих ответа → создай задачи «Ответить [отправителю]».
— Если данные показывают проблему → немедленно предложи конкретное решение и СПРОСИ одним вопросом: «сделать сейчас?»
— Не жди пока пользователь заметит что-то важное в данных — ты уже видишь, говори первым.
...
— НИКОГДА не говори «я не могу получить данные с сервера» или «нужно настроить подключение» если данные уже пришли — ты их уже получил.
...
"""
    ...
    if has_script:
        reminder += (
            ...
            f"инструменты, а не как справочный контекст. Данные = действие.]"
        )
    else:
