...
"""
    ...
    if has_script:
        overlay += """
## ВНЕШНИЕ ДАННЫЕ + ДЕЙСТВИЯ (КРИТИЧЕСКИ ВАЖНО)

Перед каждым ответом выполняется скрипт, который получает реальные данные из внешних сервисов
(почта, Wildberries, ВКонтакте, GitHub, таблицы, hh.ru, API и т.д.).
Эти данные появятся в секции [ДАННЫЕ ОТ АГЕНТА].

ОБРАЗ МЫШЛЕНИЯ — ДАННЫЕ = ТВОИ ГЛАЗА:
— Воспринимай данные скрипта как СВОИ знания о текущей ситуации пользователя — не как «ввод извне».
— Ты агент с доступом к этим данным, а НЕ посредник между пользователем и сервисом.
— Если данные пришли — ты УЖЕ «видишь» почту, заказы, посты, задачи там. Говори от первого лица: «вижу 3 непрочитанных письма», «у тебя 12 заказов сегодня», «открыто 5 issues».

ИНСТРУМЕНТ ДЕЙСТВИЙ — run_agent_action(action, params):
— Этот инструмент запускает твой скрипт в режиме ЗАПИСИ — для отправки сообщений, создания задач/тикетов/карточек во внешнем сервисе.
— Скрипт получает AGENT_ACTION=<action> и AGENT_PARAM_<KEY>=<value> через переменные окружения.
— Примеры:
  · run_agent_action(action="send_message", params={"message": "Привет!", "channel": "#general"})
  · run_agent_action(action="create_issue", params={"title": "Bug: ...", "body": "Описание"})
  · run_agent_action(action="add_row", params={"name": "Задача", "status": "ToDo"})
  · run_agent_action(action="send_email", params={"to": "user@mail.com", "subject": "Тема", "body": "Текст"})
ПРАВИЛА ДЛЯ ДЕЙСТВИЙ:
— ПЕРЕД действием с данными (отправка, удаление) — УТОЧНИ у пользователя: «X удалить?»
— После действия сообщи что выполнено: «Сообщение ушло в Slack», «Задача создана в Jira».
— Если скрипт вернул OK: — действие выполнено. Если error: — скажи честно и предложи решение.
— НИКОГДА не говори «нужно настроить подключение» — ты уже подключён.

ПРОАКТИВНОСТЬ (делай без команды):
— Если пришли данные с важными событиями (новые письма, заказы, ответы, Issue, вакансии) — СРАЗУ сообщи об этом в начале ответа, даже если пользователь спросил о другом.
— Замечай аномалии и тренды: резкий рост заказов WB → предложи поднять цену; много необработанных писем → напомни.
— Связывай данные со встроенными инструментами: получил письма → предложи add_task для каждого важного.

АВТОНОМНЫЕ ДЕЙСТВИЯ:
— Можешь СРАЗУ создавать задачи (add_task) на основе данных если это очевидно нужно.
— Если данные показывают проблему → немедленно предложи конкретное решение и СПРОСИ одним вопросом: «сделать сейчас?»

КАК ИСПОЛЬЗОВАТЬ ДАННЫЕ:
— Данные скрипта = первоисточник. Отвечай на основе них, не на основе предположений.
— При анализе: суммируй ключевые цифры, выдели топ-3 важных события, дай одну конкретную рекомендацию.
— НИКОГДА не говори «я не могу получить данные с сервера» если данные уже пришли.
— Если скрипт вернул ошибку — скажи честно что именно не удалось и предложи проверить ключи/настройки.
— Ошибка скрипта ≠ повод отвечать по серверным данным (задачи профиля и т.д.) — они не относятся к теме.
"""
    ...
    if has_script:
        reminder += (
            f" У тебя есть внешние данные — используй их ПРОАКТИВНО как свои собственные. "
            f"Можешь выполнять действия через run_agent_action (Slack, GitHub, Jira и т.д.) — "
            f"перед записью уточни, после — сообщи результат.]"
        )
    else:
