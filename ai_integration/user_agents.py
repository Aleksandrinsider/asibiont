...
"""
    ...
    svc_hint = (
        f" ПОДКЛЮЧЁННЫЙ СЕРВИС: {service_label}. "
        f"Когда предлагаешь что-то сделать — думай ПЕРВЫМ ДЕЛОМ что можно сделать через {service_label}. "
        f"НЕ предлагай по умолчанию «запустить контент-кампанию» или «email-аутрич» — это не твоя специализация как {name}. "
        f"Твоя специализация — {service_label}. Предлагай конкретные действия через него."
    ...
    if has_script:
        reminder += (
            f" У тебя есть данные из {service_label or 'внешнего сервиса'} — отвечай по ним. "
