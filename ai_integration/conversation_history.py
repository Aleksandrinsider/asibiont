...
_TOKEN_HALLUCINATION_REPLACEMENTS = [
    ...
    # "амбассадор/ambassador" → "эксперт" / "партнёр" (профессиональная терминология)
    (r'(?:программ[аеуы]\s+)?амбасс?адор(?:ов|ами|ам|а|ы)?', 'партнёрская программа'),
    (r'(?:стать|станьте|приглашаем)\s+амбасс?адор(?:ом|ами)?', 'стать партнёром проекта'),
    (r'\bambassador(?:s)?\b', 'partner'),
]
