...
class OfficeEngine:
    ...
    async def _coordinate_user_goals(self, entry: dict):
        ...
        agents_text = "\n".join(
            ...
            for aid, name, spec, desc, *_ in agents
        )
        ...
        # ── Goal→Agent mapping: универсальный анализ для любой цели ──
        _goal_agent_mapping = (
            "🧠 АНАЛИЗ ЦЕЛЕЙ → ПОДБОР АГЕНТА (выполни ПЕРЕД назначением):\n"
            "Для КАЖДОЙ цели определи ТИП работы и подходящего агента по ЕГО реальным возможностям (указаны в квадратных скобках):\n"
            "  🔹 КОНТЕНТ/ПУБЛИКАЦИИ (посты, статьи, визуал) → агент С generate_image / create_post / publish_to_*\n"
            "  🔹 OUTREACH/EMAIL (письма, кампании, фоллоу-апы) → агент С email-ключами [email, IMAP, SMTP, Gmail] и/или GitHub для поиска контактов\n"
            "  🔹 ИССЛЕДОВАНИЕ/АНАЛИЗ (тренды, конкуренты, статистика) → агент С web_search, research_topic\n"
            "  🔹 КОММУНИКАЦИЯ (DM, сообщения пользователям) → агент С find_and_message_relevant_users\n"
            "  🔹 ПЛАНИРОВАНИЕ/ОРГАНИЗАЦИЯ (задачи, напоминания, заметки) → любой агент\n\n"
            "ПРАВИЛО: задача должна напрямую вести к ЦЕЛИ. Если цель про привлечение — задача должна привлекать. "
            "Если цель про продажи — задача должна продавать. Не подменяй цель абстрактным действием.\n\n"
        )
