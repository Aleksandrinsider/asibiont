...
class OfficeEngine:
    ...
    async def _coordinate_user_goals(self, entry: dict):
        ...
        prompt = (
            ...
            f"АКТИВНОСТЬ ЗА 6Ч:\n{activity_text}\n\n"
            f"{_failed_tasks_l2}"
            "КОМАНДНАЯ СТРАТЕГИЯ (ключевое!):\n"
            "Агенты — КОМАНДА, а не дублёры. Каждый делает СВОЁ, исходя из ЦЕЛИ пользователя:\n"
            ...
            "Антипаттерн 1: давать ВСЕМ одну и ту же задачу — это дублирование\n"
            "Антипаттерн 2: давать агенту инструмент, которого у него нет\n"
            "Антипаттерн 3: давать одинаковую стратегию каждый цикл — чередуй подходы\n"
            "Антипаттерн 4: если один подход (напр. 'найти контакты на Хабре') пробовался 2+ раз без результата — СМЕНИ ПОДХОД КАРДИНАЛЬНО\n\n"
            "⛔ ЗАПРЕТ НА НЕДОСТУПНЫЕ СЕРВИСЫ:\n"
            "НЕ упоминай LinkedIn, Calendly, Apollo.io, Sales Navigator в задачах — этих сервисов НЕТ.\n"
            "НЕ пиши 'найди профили на LinkedIn', 'через Sales Navigator' — агенты не имеют к ним доступа.\n"
            "Работай ТОЛЬКО с подключёнными интеграциями (email, Telegram, Discord, web_search, RSS).\n\n"
            "ПРАВИЛА НАЗНАЧЕНИЯ:\n"
            ...
            "   ХОРОШО (бизнес): «Проверь входящие через check_emails. Если есть ответы — ответь через reply_to_outreach_email.»\n"
            "   ХОРОШО (обучение): «Исследуй тему X через research_topic, подготовь конспект через save_note и создай задачу на практику.»\n"
            "   ХОРОШО (здоровье): «Найди через web_search актуальную программу тренировок для цели Y, создай план через add_task с напоминаниями.»\n"
            "   ХОРОШО (контент): «Подготовь экспертный пост (research_topic → create_post) и опубликуй через publish_to_telegram.»\n"
            "   ПЛОХО: «Проведи анализ» без конкретного инструмента и результата.\n"
            "3. Называй КОНКРЕТНЫЙ инструмент: web_search, research_topic, create_post, add_task, save_note, set_reminder, check_emails, send_outreach_email, publish_to_telegram и др.\n"
            "4. Каждый агент получает РАЗНУЮ задачу — не давай двум агентам одно и то же.\n"
            "5. Если прошлые циклы дали мало результата — СМЕНИ подход.\n"
            "6. Агент БЕЗ email-ключей НЕ МОЖЕТ отправлять/читать письма.\n"
            "7. Если цель имеет конкретный дедлайн — urgency=high.\n"
            "8. Если все задачи уже делаются или нечего делать — wait.\n"
            "9. Задача ДОЛЖНА быть полной цепочкой до конкретного результата.\n\n"
            "Ответь JSON (без ```):\n"
