...
class OfficeEngine:
    ...
    async def _coordinate_user_goals(self, entry: dict):
        ...
        prompt = (
            ...
            "Агенты — КОМАНДА, а не дублёры. Каждый делает СВОЁ:\n"
            "  • Агент С email → OUTREACH: отправляет письма, проверяет входящие, отвечает на ответы, фоллоу-апы\n"
            "  • Агент БЕЗ email → КОНТЕНТ/РЕСЁРЧ: готовит посты, ищет тренды, анализирует данные, публикует в канал\n"
            "  • Задачи должны ДОПОЛНЯТЬ друг друга: один готовит контент → другой рассылает, один ищет данные → другой применяет\n"
            "Антипаттерн 1: давать ВСЕМ агентам задачу 'поиск контактов' — это дублирование\n"
            "Антипаттерн 2: давать агенту без email задачу 'найди email' — он не сможет отправить\n"
            ...
            "   ХОРОШО: «Проверь входящие через check_emails. Если есть ответы на рассылку — ответь каждому через reply_to_outreach_email и обнови прогресс.»\n"
            "   ХОРОШО: «Подготовь экспертный пост о трендах AI-агентов (research_topic → create_post) и опубликуй через publish_to_telegram.»\n"
            "   ПЛОХО: «Проведи анализ социальных медиа для выявления каналов привлечения.»\n"
            "   ПЛОХО: «Найди контакты авторов на Хабре» (кому агент без email передавать контакты?)\n"
            "3. Называй КОНКРЕТНЫЙ инструмент (web_search, check_emails, send_outreach_email, research_topic, create_post, publish_to_telegram, save_email_contact и др.).\n"
            ...
            "6. Агент БЕЗ email-ключей НЕ МОЖЕТ отправлять письма — давай ему задачи на контент/анализ/публикации.\n"
            "7. Если цель имеет конкретный дедлайн — urgency=high.\n"
            "8. Если все задачи уже делаются или нечего делать — wait.\n"
            "9. Задача ДОЛЖНА быть полной: не 'найди контакты', а 'найди контакты → сохрани через save_email_contact → отправь письмо'.\n\n"
            "Ответь JSON (без ```):\n"
