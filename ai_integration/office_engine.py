...
class OfficeEngine:
    ...
    async def _coordinate_user_goals(self, entry: dict):
        ...
        prompt = (
            ...
            "   ХОРОШО (обучение): «Исследуй тему X через research_topic, подготовь конспект через save_note.»\n"
            "   ХОРОШО (здоровье): «Найди через web_search актуальную программу тренировок для цели Y, создай план через add_task.»\n"
