...
class CognitiveEngine:
    ...
    def build_cognitive_hints(user_message):
        ...
        # Анализ намерения
        intent_hints = {
            'greeting': '🎯 ПРИВЕТСТВИЕ: Поздоровайся + задай ОДИН живой вопрос. ЗАПРЕЩЕНО пересказывать профиль. СНАЧАЛА проверь секцию ПРОФИЛЬ — есть пустые поля? Тогда узнай о человеке естественно. Есть задачи/цели → спроси о прогрессе.',
            ...
            'advice_seeking': '🎯 СОВЕТ: СНАЧАЛА проверь какие данные есть в ПРОФИЛЕ и ФОКУСЕ. Задай 1-2 уточняющих. Затем ОДНУ рекомендацию с аргументами, привязанную к его ситуации.',
            'emotional_sharing': '🎯 ЭМОЦИИ: Эмпатия ПЕРВАЯ. Слушать, не решать. Один вопрос.',
            'general': '🎯 ОБЩЕЕ: Проведи инвентаризацию данных — что знаешь, что нет. Пустые поля профиля — узнай естественно. Если проблема — сначала уточняющие вопросы. Тихо update_profile если узнал новое.'
        }
        if intent in intent_hints:
            hints.append(intent_hints[intent])

        # Стратегия ответа
        hints.append("🧠 СТРАТЕГИЯ: Перед ответом проведи инвентаризацию: что о человеке ЗНАЕШЬ (ПРОФИЛЬ, ФОКУС), что НЕ ЗНАЕШЬ (пустые поля). Пустые поля = узнай через живой вопрос. Одно действие максимум ценности.")
